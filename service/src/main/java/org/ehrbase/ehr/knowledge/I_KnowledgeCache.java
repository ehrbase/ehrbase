...
public interface I_KnowledgeCache {
  ...
  /**
   * Deletes a given operational template physically from cache and from template storage and from
   * cache. Should only be executed if the template is no longer referenced by any Composition. Make
   * sure you check for references before deleting a template otherwise this causes inconsistencies
   * and no longer deliverable Composition entries.
   *
