...
public class ContainsProposition {
  ...
  public ContainsProposition(
      ParserRuleContext parserRuleContext, IdentifierMapper identifierMapper) {
    this.parserRuleContext = parserRuleContext;
    this.identifierMapper = identifierMapper;
  }

  public boolean isSingleChain() {
    if (!(parserRuleContext instanceof AqlParser.ContainsExpressionContext)) return false;
    else
  ...
  public List<Object> develop(ContainPropositions containPropositions) {
    ...
    for (ParseTree child : parserRuleContext.children) {
      if (child instanceof TerminalNode) objects.add(new ContainOperator(child.getText()));
      else {
        ...
        if (containsContext.getChildCount() > 1) {
          if (!new CommonTokenCompare(containsContext.getChild(1)).equals(AqlLexer.CONTAINS))
            throw new IllegalStateException(
                "Invalid contains context:" + containsContext.getText());
