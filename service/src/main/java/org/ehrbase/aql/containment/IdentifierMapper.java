 ...
 *
 * <p>The query strategy specifies whether an identifier is associated to an ARCHETYPED structure or
 * is bound to "static" data (such as a composition composer) Created by christian on 4/12/2016.
 */
    ...
    for (Map.Entry<String, Mapper> containment : mapper.entrySet()) {
      if (containment.getValue().getContainer() instanceof Containment
          && ((Containment) containment.getValue().getContainer())
              .getClassName()
              .equals("COMPOSITION")) return (Containment) containment.getValue().getContainer();
    }
  ...
  public String getClassName(String symbol) {
    ...
    if (definition == null)
      throw new IllegalArgumentException(
          "Identifier is not defined in FROM/CONTAIN clause:" + symbol);
  ...
  public boolean isUseSimpleCompositionContainment() {
    ...
    for (Map.Entry map : mapper.entrySet()) {
      ...
      if (mapper1.getContainer() instanceof Containment) {
        ...
        if (!containment.getClassName().equalsIgnoreCase("COMPOSITION")
            && containment.getArchetypeId() != null
            && !containment.getArchetypeId().isBlank()) {
