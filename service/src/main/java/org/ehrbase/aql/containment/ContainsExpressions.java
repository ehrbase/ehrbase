...
public class ContainsExpressions {
  ...
  public ContainsExpressions(Object containExpression) {
    if (containExpression instanceof AqlParser.ContainExpressionBoolContext)
      this.containExpressionBoolContext =
          (AqlParser.ContainExpressionBoolContext) containExpression;
      ...
      throw new IllegalArgumentException(
          "Invalid/unhandled ParseTree:" + containExpression.getClass());
  }
  ...
  public boolean isExplicitContainsClause() {
    if (containExpressionBoolContext.getChild(0) instanceof AqlParser.ContainsContext)
      if (containExpressionBoolContext.getChild(0).getChildCount() > 1)
        return new CommonTokenCompare(containExpressionBoolContext.getChild(0).getChild(1))
            .equals(AqlLexer.CONTAINS);

    return false;
  }

  private String childLabel(
      AqlParser.ArchetypedClassExprContext archetypedClassExprContext, boolean fullLabel) {
    if (fullLabel) return archetypedClassExprContext.getText();
    else return archetypedClassExprContext.getChild(1).getText();
  }

  private String childLabel(
      AqlParser.SimpleClassExprContext simpleClassExprContext, boolean fullLabel) {
    if (simpleClassExprContext.getChild(0) instanceof AqlParser.ArchetypedClassExprContext)
      return childLabel(
          (AqlParser.ArchetypedClassExprContext) simpleClassExprContext.getChild(0), fullLabel);
    return UNDEFINED_;
  }

  private String childLabel(
      AqlParser.ContainsExpressionContext containsExpressionContext, boolean fullLabel) {
    if (containsExpressionContext.getChild(0) instanceof AqlParser.ContainsContext)
      return childLabel(
          (AqlParser.ContainsContext) containsExpressionContext.getChild(0), fullLabel);
    else if (containsExpressionContext.getChild(0)
        instanceof AqlParser.ContainExpressionBoolContext)
      return childLabel(
          (AqlParser.ContainExpressionBoolContext) containsExpressionContext.getChild(0),
          fullLabel);
    return UNDEFINED_;
  ...
  private String childLabel(AqlParser.ContainsContext containsContext, boolean fullLabel) {
    if (containsContext.getChildCount()
        == 3) // form: [...] contains [...], we take the right most expression
  ...
  private String childLabel(
      AqlParser.ContainExpressionBoolContext containExpressionBoolContext, boolean fullLabel) {
    if (containExpressionBoolContext.getChild(0) instanceof AqlParser.ContainsContext) {
      return childLabel(
          (AqlParser.ContainsContext) containExpressionBoolContext.getChild(0), fullLabel);
    }
    return UNDEFINED_;
  }
