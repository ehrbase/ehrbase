...
public class ContainsExpressions {
    ...
    public ContainsExpressions(AqlParser.ContainsExpressionContext containsExpressionContext) {
        this.containExpressionBoolContext = (AqlParser.ContainExpressionBoolContext) containsExpressionContext.getChild(0);
    }
    ...
    public ContainsExpressions(Object containExpression){
        if (containExpression instanceof AqlParser.ContainExpressionBoolContext)
            ...
            this.containExpressionBoolContext = (AqlParser.ContainExpressionBoolContext) ((AqlParser.ContainsExpressionContext)containExpression).getChild(0);
        else
            throw new IllegalArgumentException("Invalid/unhandled ParseTree:"+containExpression.getClass());
    ...
    private String childLabel(AqlParser.ArchetypedClassExprContext archetypedClassExprContext, boolean fullLabel){
        if (fullLabel)
            return archetypedClassExprContext.getText();
        else
            return archetypedClassExprContext.getChild(1).getText();
    }
    ...
    private String childLabel(AqlParser.ContainsExpressionContext containsExpressionContext, boolean fullLabel){
            if (containsExpressionContext.getChild(0) instanceof AqlParser.ContainsContext)
                ...
                return childLabel((AqlParser.ContainExpressionBoolContext )containsExpressionContext.getChild(0), fullLabel);
        return UNDEFINED_;
    }
