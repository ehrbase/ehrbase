  ...
  public List<Object> visitValueListItems(AqlParser.ValueListItemsContext ctx) {
    ...
    for (ParseTree tree : ctx.children) {
      if (tree instanceof AqlParser.OperandContext) {
        ...
        if (operandContext.STRING() != null) operand.add(operandContext.STRING().getText());
        else if (operandContext.BOOLEAN() != null) operand.add(operandContext.BOOLEAN().getText());
        else if (operandContext.INTEGER() != null) operand.add(operandContext.INTEGER().getText());
        else if (operandContext.DATE() != null) operand.add(operandContext.DATE().getText());
        else if (operandContext.FLOAT() != null) operand.add(operandContext.FLOAT().getText());
  ...
  public List<Object> visitInvokeExpr(AqlParser.InvokeExprContext ctx) {
    ...
    if (operationId.replace("'", "").equals("expand")) {
      try {
        ...
        throw new IllegalArgumentException(
            "Terminology server operation failed:'" + e.getMessage() + "'");
      }
  ...
  public List<Object> visitIdentifiedEquality(AqlParser.IdentifiedEqualityContext context) {
    ...
    for (ParseTree tree : context.children) {
      if (tree instanceof TerminalNodeImpl) {
        ...
        AqlParser.IdentifiedOperandContext operandContext =
            (AqlParser.IdentifiedOperandContext) tree;
        ...
        for (ParseTree child : operandContext.children) {
          if (child instanceof AqlParser.OperandContext) {
            ...
            AqlParser.IdentifiedPathContext identifiedPathContext =
                (AqlParser.IdentifiedPathContext) child;
