 ...
 *
 * <p>This pass uses the results of pass 1 to:
 *
 * <ul>
 *   <li>resolve AQL paths from symbols, example:
 *       c/items[at0002]/items[at0001]/value/value/magnitude
...
public class QueryCompilerPass2 extends AqlBaseListener {
  ...
  public void exitSelectExpr(AqlParser.SelectExprContext selectExprContext) {
    ...
    if (identifiedPathContext != null) {
      VariableDefinition variableDefinition =
          new IdentifiedPathVariable(identifiedPathContext, selectExprContext, isDistinct)
              .definition();
      ...
      if (selectExprContext.stdExpression().function() != null) {
        ...
        for (ParseTree pathTree : functionContext.children) {
          if (pathTree instanceof AqlParser.IdentifiedPathContext) {
            AqlParser.IdentifiedPathContext pathContext =
                (AqlParser.IdentifiedPathContext) pathTree;
            VariableDefinition variableDefinition =
                new IdentifiedPathVariable(pathContext, selectExprContext, false).definition();
        ...
        String alias =
            selectExprContext.IDENTIFIER() == null
                ? name
                : selectExprContext.IDENTIFIER().getText();
      ...
      if (selectExprContext.stdExpression().extension() != null) {
        ...
        String alias =
            selectExprContext.IDENTIFIER() == null
                ? "_alias_" + Math.abs(new Random().nextLong())
                : selectExprContext.IDENTIFIER().getText();
        ExtensionDefinition definition =
            new ExtensionDefinition(context, parsableExpression, alias);
        pushVariableDefinition(definition);
      }

    } else throw new IllegalArgumentException("Could not interpret select context");
  }
  ...
  private void isUnique(I_VariableDefinition variableDefinition) {
    ...
    if (variableDefinition.isFunction()) return;
  ...
  // identifiedPathContext.identifiedPath();
  //        String path = identifiedPathContext.objectPath().getText();
  //        String identifier = identifiedPathContext.IDENTIFIER().getText();
  //        String alias = null;
  //        Object parent = identifiedPathContext.getParent(); //either selectExpr or
  // identifiedOperand (no alias)
  //        if (parent instanceof AqlParser.SelectExprContext) {
  //            AqlParser.SelectExprContext selectExprContext = (AqlParser.SelectExprContext)
  // identifiedPathContext.getParent();
  //            if (selectExprContext.IDENTIFIER() != null)
  //                alias = selectExprContext.IDENTIFIER().getText();
  //
  //            VariableDefinition variableDefinition = new VariableDefinition(path, alias,
  // identifier);
  //            variableStack.push(variableDefinition);
  //        }
  //    }
  ...
  public void exitTopExpr(AqlParser.TopExprContext context) {
    ...
    if (context.TOP() != null) {
      ...
      if (context.BACKWARD() != null) direction = TopAttributes.TopDirection.BACKWARD;
      else if (context.FORWARD() != null) direction = TopAttributes.TopDirection.FORWARD;
    }
  ...
  public void exitOrderBySeq(AqlParser.OrderBySeqContext context) {
    if (orderAttributes == null) orderAttributes = new ArrayDeque<>();
    ...
    for (ParseTree tree : context.children) {
      if (tree instanceof AqlParser.OrderByExprContext) {
        ...
        String identifier =
            identifiedPathContext.IDENTIFIER().getText(); // f.e. 'e' in 'e/time_created/value

        OrderAttribute orderAttribute;
        if (path == null)
          orderAttribute =
              new OrderAttribute(new VariableDefinition(path, identifier, null, false));
        else
          orderAttribute =
              new OrderAttribute(new VariableDefinition(path, null, identifier, false));
  ...
  List<OrderAttribute> getOrderAttributes() {
    if (orderAttributes == null) return null;
