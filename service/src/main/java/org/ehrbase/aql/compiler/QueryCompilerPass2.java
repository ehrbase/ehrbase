...
public class QueryCompilerPass2 extends AqlBaseListener {
  ...
  private final String[] allowedFunctions = {"COUNT", "MIN", "MAX", "AVG", "SUM",
      "SUBSTR", "STRPOS", "SPLIT_PART", "BTRIM", "CONCAT", "CONCAT_WS", "DECODE", "ENCODE",
      "FORMAT", "INITCAP", "LEFT", "LENGTH", "LPAD", "LTRIM",
      "REGEXP_MATCH", "REGEXP_REPLACE", "REGEXP_SPLIT_TO_ARRAY", "REGEXP_SPLIT_TO_TABLE", "REPEAT",
  ...
  public void exitSelectExpr(AqlParser.SelectExprContext selectExprContext) {
    ...
    if (identifiedPathContext != null) {
      ...
      variableDefinition = new IdentifiedPathVariable(identifiedPathContext, selectExprContext,
          isDistinct, predicateDefinitionMap.get(selectExprContext)).definition();
      ...
      if (selectExprContext.stdExpression().function() != null) {
        ...
        handleCastFunctionDefinition(selectExprContext.stdExpression().castFunction(),
            selectExprContext);
  ...
  @Override
  public void exitNodePredicateComparable(
      AqlParser.NodePredicateComparableContext nodePredicateComparableContext) {
    String operand1;
    ...
    if (ruleContext instanceof AqlParser.SelectExprContext) {
      PredicateDefinition predicateDefinition = new PredicateDefinition(operand1, operator,
          operand2);
  ...
  private void handleFunctionDefinition(AqlParser.FunctionContext functionContext,
      AqlParser.SelectExprContext inSelectExprContext) {
    ...
    for (ParseTree pathTree : functionContext.children) {
      if (pathTree instanceof AqlParser.IdentifiedPathContext) {
        ...
        VariableDefinition variableDefinition = new IdentifiedPathVariable(pathContext,
            inSelectExprContext, false, null).definition();
        ...
        pushVariableDefinition(variableDefinition);
        parameters.add(new FuncParameter(FuncParameterType.VARIABLE,
            variableDefinition.getAlias() == null ? variableDefinition.getPath()
  ...
  private void handleCastFunctionDefinition(AqlParser.CastFunctionContext castFunctionContext,
      AqlParser.SelectExprContext inSelectExprContext) {
  ...
  public void handleExtensionDefinition(AqlParser.ExtensionContext extensionContext,
      AqlParser.SelectExprContext inSelectExprContext) {
    ...
    String alias =
        inSelectExprContext.IDENTIFIER() == null ? "_alias_" + random.nextLong()
            : inSelectExprContext.IDENTIFIER().getText();
    ExtensionDefinition definition = new ExtensionDefinition(context, parsableExpression, alias);
    pushVariableDefinition(definition);
  }

  public void handleTerminalNodeExpression(AqlParser.StdExpressionContext inStdExpressionContext,
      AqlParser.SelectExprContext inSelectExprContext) {
    ...
    ConstantDefinition definition = new ConstantDefinition(value,
        extractAlias(inSelectExprContext));
    pushVariableDefinition(definition);
  }
  ...
  private String extractAlias(AqlParser.SelectExprContext inSelectExprContext) {
    ...
    if (inSelectExprContext.getChildCount() == 3 && inSelectExprContext.getChild(1).getText()
        .equalsIgnoreCase("AS")) {
  ...
  public void exitOrderBySeq(AqlParser.OrderBySeqContext context) {
    ...
    for (ParseTree tree : context.children) {
      if (tree instanceof AqlParser.OrderByExprContext) {
        ...
        String identifier = identifiedPathContext.IDENTIFIER()
            .getText();  //f.e. 'e' in 'e/time_created/value

        OrderAttribute orderAttribute;
        if (path == null) {
          orderAttribute = new OrderAttribute(
              new VariableDefinition(path, identifier, null, false));
        } else {
          orderAttribute = new OrderAttribute(
