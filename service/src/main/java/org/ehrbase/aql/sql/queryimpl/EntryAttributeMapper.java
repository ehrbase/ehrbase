...
/**
 * Map a datavalue UML expression from an ARCHETYPED structure into its RM/JSON representation see
 * http://www.openehr.org/releases/trunk/UML/ for field identification Created by christian on
 * 5/11/2016.
 */
...
public class EntryAttributeMapper {
  ...
  private static Integer firstOccurence(int offset, List<String> list, String match) {
    for (int i = offset; i < list.size(); i++) {
      if (list.get(i).equals(match)) return i;
    }
    return null;
  }
  ...
  public static String map(String attribute) {
    ...
    if (fields.get(0).equals(ISM_TRANSITION)) {
      ...
      fields.add(
          1,
          "0"); // name is now formatted as /name -> array of values! Required to deal with cluster
                // items
      ...
      if (match != null) { // deals with "/value/value"
        ...
        if ((ndxInterval = intervalValueIndex(fields)) > 0) { // interval
          ...
          if (match > 1 && fields.get(match - 1).equals("name")) fields.set(match, VALUE);
          else
            // usual /value
    ...
    for (int i = floor; i < fields.size(); i++) {
      ...
      if (fields.get(i).equalsIgnoreCase("NAME")) {
        ...
        fields.set(i, "/name,0");
      } else fields.set(i, NodeIds.toCamelCase(fields.get(i)));
    }
