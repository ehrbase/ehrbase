...
public class GenericJsonField extends RMObjectAttribute {
  ...
  public Field partyRef(
      Field<String> namespace, Field<String> type, Field<String> scheme, Field<String> value) {
  ...
  public Field jsonField(String rmType, Object function, TableField... tableFields) {
    ...
    if (sqlExpression.toString().contains(QueryImplConstants.AQL_NODE_ITERATIVE_FUNCTION)
  ...
  private Field fieldWithJsonArrayIteration(
    ...
    String[] prefix =
        tokenized.subList(0, tokenized.indexOf(AQLNODEITERATIVE)).toArray(new String[] {});
    String[] remaining =
        tokenized
            .subList(tokenized.indexOf(AQLNODEITERATIVE) + 1, tokenized.size())
            .toArray(new String[] {});
    ...
    while (remaining.length > 0) {
      ...
      if (tokens.contains(AQLNODEITERATIVE)) {
        ...
        remaining =
            tokens
                .subList(tokens.indexOf(AQLNODEITERATIVE) + 1, tokens.size())
                .toArray(new String[] {});
      } else {
        prefix = remaining;
