...
public class JsonbEntryQuery extends ObjectQuery implements IQueryImpl {
  ...
  private static final String JSONB_SELECTOR_COMPOSITION_OPEN =
      ENTRY.ENTRY_ + JSONB_PATH_SELECTOR_EXPR;
  public static final String JSQUERY_COMPOSITION_OPEN = ENTRY.ENTRY_ + JSONB_AT_AT_SELECTOR_EXPR;

  // OTHER_DETAILS (Ehr Status Query)
  private static final String SELECT_EHR_OTHER_DETAILS_MACRO =
      JoinBinder.statusRecordTable.field(STATUS.OTHER_DETAILS)
          + "->('"
          + CompositionSerializer.TAG_OTHER_DETAILS
          + "')";
  private static final String JSONB_SELECTOR_EHR_OTHER_DETAILS_OPEN =
      SELECT_EHR_OTHER_DETAILS_MACRO + JSONB_PATH_SELECTOR_EXPR;

  // OTHER_CONTEXT (Composition context other_context Query)
  private static final String SELECT_EHR_OTHER_CONTEXT_MACRO =
      EVENT_CONTEXT.OTHER_CONTEXT
          + "->('"
          + CompositionSerializer.TAG_OTHER_CONTEXT
          + "[at0001]"
          + "')";
  private static final String JSONB_SELECTOR_EHR_OTHER_CONTEXT_OPEN =
      SELECT_EHR_OTHER_CONTEXT_MACRO + JSONB_PATH_SELECTOR_EXPR;
  public static final String JSQUERY_EHR_OTHER_CONTEXT_OPEN =
      SELECT_EHR_OTHER_CONTEXT_MACRO + JSONB_AT_AT_SELECTOR_EXPR;
  ...
  public static final String MATCH_NODE_PREDICATE =
      "(/(content|events|protocol|data|description|instruction|items|activities|activity|composition|entry|evaluation|observation|action)\\[([(0-9)|(A-Z)|(a-z)|\\-|_|\\.]*)\\])|"
  ...
  private static final String[] listIdentifier = {
    TAG_CONTENT, TAG_ITEMS, TAG_ACTIVITIES, TAG_EVENTS
  };

  private boolean containsJqueryPath =
      false; // true if at leas one AQL path is contained in expression
  ...
  public JsonbEntryQuery(
      I_DomainAccess domainAccess, IntrospectService introspectCache, PathResolver pathResolver) {
  ...
  // this is required to encode structure like events of events (events:{events[at0001] ...
  // events[at000x]}
  ...
  public List<String> jqueryPath(PATH_PART pathPart, String path, String defaultIndex) {
    // CHC 160607: this offset (1 or 0) was required due to a bug in generating the containment
    // table
    ...
    for (int i = offset; i < segments.size(); i++) {
      ...
      if (isList(nodeId)
          && ((pathPart.equals(PATH_PART.VARIABLE_PATH_PART) && !(i == segments.size() - 1))
              || pathPart.equals(PATH_PART.IDENTIFIER_PATH_PART))) jqueryPath.add(defaultIndex);
    }

    if (pathPart.equals(PATH_PART.VARIABLE_PATH_PART)) {
      StringBuilder stringBuilder = new StringBuilder();
      for (int i = jqueryPath.size() - 1; i >= 0; i--) {
        if (jqueryPath.get(i).matches("[0-9]*|#") || jqueryPath.get(i).contains("[at")) break;
    ...
    if (pathPart.equals(PATH_PART.VARIABLE_PATH_PART)
        && jqueryPath.get(jqueryPath.size() - 1).matches(MATCH_NODE_PREDICATE)) {
      jsonDataBlock = true;
    }
  ...
  public Field<?> makeField(
      OTHER_ITEM type, String path, String alias, String variablePath, boolean withAlias) {
    List<String> itemPathArray = new ArrayList<>();

    if (path != null) itemPathArray.addAll(jqueryPath(PATH_PART.IDENTIFIER_PATH_PART, path, "0"));
    ...
    itemPath =
        wrapQuery(
            itemPath,
            type.equals(OTHER_ITEM.OTHER_DETAILS)
                ? JSONB_SELECTOR_EHR_OTHER_DETAILS_OPEN
                : JSONB_SELECTOR_EHR_OTHER_CONTEXT_OPEN,
            JSONB_SELECTOR_CLOSE);

    if (itemPathArray
        .get(itemPathArray.size() - 1)
        .contains(MAGNITUDE)) { // force explicit type cast for DvQuantity
    ...
    } else fieldPathItem = DSL.field(itemPath, String.class);
  ...
  public Field<?> makeField(
    ...
    boolean isRootContent =
        false; // that is a query path on a full composition starting from the root content

    if (pathResolver.entryRoot(templateId)
        == null) // case of (invalid) composition with null entry!
    return null;
    ...
    if (variableDefinition.getPath() != null && variableDefinition.getPath().startsWith(CONTENT)) {
      ...
      isRootContent = true;
    } else path = pathResolver.pathOf(templateId, variableDefinition.getIdentifier());
    ...
    itemPathArray.addAll(
        jqueryPath(PATH_PART.VARIABLE_PATH_PART, variableDefinition.getPath(), "0"));
    ...
    try {
      ...
      if (reducedItemPathArray != null && !reducedItemPathArray.isEmpty()) {
        ...
        if (itemType != null) {
          ...
          if (pgType != null) itemPath = "(" + itemPath + ")::" + pgType;
        }
      }
    } catch (Exception e) {
      ...
      else logger.warn("Ignoring unresolved introspect (reason:" + e + ")");
    }
    if (itemPathArray
        .get(itemPathArray.size() - 1)
        .contains(MAGNITUDE)) { // force explicit type cast for DvQuantity
  ...
  @Override
  public Field<?> whereField(
      String templateId, String identifier, I_VariableDefinition variableDefinition) {
    ...
    itemPathArray.addAll(
        jqueryPath(PATH_PART.VARIABLE_PATH_PART, variableDefinition.getPath(), "#"));
  ...
  private static String wrapQuery(String itemPath, String open, String close) {
    if (itemPath.contains("/item_count")) {
      ...
      return QueryImplConstants.AQL_NODE_ITERATIVE_FUNCTION
          + "("
          + CONTENT
          + " #> '{"
          + pathPart
