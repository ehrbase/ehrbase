...
/**
 * check if a where variable represents a temporal object. This is used to apply proper type casting
 * and relevant operator using EPOCH_OFFSET instead of string value when dealing with date/time
 * comparison in json structure
 */
  ...
  public boolean containsTemporalItem(VariableDefinition variableDefinition) {
    ...
    for (Object item : whereItems.subList(pos, whereItems.size())) {
      ...
      if (item instanceof String) { // ignore variable definition
        if (new DateTimes((String) item).isDateTimeZoned()) return true;
      }
    }
    return false;
  }
