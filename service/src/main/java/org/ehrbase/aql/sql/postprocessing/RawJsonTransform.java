                ...
                try {
                  ...
                  if (new ResultBlock(jsonbBlockDef).isCanonical()) {
                    ...
                    JsonElement item =
                        gsonRaw
                            .create()
                            .toJsonTree(gsonRaw.create().fromJson(jsonbOrigin, List.class));
                    if (item instanceof JsonArray) jsonElement = item.getAsJsonArray();
                    else jsonElement = item.getAsJsonObject();
                  } else {
                    // this allows to deal with json array without impacting the structure of json
                    // transform
                    // as it deals with complex arrays such as items[...], content[...] etc.
                    // hence, we pass the array as a a simple key-value and then retrieve the value
                    // part
                    // referenced by "$array$"
                    if (jsonbOrigin.startsWith("[") && jsonbOrigin.endsWith("]"))
                      jsonbOrigin = "{\"$array$\":" + jsonbOrigin + "}";
                    jsonElement =
                        new LightRawJsonEncoder(jsonbOrigin)
                            .encodeContentAsJson(jsonbBlockDef.getJsonPathRoot());
                    if (jsonElement.getAsJsonObject().has(ARRAY_MARKER)) {
                      if (hasPredicate(jsonbBlockDef.getPath())) // f.e. events[at0002]
                      jsonElement =
                            jsonElement.getAsJsonObject().getAsJsonArray(ARRAY_MARKER).get(0);
