...
public class JsonbEntryQuery extends ObjectQuery implements I_QueryImpl {
  ...
  private static final String JSONBSelector_COMPOSITION_OPEN = ENTRY.ENTRY_ + " #>> '{";
  public static final String Jsquery_COMPOSITION_OPEN = ENTRY.ENTRY_ + " @@ '";

  // OTHER_DETAILS (Ehr Status Query)
  private static final String SELECT_EHR_OTHER_DETAILS_MACRO =
      I_JoinBinder.statusRecordTable.field(STATUS.OTHER_DETAILS)
          + "->('"
          + CompositionSerializer.TAG_OTHER_DETAILS
          + "')";
  private static final String JSONBSelector_EHR_OTHER_DETAILS_OPEN =
      SELECT_EHR_OTHER_DETAILS_MACRO + " #>> '{";
  public static final String Jsquery_EHR_OTHER_DETAILS_OPEN =
      SELECT_EHR_OTHER_DETAILS_MACRO + " @@ '";

  // OTHER_CONTEXT (Composition context other_context Query)
  // TODO: make the prefix dependant on the actual passed argument (eg.
  ...
  private static final String SELECT_EHR_OTHER_CONTEXT_MACRO =
      EVENT_CONTEXT.OTHER_CONTEXT
          ...
          + "[at0001]"
          + "')";
  private static final String JSONBSelector_EHR_OTHER_CONTEXT_OPEN =
      SELECT_EHR_OTHER_CONTEXT_MACRO + " #>> '{";
  ...
  private static final String listIdentifier[] = {"/content", "/items", TAG_ACTIVITIES, TAG_EVENTS};
  ...
  public List<String> jqueryPath(PATH_PART path_part, String path, String defaultIndex) {
    ...
    for (int i = offset; i < segments.size(); i++) {
      ...
      if (isList(nodeId)) {
        ...
        else if (path_part.equals(PATH_PART.IDENTIFIER_PATH_PART)) jqueryPath.add(defaultIndex);
      }
    }
    ...
    if (path_part.equals(PATH_PART.VARIABLE_PATH_PART)
        && jqueryPath.get(jqueryPath.size() - 1).matches(matchNodePredicate)) {
      jsonDataBlock = true;
    }
  ...
  public Field<?> makeField(
    ...
    Collections.replaceAll(
        referenceItemPathArray, QueryImplConstants.AQL_NODE_ITERATIVE_MARKER, "0");
    ...
    if (itemPathArray.contains(QueryImplConstants.AQL_NODE_NAME_PREDICATE_MARKER))
      ...
      itemPathArray =
          new JsonbFunctionCall(
                  itemPathArray,
                  QueryImplConstants.AQL_NODE_ITERATIVE_MARKER,
  ...
  public Field<?> whereField(
    ...
    if (pathResolver.entryRoot(templateId) == null) {
      ...
      throw new IllegalArgumentException(
          "a name/value expression for composition must be specified, where clause cannot be built without");
    }
