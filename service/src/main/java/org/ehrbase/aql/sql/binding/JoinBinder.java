...
public class JoinBinder implements IJoinBinder {
  ...
  public static final Table<CompositionRecord> compositionRecordTable =
      COMPOSITION.as(COMPOSITION_JOIN);
  ...
  public static final Table<PartyIdentifiedRecord> composerRef =
      PARTY_IDENTIFIED.as("composer_ref");
  public static final Table<PartyIdentifiedRecord> subjectRef = PARTY_IDENTIFIED.as("subject_ref");
  public static final Table<PartyIdentifiedRecord> facilityRef =
      PARTY_IDENTIFIED.as("facility_ref");
  ...
  public SelectQuery<?> addJoinClause(CompositionAttributeQuery compositionAttributeQuery) {
    ...
    if (compositionAttributeQuery == null) return selectQuery;
    ...
    if (!compositionAttributeQuery.isUseEntry() && noJoinRequired(compositionAttributeQuery)) {
      ...
      if (compositionAttributeQuery.isJoinEhrStatus()
          || compositionAttributeQuery.containsEhrStatus()) {
  ...
  private boolean noJoinRequired(CompositionAttributeQuery compositionAttributeQuery) {
    return (compositionAttributeQuery.isJoinComposer() ? 2 : 0)
            ...
            + (compositionAttributeQuery.isJoinSystem() ? 1 : 0)
        == 1;
  }

  private SelectQuery<?> simpleFromClause(
      SelectQuery<?> selectQuery, CompositionAttributeQuery compositionAttributeQuery) {
    ...
    if (compositionAttributeQuery.isJoinComposition() || compositionAttributeQuery.useFromEntry()) {
      ...
      selectQuery.addJoin(
          ...
          DSL.field(statusRecordTable.field(STATUS.EHR_ID.getName(), UUID.class))
              .eq(
                  DSL.field(
                      compositionRecordTable.field(COMPOSITION.EHR_ID.getName(), UUID.class))));
      statusJoined = true;
  ...
  private void joinContextFacility(SelectQuery<?> selectQuery) {
    ...
    selectQuery.addJoin(
        facilityTable,
        EVENT_CONTEXT.FACILITY.eq(
            DSL.field(facilityTable.field(PARTY_IDENTIFIED.ID.getName(), UUID.class))));
    facilityJoined = true;
  }
