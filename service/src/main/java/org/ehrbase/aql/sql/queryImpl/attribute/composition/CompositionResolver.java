...
public class CompositionResolver extends AttributeResolver {
  ...
  public Field<?> sqlField(String path) {
    ...
    if (path.startsWith(FEEDER_AUDIT)) {
      ...
      Field<?> retField =
          new GenericJsonField(fieldResolutionContext, joinSetup)
              .forJsonPath(FEEDER_AUDIT, path)
              .jsonField(
                  "FEEDER_AUDIT",
                  null,
                  I_JoinBinder.compositionRecordTable.field(FEEDER_AUDIT, JSONB.class));

      String regexpTerminalValues =
          ".*(id|issuer|assigner|type|original_content|system_id|name|namespace|type|value)$";
    ...
    switch (path) {
      ...
      case "name/value":
        return new GenericJsonField(fieldResolutionContext, joinSetup)
            .forJsonPath("value")
            .jsonField("DV_CODED_TEXT", "ehr.js_dv_coded_text", ENTRY.NAME);
      ...
      case "language/value":
        return new SimpleCompositionAttribute(fieldResolutionContext, joinSetup)
            .forTableField(COMPOSITION.LANGUAGE)
            .sqlField();
      case "territory/value":
        return new SimpleCompositionAttribute(fieldResolutionContext, joinSetup)
            .forTableField(COMPOSITION.TERRITORY)
            .sqlField();
