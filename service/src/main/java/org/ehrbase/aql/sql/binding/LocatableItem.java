...
/**
 * evaluate the SQL expression for locatables in the ITEM_STRUCTURE: OBSERVATION, INSTRUCTION,
 * CLUSTER etc. NB. At the moment, direct resolution of ELEMENT is not supported.
 */
...
public class LocatableItem {
  ...
  public LocatableItem(
      CompositionAttributeQuery compositionAttributeQuery, JsonbEntryQuery jsonbEntryQuery) {
    this.compositionAttributeQuery = compositionAttributeQuery;
    this.jsonbEntryQuery = jsonbEntryQuery;
  }

  public Field<?> toSql(
      String templateId, I_VariableDefinition variableDefinition, String className) {
    Field<?> field;
    ...
    if (jsonbEntryQuery.isJsonDataBlock()) {
      ...
      if (jsonbEntryQuery.getItemType() != null) {
        ...
        if (itemClass == null
            && className != null) // this may occur f.e. for itemType 'MULTIPLE'. try we classname
        ...
        if (DataValue.class.isAssignableFrom(itemClass)) {
          ...
          if (variableAqlPath.getSuffix().equals("value")) {
            if (Objects.equals(
                className, "COMPOSITION")) { // assumes this is a data value within an ELEMENT
              try {
                ...
                throw new InternalServerException(
                    "Couldn't handle variable:"
                        + variableDefinition.toString()
                        + "Code error:"
                        + e);
              }
            } else if (jsonbEntryQuery.getItemCategory().equals("ELEMENT")
                || jsonbEntryQuery.getItemCategory().equals("CLUSTER")) {
              int cut = jsonbItemPath.lastIndexOf(",/value");
              if (cut != -1)
                // we keep the path that select the json element value block, and call the
                // formatting function
                // to pass the actual value datatype into the json block
                field =
                    DSL.field(
                        "(ehr.js_typed_element_value("
                            + jsonbItemPath.substring(0, cut)
                            + "}')::jsonb))");
        ...
        throw new IllegalStateException(
            "Internal: unsupported item type for:" + variableDefinition);
    }
    return field;
  }
