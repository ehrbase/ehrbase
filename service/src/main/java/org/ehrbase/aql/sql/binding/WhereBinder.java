...
/**
 * Bind the abstract WHERE clause parameters into a SQL expression Created by christian on
 * 5/20/2016.
 */
// ignore cognitive complexity flag as this code as evolved historically and adjusted depending on
// use cases
...
public class WhereBinder {
  ...
  // from AQL grammar
  private static final Set<String> sqloperators =
      new HashSet<>(
          Arrays.asList("=", "!=", ">", ">=", "<", "<=", MATCHES, EXISTS, NOT, "(", ")", "{", "}"));
  ...
  private String sqlConditionalFunctionalOperatorRegexp =
      "(?i)(like|ilike|in|not in)"; // list of subquery and operators
  ...
  private TaggedStringBuilder encodeWhereVariable(
    ...
    if (className == null)
      throw new IllegalArgumentException(
          "Could not bind identifier in WHERE clause:'" + identifier + "'");
    Field<?> field;
    ...
    else usePgExtensions = false;

    if (forceSQL || Boolean.FALSE.equals(usePgExtensions)) {
      // EHR-327: also supports EHR attributes in WHERE clause
      if ((className.equals(COMPOSITION) && !variableDefinition.getPath().contains(CONTENT))
          || className.equals(EHR)) {
        field = compositionAttributeQuery.whereField(templateId, identifier, variableDefinition);
      } else { // should be removed (?)
        field =
            jsonbEntryQuery.makeField(
                templateId, identifier, variableDefinition, IQueryImpl.Clause.WHERE);
      }
      ...
      switch (className) {
        case COMPOSITION:
          if (variableDefinition.getPath().startsWith(CONTENT)) {
            ...
            TaggedStringBuilder taggedStringBuilder =
                new TaggedStringBuilder(field.toString(), I_TaggedStringBuilder.TagField.JSQUERY);
        ...
        default:
          if (compositionAttributeQuery.isCompositionAttributeItemStructure(
            ...
            field =
                new ContextualAttribute(
                        compositionAttributeQuery, jsonbEntryQuery, IQueryImpl.Clause.WHERE)
                    .toSql(templateId, variableDefinition);
            return new TaggedStringBuilder(
                field.toString(), I_TaggedStringBuilder.TagField.SQLQUERY);
          } else {
            field = jsonbEntryQuery.whereField(templateId, identifier, variableDefinition);
            return new TaggedStringBuilder(
                field.toString(), I_TaggedStringBuilder.TagField.JSQUERY);
          }
      }
  ...
  private TaggedStringBuilder buildWhereCondition(
      String templateId, TaggedStringBuilder taggedBuffer, List<Object> item) {
    for (Object part : item) {
      if (part instanceof String) taggedBuffer.append((String) part);
        ...
        TaggedStringBuilder taggedStringBuilder =
            encodeWhereVariable(templateId, (VariableDefinition) part, false, null);
        ...
        TaggedStringBuilder taggedStringBuilder =
            buildWhereCondition(templateId, taggedBuffer, (List) part);
        taggedBuffer.append(taggedStringBuilder.toString());
        taggedBuffer.setTagField(taggedStringBuilder.getTagField());
      }
    }
  ...
  public Condition bind(String templateId) {
    ...
    for (int cursor = 0; cursor < whereItems.size(); cursor++) {
      ...
      if (item instanceof String) {
        ...
        if (isFollowedBySQLConditionalOperator(cursor)) {
          String expanded =
              expandForCondition(
                  encodeWhereVariable(templateId, (I_VariableDefinition) item, true, null));
          if (expanded != null) taggedStringBuilder.append(expanded);
          else {
          ...
          if (((I_VariableDefinition) item).getPath() != null && isWholeComposition) {
            ...
            if (compositionName != null) {
              taggedStringBuilder =
                  encodeWhereVariable(
                      templateId, (I_VariableDefinition) item, false, compositionName);
            } else
              throw new IllegalArgumentException(
            ...
            if (new VariablePath(((I_VariableDefinition) item).getPath()).hasPredicate()) {
              ...
              if (item instanceof VariableDefinition
                      && new WhereTemporal(whereItems)
                          .containsTemporalItem((VariableDefinition) item)
                  || new WhereEvaluation(whereItems).requiresSQL()) {
                String expanded =
                    expandForCondition(
                        encodeWhereVariable(templateId, (I_VariableDefinition) item, true, null));
                if (expanded != null) taggedStringBuilder.append(expanded);
                ...
                String expanded =
                    expandForCondition(
                        encodeWhereVariable(templateId, (I_VariableDefinition) item, false, null));
                if (expanded != null) taggedStringBuilder.append(expanded);
                else {
                  unresolvedVariable = true;
                  break;
        ...
        TaggedStringBuilder taggedStringBuilder =
            buildWhereCondition(templateId, taggedBuffer, (List) item);
        taggedBuffer.append(taggedStringBuilder.toString());
        taggedBuffer.setTagField(taggedStringBuilder.getTagField());
      }
    }
    ...
    } else return DSL.condition("false");
  }
    ...
    for (lcursor = 0; lcursor < whereClause.size() - 1; lcursor++) {
      if (whereClause.get(lcursor) instanceof VariableDefinition
        ...
        if (nextToken instanceof String && !nextToken.equals("="))
          throw new IllegalArgumentException(
              "name/value for CompositionAttribute must be an equality");
