...
public class IterativeNode implements IIterativeNode {
  ...
  public IterativeNode(
      I_DomainAccess domainAccess, String templateId, IntrospectService introspectCache) {
      ...
      for (int i = unbounded.size() - 1; i >= 0; i--) {
        ...
        if (ignoreIterativeNode != null && !ignoreIterativeNode.isEmpty()) {
          for (String ignoreItemRegex : ignoreIterativeNode) {
            if (aqlPathSegments
                .get(aqlPathSegments.size() - 1)
                .matches("^" + ignoreItemRegex + ".*")) {
      ...
      try {
        ...
        try {
          ...
          throw new IllegalArgumentException(
              "Drift in locating array marker: aql:"
                  + aqlSegmented.get(aqlSegIndex)
                  + ", jsonb:"
                  + jsonbSegmented.get(retval)
                  + ", @index:"
                  + retval);
        }
  ...
  private void initAqlRuntimeParameters() {
    ...
    if (System.getenv(ENV_AQL_ARRAY_IGNORE_NODE) != null) {
      ...
      Arrays.asList(domainAccess.getServerConfig().getAqlIterationSkipList().split(",")).stream()
          .forEach(ignoreIterativeNode::add);
