...
public class GenericJsonPath {
  ...
  public String jqueryPath() {
    ...
    for (int i = 0; i < jqueryPaths.size(); i++) {
      ...
      } else if (segment.matches(OTHER_DETAILS + "|" + OTHER_CONTEXT)) actualPaths.add(segment);
      else if (isNonItemStructureAttribute(path)) actualPaths.add(segment);
      else actualPaths.add(NodeIds.toCamelCase(segment));
    }
  ...
  public boolean isTerminalValue(List<String> paths, int index) {
    return paths.size() == 1
        || (paths.size() > 1
            ...
            // match node predicate regexp starts with '/' which is not the case when splitting the
            // path
            && !paths.get(index - 1).matches(I_DvTypeAdapter.matchNodePredicate.substring(1)));
  }

  /**
   * identifies if the encoding comes from a db canonical function NB. the DB encoding still uses
   * camel case instead of snake case for datavalues
   *
