 ...
 *
 * <p>The strategy depends on whether the query contains elements which path depends on the context
 * (e.g. a composition).
 *
 * <ul>
 ...
 * </ul>
 *
 * <p>Created by christian on 4/28/2016.
 */
...
public class QueryProcessor extends TemplateMetaData {
  ...
  static class AqlSelectQuery {
    ...
    AqlSelectQuery(
        ...
        Collection<QuerySteps> querySteps,
        boolean outputWithJson) {
  ...
  AqlSelectQuery buildAqlSelectQuery() {
    ...
    for (QuerySteps queryStep : cacheQuery.values()) {
      ...
      select =
          new JoinBinder(domainAccess, select, false)
              .addJoinClause(queryStep.getCompositionAttributeQuery());
      ...
      if (condition != null) select.addConditions(Operator.AND, condition);

      if (first) {
        unionSetQuery = select;
        first = false;
      } else unionSetQuery.unionAll(select);
    }
    ...
    if (new Variables(statements.getVariables()).hasDefinedDistinct()
      ...
      SuperQuery superQuery =
          new SuperQuery(domainAccess, statements.getVariables(), unionSetQuery);
      ...
      unionSetQuery =
          new SuperQuery(domainAccess, statements.getVariables(), unionSetQuery)
              .selectOrderBy(statements.getOrderAttributes());
    }
  ...
  private List<List<String>> buildExplain(Select<?> select) {
    ...
    DSLContext pretty =
        DSL.using(domainAccess.getContext().dialect(), new Settings().withRenderFormatted(true));
