...
/**
 * Bind the abstract representation of a SELECT clause into a SQL expression Created by christian on
 * 5/4/2016.
 */
public class SelectBinder extends TemplateMetaData implements I_SelectBinder {
  ...
  public SelectQuery<Record> bind(String template_id) {
    ...
    while (variableDefinitions.hasNext()) {
      ...
      handleJsonDataBlock(
          expressionField,
          field,
          expressionField.getRootJsonKey(),
          expressionField.getOptionalPath());
  ...
  private void handleJsonDataBlock(
      ExpressionField expressionField, Field field, String rootJsonKey, String optionalPath) {
    if (expressionField.isContainsJsonDataBlock())
      jsonDataBlock.add(
          new JsonbBlockDef(
              optionalPath == null ? expressionField.getJsonbItemPath() : optionalPath,
              field,
              rootJsonKey));
  }
