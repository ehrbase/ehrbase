...
public class EhrFolderRepository {
  ...
  public List<EhrFolderRecord> toRecord(UUID ehrId, Folder folder) {
    ...
    List<EhrFolderRecord> ehrFolderRecords =
       IntStream.range(0,flatten.size()).mapToObj(i -> toRecord(i,flatten.get(i), ehrId)).toList();
  ...
  private List<Triple<List<String>, List<Integer>, Folder>> flatten(Folder folder) {
    ...
    if (folder.getFolders() != null) {
      ...
      IntStream.range(0, folder.getFolders().size()).forEach(

          i -> flatten(folder.getFolders().get(i))
              .forEach(p -> {
