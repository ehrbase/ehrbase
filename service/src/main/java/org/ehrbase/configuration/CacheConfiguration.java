...
public class CacheConfiguration {
  ...
  @Bean
  public static CacheManager cacheManagerCustomizer(CacheConfiguration cacheProperties)
    ...
    if (cacheProperties.isEnabled()) {
      cacheManager =
          cachingProvider.getCacheManager(
              CacheConfiguration.class.getResource(cacheProperties.getConfigPath()).toURI(),
              CacheConfiguration.class.getClassLoader());
    } else {
      cacheManager = cachingProvider.getCacheManager();
    }
    buildCache(
        INTROSPECT_CACHE, UUID.class, WebTemplate.class, cacheManager, cacheProperties.isEnabled());
    buildCache(
        OPERATIONAL_TEMPLATE_CACHE,
        String.class,
        OPERATIONALTEMPLATE.class,
        cacheManager,
        cacheProperties.isEnabled());
    buildCache(
        VALIDATOR_CACHE, UUID.class, Validator.class, cacheManager, cacheProperties.isEnabled());
    buildCache(
        QUERY_CACHE,
        TemplateIdQueryTuple.class,
        JsonPathQueryResult.class,
        cacheManager,
        cacheProperties.isEnabled());
    buildCache(
        FIELDS_CACHE,
        TemplateIdAqlTuple.class,
        ItemInfo.class,
        cacheManager,
        cacheProperties.isEnabled());
    buildCache(
        MULTI_VALUE_CACHE, String.class, List.class, cacheManager, cacheProperties.isEnabled());
    return cacheManager;
  }

  public static <K, V> void buildCache(
      String cacheName,
      Class<K> keyClass,
      Class<V> valueClass,
