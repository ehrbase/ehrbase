...
public class KnowledgeCacheService implements I_KnowledgeCache, IntrospectService {
  ...
  public KnowledgeCacheService(TemplateStorage templateStorage,
    ...
    atOptCache = cacheManager.getCache(CacheOptions.OPERATIONAL_TEMPLATE_CACHE, String.class, OPERATIONALTEMPLATE.class);
    webTemplateCache = cacheManager.getCache(CacheOptions.INTROSPECT_CACHE, UUID.class, WebTemplate.class);
    jsonPathQueryResultCache = cacheManager.getCache(CacheOptions.QUERY_CACHE, TemplateIdQueryTuple.class, JsonPathQueryResult.class);
    fieldCache = cacheManager.getCache(CacheOptions.FIELDS_CACHE, TemplateIdAqlTuple.class, ItemInfo.class);
    multivaluedCache = cacheManager.getCache(CacheOptions.MULTI_VALUE_CACHE, String.class, List.class);
  }
  ...
  public void init() {
    ...
    for (TemplateMetaData metaData : listAllOperationalTemplates()) {
      ...
      try {
        value =
            Optional.ofNullable(operationaltemplate)
    ...
    while (iterator.hasNext()) {
      try {
        Optional.ofNullable(iterator.next())
            ...
            .map(WebTemplate::getTemplateId)
            .ifPresent(templateIdInCache::add);
      } catch (RuntimeException e) {
        // NOP
      }
    }
  ...
  public WebTemplate getQueryOptMetaData(UUID uuid) {
    ...
    if (webTemplateCache.containsKey(uuid)) retval = getFromCache(uuid, webTemplateCache);
    else {
  ...
  private WebTemplate buildAndCacheQueryOptMetaData(OPERATIONALTEMPLATE operationaltemplate) {
    log.info(
        "Updating WebTemplate cache for template: {}",
  ...
  private OPERATIONALTEMPLATE getOperationaltemplateFromFileStorage(String filename) {
    OPERATIONALTEMPLATE operationaltemplate =
        templateStorage.readOperationaltemplate(filename).orElse(null);
    if (operationaltemplate != null) {
      atOptCache.put(
          filename,
          operationaltemplate); // manual putting into cache (actual opt cache and then id cache)
      idxCacheUuidToTemplateId.put(
          UUID.fromString(operationaltemplate.getUid().getValue()), filename);
    }
