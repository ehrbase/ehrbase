...
public class RecordedDvDateTime {
  ...
  public Timestamp toTimestamp() {
    ...
    long millis =
        accessor.getLong(ChronoField.INSTANT_SECONDS) * 1000
  ...
  public String zoneId() {
    ...
    else return ZoneId.systemDefault().getId();
  }
   ...
   */
  // TODO unit test - until this is done please note this method was refactored from joda to java
  // time classes
  public DvDateTime decodeDvDateTime(Timestamp timestamp, String timezone) {
    ...
    else codedLocalDateTime = Optional.of(timestamp.toLocalDateTime());

    Optional<String> convertedDateTime =
        codedLocalDateTime.map(
            i -> i.format(java.time.format.DateTimeFormatter.ofPattern(DATE_FORMAT)));
    ...
    return new DvDateTime(
        convertedDateTime.orElseThrow(
            () -> new InternalServerException("Decoding DvDateTime failed")));
  }
}
