...
public class FolderServiceImp extends BaseService implements FolderService {
  ...
  @Autowired
  FolderServiceImp(
      KnowledgeCacheService knowledgeCacheService, DSLContext context, ServerConfig serverConfig) {
  ...
  public ObjectVersionId create(UUID ehrId, Folder content, UUID contribution) {
    ...
    I_FolderAccess folderAccess =
        FolderAccess.buildNewFolderAccessHierarchy(
            getDataAccess(), content, currentTimeStamp, ehrId, contributionAccess);
  ...
  public Optional<FolderDto> get(ObjectVersionId folderId, String path) {
    ...
    if (version == null) {
      ...
      Timestamp versionTimestamp =
          FolderAccess.getTimestampForVersion(getDataAccess(), folderId, version);
      folderAccess =
          I_FolderAccess.getInstanceForExistingFolder(getDataAccess(), folderId, versionTimestamp);
    }
  ...
  /** {@inheritDoc} */
  @Override
  public Optional<FolderDto> getByTimeStamp(
      ObjectVersionId folderId, Timestamp timestamp, String path) {
    ...
    if (timestamp.after(latest.getFolderSysTransaction())
        || timestamp.equals(latest.getFolderSysTransaction())) {
      if (version == null) {
        version = getLastVersionNumber(folderId);
      }
  ...
  public Optional<FolderDto> getLatest(ObjectVersionId folderId, String path) {
    ...
    I_FolderAccess folderAccess =
        I_FolderAccess.getInstanceForExistingFolder(
            getDataAccess(), folderId, Timestamp.from(Instant.now()));
    Integer version = FolderUtils.extractVersionNumberFromObjectVersionId(folderId);
  ...
  /** {@inheritDoc} */
  @Override
  public Optional<FolderDto> update(ObjectVersionId folderId, Folder update, UUID ehrId) {
  ...
  public LocalDateTime delete(ObjectVersionId folderId) {
    ...
    if (folderAccess.delete() > 0) {
      ...
      // Not found and bad argument exceptions are handled before thus this case can only occur on
      // unknown errors
      // On the server side
      ...
      case XML:
        folderString =
            new StructuredString(
                new CanonicalXML().marshal(folder, false), StructuredStringFormat.XML);
        break;
      case JSON:
        folderString =
            new StructuredString(new CanonicalJson().marshal(folder), StructuredStringFormat.JSON);
        break;
      default:
   ...
   * Object for internal usage with all sub folders and items which belong to the folder structure.
   *
  ...
  private Folder createFolderObject(I_FolderAccess folderAccess) {
    ...
    Folder result =
        new Folder(
            new HierObjectId(
                String.format(
                    "%s::%s",
                    folderAccess.getFolderId().toString(), this.getServerConfig().getNodename())),
    ...
    if (!folderAccess.getSubfoldersList().isEmpty()) {
      ...
      result.setFolders(
          folderAccess.getSubfoldersList().values().stream()
  ...
  /**
   * If a path was sent by the client the folderAccess retrieved from database will be iterated
   * recursive to find a given sub folder. If the path is empty or contains only one forward slash
   * the root folder will be returned. Trailing slashes at the end of a path will be ignored. If the
   * path cannot be found an ObjectNotFound exception will be thrown which can be handled by the
   * controller layer.
   *
  ...
  public void adminDeleteFolder(UUID folderId) {
    I_FolderAccess folderAccess =
        I_FolderAccess.retrieveInstanceForExistingFolder(getDataAccess(), folderId);
