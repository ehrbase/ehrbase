...
public class QueryServiceImp extends BaseService implements QueryService {
          ...
          if (record.getValue(field) instanceof JsonElement) {
            fieldMap.putResult(
                ...
                new StructuredString(
                    (record.getValue(field)).toString(), StructuredStringFormat.JSON));
  ...
  private QueryResultDto queryAql(
      String queryString, Map<String, Object> parameters, boolean explain) {
    try {
      AqlQueryHandler queryHandler = new AqlQueryHandler(getDataAccess(), tsAdapter);
    ...
    try {
      if (fullyQualifiedName == null || fullyQualifiedName.isEmpty()) {
        for (I_StoredQueryAccess storedQueryAccess :
            StoredQueryAccess.retrieveQualifiedList(getDataAccess())) {
  ...
  public QueryDefinitionResultDto retrieveStoredQuery(String qualifiedName, String version) {
    String queryQualifiedName =
        qualifiedName + ((version != null && !version.isEmpty()) ? "/" + version : "");
  ...
  @Override
  public QueryDefinitionResultDto updateStoredQuery(
      String qualifiedName, String version, String queryString) {

    try {
      I_StoredQueryAccess storedQueryAccess =
          StoredQueryAccess.retrieveQualified(
              getDataAccess(),
              qualifiedName + ((version != null && !version.isEmpty()) ? "/" + version : ""));
  ...
  private QueryDefinitionResultDto mapToQueryDefinitionDto(I_StoredQueryAccess storedQueryAccess) {
    ...
    dto.setQualifiedName(
        storedQueryAccess.getReverseDomainName() + "::" + storedQueryAccess.getSemanticId());
