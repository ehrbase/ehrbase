...
/**
 * since parsing of array of UDTs seems to fail with jOOQ 3.12, we are encoding term_mappings as an
 * array for TEXT with the following format:
 * match|purpose_value|purpose_terminology_id|purpose_code_string|target_terminology_id|target_code_string.
 * For example: =|Erfasst|local|irgendein Purpose|SNOMED-CT|345356676789 Methods encodeAsString and
 * decode deal with the formatting
 */
public class PersistentTermMapping {
  ...
  public String encodeAsString() {
    ...
    stringBuilder
        .append(rmTermMapping.getPurpose().getDefiningCode().getTerminologyId().getValue())
        .append("|");
  ...
  public TermMapping decode(String termMappingString) {
    ...
    return new TermMapping(
        ...
        new DvCodedText(
            attributes[1], new CodePhrase(new TerminologyId(attributes[2]), attributes[3])));
  }
