...
public class ContributionServiceHelper {
   ...
   * @return Map split at first level of input, so access to the version list and audit is directly
   *     possible
   */
   ...
   */
  public static List<Version> extractVersionObjects(
      ArrayList listVersions, CompositionFormat format) {
   ...
   * <string, object> without JSON specifics. only the problematic round trip conversation depends
   * of a format, but that could be fix. unmarshaller that creates an RMObject from a Map's content
   *
   * @param content Map instance containing data for a RMObject (i.e. pseudo marshalled)
   * @param format Format of the origin payload // TODO technically given content doesn't contain
   *     any (real) marshalled, for instance, json parts anymore
   * @return RM Object representation fitting the given content
   * @throws IllegalArgumentException when processing of given input fails
   */
  public static RMObject unmarshalMapContentToRmObject(
      LinkedHashMap content, CompositionFormat format) {
    switch (format) {
      case JSON:
        String json = null;
  ...
  /**
   * Convenience helper that combines some methods from above and prepares direct access to the list
   * of contained versions
   *
