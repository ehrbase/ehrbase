 ...
 * @since 1.0.0
 */
public class FolderHistoryAccess extends DataAccess implements I_FolderAccess,
  ...
  public FolderHistoryAccess(I_DomainAccess domainAccess, UUID ehrId,
  ...
  private static FolderHistoryAccess buildFolderAccessFromGenericRecord(final Record record_,
    ...
    folderAccess.getItems().addAll(
        FolderHistoryAccess.retrieveItemsByFolderAndContributionId(record.value1(), record.value3(),
            domainAccess));
    return folderAccess;
  }
  ...
  private static FolderHistoryAccess buildFolderAccessFromFolderRecord(final FolderRecord record_,
    ...
    folderAccess.getItems().addAll(
        FolderHistoryAccess.retrieveItemsByFolderAndContributionId(record.getId(),
            record.getInContribution(), domainAccess));
    return folderAccess;
  }
   ...
   */
  private static FolderHistoryAccess buildFolderAccessFromFolderId(final UUID id,
      final UUID contributionId, final I_DomainAccess domainAccess,
    ...
    FolderRecord folderSelectedRecord = getFolderRecordByUidAndContribution(id, contributionId,
        domainAccess);
   ...
   * @return
   */
  private static FolderRecord getFolderRecordByUidAndContribution(final UUID folderId,
      final UUID contributionId, final I_DomainAccess domainAccess) {
    Table<?> united_hierarchies_table1 = table(select().from(FOLDER)
        .where(FOLDER.ID.eq(folderId).and(FOLDER.IN_CONTRIBUTION.eq(contributionId))).union(
            select().from(FOLDER_HISTORY).where(FOLDER_HISTORY.ID.eq(folderId)
                .and(FOLDER_HISTORY.IN_CONTRIBUTION.eq(contributionId))))).asTable(
        "folder_union_fol_hist");

    FolderRecord folderSelectedRecord = (FolderRecord) domainAccess.getContext()
        .selectFrom(united_hierarchies_table1).fetchOne();
   ...
   * @return
   */
  private static List<ObjectRef<?>> retrieveItemsByFolderAndContributionId(UUID folderId,
      UUID in_contribution, I_DomainAccess domainAccess) {

    Table<?> table_items_and_objref = table(
        select(FOLDER_ITEMS.FOLDER_ID, FOLDER_ITEMS.OBJECT_REF_ID.as("item_object_ref_id"),
            FOLDER_ITEMS.IN_CONTRIBUTION.as("item_in_contribution"), FOLDER_ITEMS.SYS_TRANSACTION,
            FOLDER_ITEMS.SYS_PERIOD, OBJECT_REF.ID_NAMESPACE, OBJECT_REF.TYPE,
            OBJECT_REF.ID.as("obj_ref_id"), OBJECT_REF.IN_CONTRIBUTION.as("obj_ref_in_cont"),
            OBJECT_REF.SYS_TRANSACTION.as("objRefSysTran"),
            OBJECT_REF.SYS_PERIOD.as("oref_sysperiod")).from(FOLDER_ITEMS).leftJoin(OBJECT_REF)
            .on(FOLDER_ITEMS.FOLDER_ID.eq(folderId).and(
                FOLDER_ITEMS.IN_CONTRIBUTION.eq(in_contribution).and(
                    OBJECT_REF.ID.eq(FOLDER_ITEMS.OBJECT_REF_ID)
                        .and(OBJECT_REF.IN_CONTRIBUTION.eq(FOLDER_ITEMS.IN_CONTRIBUTION))))).where(
                FOLDER_ITEMS.FOLDER_ID.eq(folderId)
                    .and(FOLDER_ITEMS.IN_CONTRIBUTION.eq(in_contribution))));

    Table<?> table_items_and_objref_hist = table(select(FOLDER_ITEMS_HISTORY.FOLDER_ID,
        FOLDER_ITEMS_HISTORY.OBJECT_REF_ID.as("item_object_ref_id"),
        FOLDER_ITEMS_HISTORY.IN_CONTRIBUTION.as("item_in_contribution"),
        FOLDER_ITEMS_HISTORY.SYS_TRANSACTION, FOLDER_ITEMS_HISTORY.SYS_PERIOD,
        OBJECT_REF_HISTORY.ID_NAMESPACE, OBJECT_REF_HISTORY.TYPE,
        ...
        OBJECT_REF_HISTORY.SYS_PERIOD.as("oref_sysperiod")).from(FOLDER_ITEMS_HISTORY)
        .leftJoin(OBJECT_REF_HISTORY).on(FOLDER_ITEMS_HISTORY.FOLDER_ID.eq(folderId).and(
            FOLDER_ITEMS_HISTORY.IN_CONTRIBUTION.eq(in_contribution).and(
                OBJECT_REF_HISTORY.ID.eq(FOLDER_ITEMS_HISTORY.OBJECT_REF_ID).and(
                    OBJECT_REF_HISTORY.IN_CONTRIBUTION.eq(FOLDER_ITEMS_HISTORY.IN_CONTRIBUTION)))))
        .where(FOLDER_ITEMS_HISTORY.FOLDER_ID.eq(folderId)
            .and(FOLDER_ITEMS_HISTORY.IN_CONTRIBUTION.eq(in_contribution))));

    Table<?> table_all_items_and_objref = table(
        select().from(table_items_and_objref).union(select().from(table_items_and_objref_hist)));

    Result<Record> retrievedRecords = domainAccess.getContext().select()
        .from(table_all_items_and_objref).fetch();
   ...
   */
  private static ObjectRef parseObjectRefRecordIntoObjectRef(ObjectRefRecord objectRefRecord,
      I_DomainAccess domainAccess) {
    ObjectRef result = new ObjectRef();
    ObjectRefId oref = new FolderHistoryAccess(domainAccess).new ObjectRefId(
        objectRefRecord.getId().toString());
    result.setId(oref);
  ...
  public static Integer getLastVersionNumber(I_DomainAccess domainAccess, UUID folderId) {
    ...
    int versionCount = domainAccess.getContext()
  ...
  public void setFolderSysPeriod(
      AbstractMap.SimpleEntry<OffsetDateTime, OffsetDateTime> folderSysPeriod) {
   ...
   */
  public static int getVersionFromTimeStamp(I_DomainAccess domainAccess, UUID vFolderUid,
      Timestamp timeCommitted) throws InternalServerException {
    ...
    Record result;
    try {
      result = domainAccess.getContext().select(max(FOLDER.SYS_TRANSACTION).as("mostRecentInTable"))
          .from(FOLDER).where(FOLDER.ID.eq(vFolderUid)).fetchOne();
    ...
    Record result2;
    try {
      result2 = domainAccess.getContext().select(count().as("countVersionInTable")).from(FOLDER)
          .where(FOLDER_HISTORY.SYS_TRANSACTION.lessOrEqual(timeCommitted)
              .and(FOLDER_HISTORY.ID.eq(vFolderUid))).fetchOne();
    } catch (RuntimeException e) { // generalize SQL exceptions
      throw new InternalServerException("Problem with SQL statement or input", e);
    }
    int versionComHist = (int) result2.get("countVersionInTable");
    if (timeCommitted.compareTo(latestCompoTime)
        >= 0) {//if the timestamp is after or equal to the sys_transaction of the latest folder available, add one since its version has not been counted for being the one stored in the ehr.folder table
      versionComHist++;
    }
    if (versionComHist == 0) {
      throw new ObjectNotFoundException("FOLDER VERSION",
          "There are no versions available prior to date " + timeCommitted
              + " for the the FOLDER with id: " + vFolderUid);
    }
   ...
   */
  private static Result<Record> buildUnionOfFolderHierarchiesTable(UUID folderUid, Timestamp ts,
      I_DomainAccess domainAccess) {
    ...
    Table<?> united_hierarchies_table1 = table(
        select().from(FOLDER_HIERARCHY).where(FOLDER_HIERARCHY.SYS_TRANSACTION.le(timestamp)).union(
            select().from(FOLDER_HIERARCHY_HISTORY)
                .where(FOLDER_HIERARCHY_HISTORY.SYS_TRANSACTION.le(timestamp)))).asTable(
        "united_hierarchies_table1");

    Table<?> united_hierarchies_table2 = table(
        select().from(FOLDER_HIERARCHY).where(FOLDER_HIERARCHY.SYS_TRANSACTION.le(timestamp)).union(
            select().from(FOLDER_HIERARCHY_HISTORY)
                .where(FOLDER_HIERARCHY_HISTORY.SYS_TRANSACTION.le(timestamp)))).asTable(
        "united_hierarchies_table2");

    Table<?> united_hierarchies_tableFileted = select().from(united_hierarchies_table1).where(
        united_hierarchies_table1.field("sys_transaction",
            FOLDER_HIERARCHY.SYS_TRANSACTION.getType()).eq(select(
            max(united_hierarchies_table2.field("sys_transaction",
                FOLDER_HIERARCHY.SYS_TRANSACTION.getType()))).from(united_hierarchies_table2).where(
            (united_hierarchies_table1.field("parent_folder",
                FOLDER_HIERARCHY.PARENT_FOLDER.getType())).eq(
                united_hierarchies_table2.field("parent_folder",
                    FOLDER_HIERARCHY.PARENT_FOLDER.getType())

            ).and(united_hierarchies_table1.field("child_folder",
                    FOLDER_HIERARCHY.CHILD_FOLDER.getType())
                .eq(united_hierarchies_table2.field("child_folder",
                    FOLDER_HIERARCHY.CHILD_FOLDER.getType()))
    ...
    Table<?> fhf_timestamp_version2 = select(
        fhf_timestamp1.field("parent_folder", FOLDER.ID.getType()).as("parent_folder_id"),
        max(fhf_timestamp2.field("sys_transaction", FOLDER.SYS_TRANSACTION.getType())).as(
            "latest_sys_transaction")).from(fhf_timestamp1)
        .groupBy(fhf_timestamp1.field("parent_folder", FOLDER.ID.getType())).asTable();

    /*make the unified table with only the rows that correspond to the latest transactions*/
    Table<?> filteredHierarchicalTable = select().from(united_hierarchies_tableFileted,
        fhf_timestamp_version2).where(
        united_hierarchies_tableFileted.field("parent_folder", FOLDER.ID.getType())
            .eq(fhf_timestamp_version2.field("parent_folder_id", FOLDER.ID.getType())).and(
                united_hierarchies_tableFileted.field("sys_transaction",
                        FOLDER.SYS_TRANSACTION.getType())
                    .eq(fhf_timestamp_version2.field("latest_sys_transaction",
                        FOLDER.SYS_TRANSACTION.getType())))).asTable();
    ...
    Table<?> allFolderRowsFolderTable = domainAccess.getContext()
        .select(FOLDER.ID, FOLDER.IN_CONTRIBUTION, FOLDER.NAME, FOLDER.ARCHETYPE_NODE_ID,
            FOLDER.ACTIVE, FOLDER.DETAILS, FOLDER.SYS_TRANSACTION, FOLDER.SYS_PERIOD)
        .from(FOLDER, filteredHierarchicalTable).where(
            FOLDER.ID.eq(filteredHierarchicalTable.field("parent_folder", UUID.class)).and(
                FOLDER.IN_CONTRIBUTION.eq(
                    filteredHierarchicalTable.field("in_contribution", UUID.class)))).asTable();

    Table<?> allFolderRowsFolderHistoryTable = domainAccess.getContext()
        .select(FOLDER_HISTORY.ID, FOLDER_HISTORY.IN_CONTRIBUTION, FOLDER_HISTORY.NAME,
            FOLDER_HISTORY.ARCHETYPE_NODE_ID, FOLDER_HISTORY.ACTIVE, FOLDER_HISTORY.DETAILS,
            FOLDER_HISTORY.SYS_TRANSACTION, FOLDER_HISTORY.SYS_PERIOD)
        .from(FOLDER_HISTORY, filteredHierarchicalTable).where(
            FOLDER_HISTORY.ID.eq(filteredHierarchicalTable.field("parent_folder", UUID.class)).and(
                FOLDER_HISTORY.IN_CONTRIBUTION.eq(
                    filteredHierarchicalTable.field("in_contribution", UUID.class)))).asTable();

    Table<?> allFolderRowsUnifiedAndFilteredInitial = domainAccess.getContext()
        .select(allFolderRowsFolderTable.field("id", UUID.class),
            allFolderRowsFolderTable.field("in_contribution", UUID.class)
            ...
            allFolderRowsFolderTable.field("sys_transaction", FOLDER.SYS_TRANSACTION.getType())
                .as("sys_transaction_folder"),
            allFolderRowsFolderTable.field("sys_period", FOLDER.SYS_PERIOD.getType())
                .as("sys_period_folder")).from(allFolderRowsFolderTable).union(
            select(allFolderRowsFolderHistoryTable.field("id", UUID.class),
                allFolderRowsFolderHistoryTable.field("in_contribution", UUID.class)
                    .as("in_contribution_folder_info"),
                allFolderRowsFolderHistoryTable.field("name", FOLDER.NAME.getType()),
                allFolderRowsFolderHistoryTable.field("archetype_node_id",
                    FOLDER.ARCHETYPE_NODE_ID.getType()),
                ...
                allFolderRowsFolderHistoryTable.field("sys_transaction",
                    FOLDER.SYS_TRANSACTION.getType()).as("sys_transaction_folder"),
                allFolderRowsFolderHistoryTable.field("sys_period", FOLDER.SYS_PERIOD.getType())
                    .as("sys_period_folder")).from(allFolderRowsFolderHistoryTable)).asTable();
    ...
    Field<UUID> subfolderChildFolder = field("subfolders.{0}",
        FOLDER_HIERARCHY.CHILD_FOLDER.getDataType(),
        FOLDER_HIERARCHY.CHILD_FOLDER.getUnqualifiedName());
    Field<Timestamp> subfolderSysTran = field("\"subfolders\".\"sys_transaction\"",
    ...
    Table<?> initial_table2 = (select().from(filteredHierarchicalTable)
        ...
        .on(filteredHierarchicalTable.field("parent_folder", UUID.class)
            .eq(allFolderRowsUnifiedAndFilteredInitial.field("id", UUID.class))).where(
            filteredHierarchicalTable.field("parent_folder", UUID.class).eq(folderUid))).asTable();

    return domainAccess.getContext().withRecursive("subfolders")
        .as(select(initial_table2.fields()).from(initial_table2).union((select(
            ArrayUtils.addAll(filteredHierarchicalTable.fields(),
                allFolderRowsUnifiedAndFilteredIterative.fields())).from(filteredHierarchicalTable)
            .innerJoin("subfolders").on(filteredHierarchicalTable.field("parent_folder",
                FOLDER_HIERARCHY.PARENT_FOLDER.getType()).eq(subfolderChildFolder))).leftJoin(
                allFolderRowsUnifiedAndFilteredIterative)
            .on(allFolderRowsUnifiedAndFilteredIterative.field("id", FOLDER.ID.getType())
                .eq(subfolderChildFolder)))).select().from(table(name("subfolders"))).fetch();
  }

  public static I_FolderAccess getInstanceForExistingFolder(I_DomainAccess domainAccess,
      ObjectVersionId folderId, Timestamp timestamp) {
    return FolderHistoryAccess.retrieveInstanceForExistingFolder(domainAccess,
        FolderUtils.extractUuidFromObjectVersionId(folderId), timestamp);
  }
  ...
  public static I_FolderAccess retrieveInstanceForExistingFolder(I_DomainAccess domainAccess,
    ...
    Result<Record> folderSelectedRecordSub = buildUnionOfFolderHierarchiesTable(folderId, timestamp,
        domainAccess);
    ...
    for (Record record : folderSelectedRecordSub) {
      ...
      fHierarchyMap.get(record.getValue("parent_folder"))
          .put((UUID) record.getValue("child_folder"),
              buildFolderAccessFromFolderId((UUID) record.getValue("child_folder"),
                  (UUID) record.getValue("in_contribution"), domainAccess,
                  folderSelectedRecordSub));
    }

    /**3-populate result and return**/
    return FolderHistoryAccess.buildFolderAccessHierarchy(fHierarchyMap, folderId, null,
        folderSelectedRecordSub, domainAccess);
  }

  private static I_FolderAccess buildFolderAccessHierarchy(
      final Map<UUID, Map<UUID, I_FolderAccess>> fHierarchyMap, final UUID currentFolder,
      final I_FolderAccess parentFa, final Result<Record> folderSelectedRecordSub,
      final I_DomainAccess domainAccess) {
    ...
    I_FolderAccess folderAccess = buildFolderAccessFromFolderId(currentFolder, contributionUid,
        domainAccess, folderSelectedRecordSub);
    if (parentFa != null) {
    ...
    if (fHierarchyMap.get(currentFolder) != null) {//if not leave node call children
      ...
      for (UUID newChild : fHierarchyMap.get(currentFolder).keySet()) {
        buildFolderAccessHierarchy(fHierarchyMap, newChild, folderAccess, folderSelectedRecordSub,
            domainAccess);
      }
    }
    return folderAccess;
