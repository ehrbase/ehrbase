 ...
 */
public class FolderHistoryAccess extends DataAccess
  ...
  private static FolderHistoryAccess buildFolderAccessFromFolderId(
    ...
    FolderRecord folderSelectedRecord =
        getFolderRecordByUidAndContribution(id, contributionId, domainAccess);
  ...
  private static FolderRecord getFolderRecordByUidAndContribution(
    ...
    Table<?> united_hierarchies_table1 =
        table(
                select()
                    .from(FOLDER)
    ...
    FolderRecord folderSelectedRecord =
        (FolderRecord) domainAccess.getContext().selectFrom(united_hierarchies_table1).fetchOne();
                            ...
                            FOLDER_ITEMS_HISTORY
                                ...
                                .and(
                                    OBJECT_REF_HISTORY
                                        ...
                                        .and(
                                            OBJECT_REF_HISTORY.IN_CONTRIBUTION.eq(
    ...
    Table<?> table_all_items_and_objref =
        table(
            select()
                ...
                .union(select().from(table_items_and_objref_hist)));

    Result<Record> retrievedRecords =
        domainAccess.getContext().select().from(table_all_items_and_objref).fetch();
  ...
  private static ObjectRef parseObjectRefRecordIntoObjectRef(
    ...
    ObjectRefId oref =
        new FolderHistoryAccess(domainAccess).new ObjectRefId(objectRefRecord.getId().toString());
    result.setId(oref);
  ...
  public static Integer getLastVersionNumber(I_DomainAccess domainAccess, UUID folderId) {
    ...
    int versionCount =
        domainAccess.getContext().fetchCount(FOLDER_HISTORY, FOLDER_HISTORY.ID.eq(folderId));
  ...
  public static int getVersionFromTimeStamp(
      I_DomainAccess domainAccess, UUID vFolderUid, Timestamp timeCommitted)
    ...
    if (timeCommitted.compareTo(latestCompoTime)
        >= 0) { // if the timestamp is after or equal to the sys_transaction of the latest folder
                // available, add one since its version has not been counted for being the one
                // stored in the ehr.folder table
      versionComHist++;
    }
    if (versionComHist == 0) {
   ...
   *     a table.
   * @param ts The timestamp which used to determine which version to retrieve. The method will use
   *     the closest version available before or equal to the timestamp provided.
  ...
  private static Result<Record> buildUnionOfFolderHierarchiesTable(
    ...
    Table<?> united_hierarchies_table2 =
        table(
                select()
                    .from(FOLDER_HIERARCHY)
                    .where(FOLDER_HIERARCHY.SYS_TRANSACTION.le(timestamp))
    ...
    Table<?> united_hierarchies_tableFileted =
        select()
            .from(united_hierarchies_table1)
            .where(
                united_hierarchies_table1
                    ...
                    .eq(
                        select(
                                max(
                                    united_hierarchies_table2.field(
                                        "sys_transaction",
                                        FOLDER_HIERARCHY.SYS_TRANSACTION.getType())))
                            .from(united_hierarchies_table2)
                            .where(
                                (united_hierarchies_table1.field(
    ...
    Table<?> filteredHierarchicalTable =
        select()
            .from(united_hierarchies_tableFileted, fhf_timestamp_version2)
            .where(
                united_hierarchies_tableFileted
    ...
    Table<?> allFolderRowsFolderTable =
        domainAccess
            .getContext()
            .select(
                FOLDER.ID,
    ...
    Table<?> allFolderRowsUnifiedAndFilteredInitial =
        domainAccess
            ...
            .select(
                ...
                allFolderRowsFolderTable.field(
    ...
    Table<?> allFolderRowsUnifiedAndFilteredIterative =
        domainAccess
            .getContext()
            .select(
                allFolderRowsFolderTable.field("id", UUID.class),
    ...
    Field<Timestamp> subfolderSysTran =
        field(
    ...
    Table<?> initial_table2 =
        (select()
                ...
                .on(
                    filteredHierarchicalTable
                        .field("parent_folder", UUID.class)
                        .eq(allFolderRowsUnifiedAndFilteredInitial.field("id", UUID.class)))
                .where(filteredHierarchicalTable.field("parent_folder", UUID.class).eq(folderUid)))
            .asTable();
  ...
  public static I_FolderAccess retrieveInstanceForExistingFolder(
    ...
    Result<Record> folderSelectedRecordSub =
        buildUnionOfFolderHierarchiesTable(folderId, timestamp, domainAccess);
  ...
  private static I_FolderAccess buildFolderAccessHierarchy(
    ...
    I_FolderAccess folderAccess =
        buildFolderAccessFromFolderId(
            currentFolder, contributionUid, domainAccess, folderSelectedRecordSub);
    if (parentFa != null) {
  ...
  public void adminDeleteFolder() {
    // needed because the interface declares it, but the ACTUAL admin delete handling is done at
    // non-history level
  }
}
