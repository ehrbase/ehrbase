...
public class EhrAccess extends DataAccess implements I_EhrAccess {
  ...
  private I_ContributionAccess
      contributionAccess; // locally referenced contribution associated to ehr transactions
  ...
  public EhrAccess(
    ...
    if (ehrRecord.getSystemId()
        == null) { // storeComposition a default entry for the current system
  ...
  public static UUID retrieveInstanceBySubject(I_DomainAccess domainAccess, UUID subjectUuid) {
    ...
    try {
      ...
      throw new IllegalArgumentException(
          "Could not retrieve  EHR for party:" + subjectUuid + EXCEPTION + e);
    }
  ...
  /** @throws IllegalArgumentException if retrieving failed for given input */
  public static UUID retrieveInstanceBySubject(
    ...
    IdentifierRecord identifierRecord =
        context.fetchOne(
            IDENTIFIER, IDENTIFIER.ID_VALUE.eq(subjectId).and(IDENTIFIER.ISSUER.eq(issuerSpace)));

    if (identifierRecord == null)
      throw new IllegalArgumentException(
          "Could not invalidateContent an identified party for code:"
              + subjectId
              + " issued by:"
              + issuerSpace);
    ...
    try {
      ...
      throw new IllegalArgumentException(
          COULD_NOT_RETRIEVE_EHR_FOR_PARTY + subjectId + EXCEPTION + e);
    }
  ...
  public static I_EhrAccess retrieveInstanceByStatus(
    ...
    if (version < 1) throw new IllegalArgumentException("Version number must be > 0");

    EhrAccess ehrAccess =
        new EhrAccess(
            domainAccess, ehrId); // minimal access, needs attributes to be set before returning
    EhrRecord record;

    // necessary anyway, but if no version is provided assume latest version (otherwise this one
    // will be overwritten with wanted one)
    I_StatusAccess statusAccess = I_StatusAccess.retrieveInstance(domainAccess, status);
    ehrAccess.setStatusAccess(statusAccess);

    // first step of retrieving a particular version is to query for the amount of versions, which
    // depends on the latest one above
    Integer versions =
        domainAccess
                .getContext()
                .fetchCount(
                    STATUS_HISTORY,
                    STATUS_HISTORY.EHR_ID.eq(
                        ehrAccess.getStatusAccess().getStatusRecord().getEhrId()))
            + 1;
    // check if input version number fits into existing amount of versions, but is not the same
    // (same equals latest version)
    // when either there is only one version or the requested one is the latest, continue with
    // record already set
    if (versions > version
        && !version.equals(versions)) { // or get the particular requested version
      // TODO: why does sonarlint says that the expression above is always true? tested it, it can
      // be true and false!?
      // note: here version is > 1 and there has to be at least one history entry
      Result<StatusHistoryRecord> result =
          domainAccess
              .getContext()
      ...
      // FIXME EHR_STATUS: manually converting types. dirty, formally break jooq-style, right? the
      // record would considered to be updated when calling methods like .store()
      ehrAccess
          .getStatusAccess()
          .getStatusRecord()
          .setIsQueryable(statusHistoryRecord.getIsQueryable());
      ehrAccess
          .getStatusAccess()
          .getStatusRecord()
          .setIsModifiable(statusHistoryRecord.getIsModifiable());
      ehrAccess.getStatusAccess().getStatusRecord().setParty(statusHistoryRecord.getParty());
      ehrAccess
          .getStatusAccess()
          .getStatusRecord()
          .setOtherDetails(statusHistoryRecord.getOtherDetails());
      ehrAccess
          .getStatusAccess()
          .getStatusRecord()
          .setSysTransaction(statusHistoryRecord.getSysTransaction());
      ehrAccess
          .getStatusAccess()
          .getStatusRecord()
          .setSysPeriod(statusHistoryRecord.getSysPeriod());
    }
  ...
  public static I_EhrAccess retrieveInstance(I_DomainAccess domainAccess, UUID ehrId) {
    ...
    I_StatusAccess statusAccess =
        I_StatusAccess.retrieveInstanceByEhrId(domainAccess, ehrAccess.ehrRecord.getId());
    ehrAccess.setStatusAccess(statusAccess);
    ...
    ContributionRecord contributionRecord =
        context.fetchOne(
            ...
            CONTRIBUTION
                .EHR_ID
                .eq(ehrAccess.ehrRecord.getId())
                .and(CONTRIBUTION.CONTRIBUTION_TYPE.eq(ContributionDataType.ehr)));
    if (contributionRecord == null)
      throw new IllegalArgumentException(
          "DB inconsistency: could not find a related contribution for ehr="
              + ehrAccess.ehrRecord.getId());

    UUID contributionId = contributionRecord.getId();

    if (contributionId != null) {
      ehrAccess.setContributionAccess(
          I_ContributionAccess.retrieveInstance(domainAccess, contributionId));
    }
  ...
  public static Map<String, Object> fetchSubjectIdentifiers(
      I_DomainAccess domainAccess, UUID ehrId) {
    EhrAccess ehrAccess = (EhrAccess) retrieveInstance(domainAccess, ehrId);
    DSLContext context = domainAccess.getContext();

    if (ehrAccess == null) throw new IllegalArgumentException("No ehr found for id:" + ehrId);
  ...
  /**
   * FIXME: check this method. appears to be needed later on. problematic: it actually gets a list
   * of entries, not compositions. why only with three attributes? what about the unique key problem
   * below?
   *
   * @throws IllegalArgumentException when no EHR found for ID
   */
  public static Map<String, Map<String, String>> getCompositionList(
      I_DomainAccess domainAccess, UUID ehrId) {
    EhrAccess ehrAccess = (EhrAccess) retrieveInstance(domainAccess, ehrId);
    DSLContext context = domainAccess.getContext();
    ...
    context
        ...
        .forEach(
            record -> {
              ...
              compositionlist.put(
                  "details",
                  details); // FIXME: bug? gets overwritten if more than 1 put() with this static
                            // key
            });
  ...
  public Boolean update(
    ...
    if (contributionId != null) {
      I_ContributionAccess access =
          I_ContributionAccess.retrieveInstance(this.getDataAccess(), contributionId);
      if (access != null) {
        provisionContributionAccess(
            access, committerId, systemId, description, state, contributionChangeType);
        this.contributionAccess = access;
      } else throw new InternalServerException("Can't update status with invalid contribution ID.");
    } else {
      provisionContributionAccess(
  ...
  private void provisionContributionAccess(
    ...
    access.setAuditDetailsChangeType(
  ...
  public void setOtherDetails(ItemStructure otherDetails, String templateId) {
    ...
    this.otherDetailsTemplateId =
        Optional.ofNullable(otherDetails)
  ...
  public void setStatus(EhrStatus status) {
    ...
    if (status.getArchetypeNodeId() != null) setArchetypeNodeId(status.getArchetypeNodeId());

    if (status.getName() != null) setName(status.getName());
  ...
  public EhrStatus getStatus() {
    ...
    Object name =
        new RecordedDvCodedText().fromDB(getStatusAccess().getStatusRecord(), STATUS.NAME);
  ...
  public void adminDeleteEhr() {
    ...
    Result<AdminGetLinkedCompositionsRecord> linkedCompositions =
        Routines.adminGetLinkedCompositions(getContext().configuration(), this.getId());
    Result<AdminGetLinkedContributionsRecord> linkedContributions =
        Routines.adminGetLinkedContributions(getContext().configuration(), this.getId());
    ...
    // delete EHR itself
    Result<AdminDeleteEhrRecord> adminDeleteEhr =
        Routines.adminDeleteEhr(getContext().configuration(), this.getId());
    if (adminDeleteEhr.isEmpty()) {
      throw new InternalServerException("Admin deletion of EHR failed! Unexpected result.");
    }

    // adminDeleteEhr returns all linked contributions, audits and statuses - go through and delete
    // them
    adminDeleteEhr.forEach(
        response -> {
          ...
          linkedContributions.forEach(
              contrib ->
                  adminApi.deleteContribution(contrib.getContribution(), contrib.getAudit(), true));

          // final cleanup of auxiliary objects
          int res =
              getContext().selectQuery(new AdminDeleteEhrHistory().call(this.getId())).execute();
          if (res != 1) throw new InternalServerException("Admin deletion of EHR failed!");
        });
  }
}
