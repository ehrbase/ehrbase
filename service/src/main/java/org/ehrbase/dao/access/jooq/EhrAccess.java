...
public class EhrAccess extends DataAccess implements I_EhrAccess {
   ...
   */
  public EhrAccess(DSLContext context, ServerConfig serverConfig, UUID partyId, UUID systemId,
      UUID directoryId, UUID accessId, UUID ehrId) {
   ...
   * @throws IllegalArgumentException if retrieving failed for given input
   */
  public static UUID retrieveInstanceBySubject(I_DomainAccess domainAccess, String subjectId,
      String issuerSpace) {
    Record record;
    DSLContext context = domainAccess.getContext();

    //get the corresponding party Id from the codification space provided by an issuer
    IdentifierRecord identifierRecord = context.fetchOne(IDENTIFIER,
        IDENTIFIER.ID_VALUE.eq(subjectId).and(IDENTIFIER.ISSUER.eq(issuerSpace)));

    if (identifierRecord == null) {
      throw new IllegalArgumentException(
          "Could not invalidateContent an identified party for code:" + subjectId + " issued by:"
              + issuerSpace);
    }
  ...
  public static UUID retrieveInstanceBySubjectExternalRef(I_DomainAccess domainAccess,
      String subjectId, String issuerSpace) {
    Record record;
    DSLContext context = domainAccess.getContext();
  ...
  public static I_EhrAccess retrieveInstanceByStatus(I_DomainAccess domainAccess, UUID ehrId,
      UUID status, Integer version) {
    if (version < 1) {
      throw new IllegalArgumentException("Version number must be > 0");
    }

    EhrAccess ehrAccess = new EhrAccess(domainAccess,
        ehrId);  // minimal access, needs attributes to be set before returning
    EhrRecord record;
    ...
    Integer versions = domainAccess.getContext().fetchCount(STATUS_HISTORY,
        STATUS_HISTORY.EHR_ID.eq(ehrAccess.getStatusAccess().getStatusRecord().getEhrId())) + 1;
    ...
    if (versions > version && !version.equals(
        versions)) { // or get the particular requested version
      ...
      ehrAccess.getStatusAccess().getStatusRecord()
      ...
      ehrAccess.getStatusAccess().getStatusRecord()
          .setAttestationRef(statusHistoryRecord.getAttestationRef());
      ehrAccess.getStatusAccess().getStatusRecord()
          .setInContribution(statusHistoryRecord.getInContribution());
      ehrAccess.getStatusAccess().getStatusRecord()
          .setArchetypeNodeId(statusHistoryRecord.getArchetypeNodeId());
  ...
  public static I_EhrAccess retrieveInstance(I_DomainAccess domainAccess, UUID ehrId) {
    ...
    I_StatusAccess statusAccess = I_StatusAccess.retrieveInstanceByEhrId(domainAccess,
    ...
    ehrAccess.setContributionAccess(I_ContributionAccess.retrieveInstance(domainAccess,
        ehrAccess.getStatusAccess().getContributionId()));
  ...
  public UUID commit(Timestamp transactionTime) {
    ...
    ehrRecord.setDateCreatedTzid(OffsetDateTime.from(
            transactionTime.toLocalDateTime().atOffset(ZoneOffset.from(OffsetDateTime.now())))
        .getOffset().getId());    // get zoneId independent of "transactionTime"
    ehrRecord.store();
  ...
  @Override
  public Boolean update(UUID committerId, UUID systemId, UUID contributionId,
    ...
    if (contributionId != null) {
      I_ContributionAccess access = I_ContributionAccess.retrieveInstance(this.getDataAccess(),
          contributionId);
      if (access != null) {
      ...
      provisionContributionAccess(contributionAccess, committerId, systemId, description, state,
   ...
   */
  private void provisionContributionAccess(I_ContributionAccess access, UUID committerId,
      UUID systemId, String description, ContributionDef.ContributionState state,
  ...
  public UUID reload() {
    ...
    ehrRecord = record;
    //retrieve the corresponding status
    I_StatusAccess retStatusAccess = I_StatusAccess.retrieveInstanceByEhrId(this.getDataAccess(),
        ehrRecord.getId());
...
//        this.otherDetailsTemplateId = Optional.ofNullable(otherDetails).map(Locatable::getArchetypeDetails).map(Archetyped::getTemplateId).map(ObjectId::getValue).orElse(null);
  ...
  public EhrStatus getStatus() {
    ...
    Object name = new RecordedDvCodedText().fromDB(getStatusAccess().getStatusRecord(),
        STATUS.NAME);
    status.setName(name instanceof DvText ? (DvText) name : (DvCodedText) name);

    UUID statusId = getStatusAccess().getStatusRecord().getId();
    status.setUid(
        new HierObjectId(statusId.toString() + "::" + getServerConfig().getNodename() + "::" +
  ...
  public void adminDeleteEhr() {
    Result<AdminDeleteEhrFullRecord> result = Routines.adminDeleteEhrFull(
        getContext().configuration(), this.getId());
