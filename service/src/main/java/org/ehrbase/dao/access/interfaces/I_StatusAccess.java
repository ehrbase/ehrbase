 ...
 * the status entry holds data pertaining to an Ehr owner, generally a patient Created by Christian
...
public interface I_StatusAccess extends I_SimpleCRUD {
   ...
   * NB. for security reason, most deployment will not provide an explicit subject name, this method
   * is provided for small deployment or test purpose.
   *
   * @param domainAccess SQL access
   * @param partyName a subject name
   * @return UUID or null
   */
  static I_StatusAccess retrieveInstanceByNamedSubject(
   ...
   */
  static Map<I_StatusAccess, Integer> retrieveInstanceByContribution(
   ...
   * @return ID of DB entry if successful
   */
  UUID commitWithCustomContribution(
