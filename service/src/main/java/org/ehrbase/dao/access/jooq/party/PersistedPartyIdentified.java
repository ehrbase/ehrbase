...
class PersistedPartyIdentified extends PersistedParty {
  ...
  public PartyProxy render(PartyIdentifiedRecord partyIdentifiedRecord) {
    ...
    List<DvIdentifier> identifierList =
        new PartyIdentifiers(domainAccess).retrieve(partyIdentifiedRecord);
  ...
  /**
   * Retrieve a party identified by: External Ref if none, by matching name and matching identifiers
   * if any
   *
   * @param partyProxy
   * @return
      ...
      if (partyProxy.getExternalRef() == null) { // check for the same name
        ...
        if (record != null) {
          ...
          if (!new PartyIdentifiers(domainAccess)
              .compare(
                  (PartyIdentifiedRecord) record, ((PartyIdentified) partyProxy).getIdentifiers()))
            uuid = null;
        }
      }
    }
