 ...
 *
 * <p>Created by Christian Chevalley on 4/9/2015.
 */
public class EntryAccess extends DataAccess implements I_EntryAccess {
  ...
  public static List<I_EntryAccess> retrieveInstanceInComposition(
    ...
    Result<EntryRecord> entryRecords =
        domainAccess
            .getContext()
            .selectFrom(ENTRY)
    ...
    values.put(
        SystemValue.TERRITORY, new CodePhrase(new TerminologyId("ISO_3166-1"), territory2letters));

    if (compositionAccess.getFeederAudit() != null)
      values.put(
          SystemValue.FEEDER_AUDIT,
    ...
    try {
      ...
      for (EntryRecord record : entryRecords) {
        ...
        Integer version =
            I_CompositionAccess.getLastVersionNumber(domainAccess, compositionAccess.getId());
        values.put(
            SystemValue.UID,
            new ObjectVersionId(
                compositionAccess.getId().toString()
                    + "::"
                    + domainAccess.getServerConfig().getNodename()
                    + "::"
                    + version));
  ...
  public static List<I_EntryAccess> retrieveInstanceInCompositionVersion(
    ...
    Result<EntryHistoryRecord> entryHistoryRecords =
        domainAccess
            .getContext()
    ...
    EventContext context =
        I_ContextAccess.retrieveHistoricalEventContext(
            domainAccess,
            compositionHistoryAccess.getId(),
  ...
  private static void setCompositionAttributes(
      Composition composition, Map<SystemValue, Object> values) {
   ...
   *   <li>category
   *   <li>item type
   *   <li>archetype node Id
   *   <li>entry content (json)
   * </ul>
   *
  ...
  private void setCompositionFields(EntryRecord record, Composition composition) {
    ...
    Integer categoryId =
        Integer.parseInt(composition.getCategory().getDefiningCode().getCodeString());
    ...
    if (composition.getContent() != null && !composition.getContent().isEmpty()) {
      ...
      if (node instanceof Section) record.setItemType(EntryType.valueOf("section"));
      else if (node instanceof Evaluation
          || node instanceof Observation
          || node instanceof Instruction
          || node instanceof Action) record.setItemType(EntryType.valueOf("care_entry"));
      else if (node instanceof AdminEntry) record.setItemType(EntryType.valueOf("admin"));
    } else record.setItemType(EntryType.valueOf("admin"));
   ...
   * given {@link Composition} as a result. TODO: memory only until committed, correct?
   *
  ...
  private void setFields(
      String templateId, Integer sequence, UUID compositionId, Composition composition) {
  ...
  public Integer delete() {
    ...
    if (entryRecord != null) return entryRecord.delete();
