...
public class AuditDetailsAccess extends DataAccess implements I_AuditDetailsAccess {
  ...
  public AuditDetailsAccess(I_DomainAccess dataAccess, UUID systemId, UUID committer,
  ...
  public I_AuditDetailsAccess retrieveInstance(I_DomainAccess dataAccess, UUID auditId) {
    ...
    try {
      auditDetailsAccess.auditDetailsRecord = dataAccess.getContext().fetchOne(AUDIT_DETAILS,
          AUDIT_DETAILS.ID.eq(auditId));
  ...
  @Override
  public Boolean update(UUID systemId, UUID committer, I_ConceptAccess.ContributionChangeType changeType,
      String description) {
    if (systemId != null)
