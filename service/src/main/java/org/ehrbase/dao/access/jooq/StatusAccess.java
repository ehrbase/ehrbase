...
public class StatusAccess extends DataAccess implements I_StatusAccess {
  ...
  private I_ContributionAccess
      contributionAccess; // locally referenced contribution associated to this status
  ...
  public static I_StatusAccess retrieveInstanceByNamedSubject(
      I_DomainAccess domainAccess, String partyName) {
  ...
  public static I_StatusAccess retrieveInstanceByParty(
      I_DomainAccess domainAccess, UUID partyIdentified) {
  ...
  public static I_StatusAccess retrieveInstanceByEhrId(I_DomainAccess domainAccess, UUID ehrId) {
    ...
    if (domainAccess.getContext().fetchExists(STATUS, STATUS.EHR_ID.eq(ehrId)))
      ...
      if (!domainAccess.getContext().fetchExists(STATUS_HISTORY, STATUS_HISTORY.EHR_ID.eq(ehrId)))
        ...
        Result<StatusHistoryRecord> recordsRes =
            domainAccess
                .getContext()
  ...
  public static Map<I_StatusAccess, Integer> retrieveInstanceByContribution(
    ...
    // get whole "version map" of each matching status and do fine-grain check for matching
    // contribution
    // precondition: each UUID in `statuses` set is unique, so for each the "version map" is only
    // created once below
  ...
  public static Map<I_StatusAccess, Integer> getVersionMapOfStatus(
      I_DomainAccess domainAccess, UUID statusId) {
    ...
    Result<StatusHistoryRecord> historyRecords =
        domainAccess
            .getContext()
            .selectFrom(STATUS_HISTORY)
    ...
    for (StatusHistoryRecord historyRecord : historyRecords) {
      I_StatusAccess historyAccess =
          createStatusAccessForRetrieval(domainAccess, null, historyRecord);
  ...
  private static I_StatusAccess createStatusAccessForRetrieval(
    ...
    } else throw new InternalServerException("Error creating version map of EHR_STATUS");
  ...
  @Override
  public UUID commitWithCustomContribution(
      Timestamp transactionTime, UUID ehrId, ItemStructure otherDetails) {
    return internalCommit(transactionTime, ehrId, otherDetails);
  }
  ...
  private UUID internalCommit(Timestamp transactionTime, UUID ehrId, ItemStructure otherDetails) {
    ...
    if (statusRecord.store() == 0) {
      throw new InvalidApiParameterException(
          "Input EHR couldn't be stored; Storing EHR_STATUS failed");
    }
  ...
  public void setAuditAndContributionAuditValues(
      UUID systemId, UUID committerId, String description) {
    if (systemId != null) this.auditDetailsAccess.setSystemId(systemId);

    if (committerId != null) this.auditDetailsAccess.setCommitter(committerId);

    if (description != null) this.auditDetailsAccess.setDescription(description);
  ...
  public static Integer getLatestVersionNumber(I_DomainAccess domainAccess, UUID statusId) {
    ...
    if (!hasPreviousVersionOfStatus(domainAccess, statusId)) return 1;

    int versionCount =
        domainAccess.getContext().fetchCount(STATUS_HISTORY, STATUS_HISTORY.ID.eq(statusId));
  ...
  public int getEhrStatusVersionFromTimeStamp(Timestamp time) {
    ...
    I_StatusAccess retStatusAccess =
        I_StatusAccess.retrieveInstance(this.getDataAccess(), statusUid);
    ...
    throw new ObjectNotFoundException(
        "EHR_STATUS", "Could not find EHR_STATUS version matching given timestamp");
  }
