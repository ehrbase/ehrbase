 ...
 * NB: read is implemented as a static (generally as retrieveInstanceForExistingFolder(...)) Created
 * by Christian Chevalley on 4/10/2015.
 */
public interface I_SimpleCRUD {
   ...
   * depending on the implementation, records are updated only if one or more field(s) have been
   * changed
   *
   * @return
   * @throws RuntimeException
   ...
   * only implemented at root level object (contribution, composition, ehr). depending on the
   * implementation, records are updated only if one or more field(s) have been changed
   *
   * @return
   * @throws RuntimeException
   */
  ...
  /**
   * delete the Versioned Object associated with the instance implementing this Data Access
   * Interface. Relies on ON DELETE CASCADE
   *
   * @return
   * @throws RuntimeException
   */
