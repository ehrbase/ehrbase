...
public class FolderUtils {
  ...
  public static I_FolderAccess getPath(
      I_FolderAccess folderAccess, int currentIndex, String[] path) {
    ...
    Optional<I_FolderAccess> foundFolder =
  ...
  /**
   * Checks all fields that can be updated from update Folder object and sets the contents to the
   * target folderAccess.
   *
  ...
  /**
   * Checks each sub folder level for conflicts. For this purpose for each sub folder level there
   * will be a set created that contains all names of the siblings as values. If at least one value
   * could not be inserted it will be identified as duplicate and will throw an
   * IllegalArgumentException that results to a 400 error on the controller layer.
   *
      ...
      folder
          ...
          .forEach(
              subFolder -> {
                ...
                // A new entry in the set results to false if there is already a duplicate element
                // existing
                if (!folderNames.add(subFolder.getNameAsString())) {
                  throw new IllegalArgumentException(
                      "Duplicate folder name " + subFolder.getNameAsString());
                } else {
