...
public class FolderAccess extends DataAccess implements I_FolderAccess, Comparable<FolderAccess> {
  ...
  public FolderAccess(
      I_DomainAccess domainAccess, UUID ehrId, I_ContributionAccess contributionAccess) {
    super(domainAccess);
    this.ehrId = ehrId;
    this.folderRecord = getContext().newRecord(org.ehrbase.jooq.pg.tables.Folder.FOLDER);
  ...
  public Boolean update(final Timestamp transactionTime, final boolean force, UUID contribution) {
    ...
    if (contribution == null) {
      ...
      if (this.contributionAccess.getEhrId() == null) {
        final Record1<UUID> result1 =
            getContext()
                .select(CONTRIBUTION.EHR_ID)
                .from(CONTRIBUTION)
                .where(CONTRIBUTION.ID.eq(oldContribution))
                .fetch()
                .get(0);
  ...
  public ObjectVersionId create(UUID customContribution) {
    if (customContribution == null) {
      return new ObjectVersionId(
          this.commit().toString() + "::" + getServerConfig().getNodename() + "::1");
    } else {
      this.contributionAccess =
          I_ContributionAccess.retrieveInstance(getDataAccess(), customContribution);
      return new ObjectVersionId(
          (this.commit(Timestamp.from(Instant.now()), this.contributionAccess.getContributionId())
                  .toString()
              + "::"
  ...
  private Boolean update(
    ...
    if (rootFolder) { // if it is the root folder preserve the original id, itherwise let the DB
      // provide a new one for the overriden subfolders.
    ...
    this.saveFolderItems(
        updatedFolderId, oldContribution, newContribution, transactionTime, getContext());
    ...
    boolean anySubfolderModified =
        this.getSubfoldersList() // Map of sub folders with UUID
  ...
  private void saveFolderItems(
      final UUID folderId,
      final UUID old_contribution,
      final UUID new_contribution,
      final Timestamp transactionTime,
      DSLContext context) {
    ...
    for (ObjectRef or : this.getItems()) {
      ...
      FolderItemsRecord fir =
          new FolderItemsRecord(
              folderId,
              UUID.fromString(or.getId().getValue()),
              new_contribution,
              transactionTime,
  ...
  public UUID commit(Timestamp transactionTime, UUID contributionId) {
    ...
    this.getSubfoldersList()
        .values()
        .forEach(
            child -> {
              child.commit(transactionTime, contributionId);
              FolderHierarchyRecord fhRecord =
                  this.buildFolderHierarchyRecord(
                      this.getFolderRecord().getId(),
  ...
  public static I_FolderAccess retrieveInstanceForExistingFolder(
    ...
    Table<?> sf_table = table(select().from(FOLDER_HIERARCHY));

    Table<?> folder_table = table(select().from(FOLDER)).as("t_folder1");
    Table<?> folder_table2 = table(select().from(FOLDER)).as("t_folder2");

    Table<?> initial_table =
        table(select().from(FOLDER_HIERARCHY).where(FOLDER_HIERARCHY.PARENT_FOLDER.eq(folderId)));

    Field<UUID> subfolderChildFolder =
        field(
            "subfolders.{0}",
    ...
    Result<Record> folderSelectedRecordSub =
        domainAccess
            .getContext()
            ...
            .as(
                select(ArrayUtils.addAll(initial_table.fields(), folder_table.fields()))
                    .from(initial_table)
                    .leftJoin(folder_table)
                    .on(
                        initial_table
                            .field("parent_folder", FOLDER_HIERARCHY.PARENT_FOLDER.getType())
                            .eq(folder_table.field("id", FOLDER.ID.getType())))
                    .union(
                        (select(ArrayUtils.addAll(sf_table.fields(), folder_table2.fields()))
                                .from(sf_table)
    ...
    Map<UUID, Map<UUID, I_FolderAccess>> fHierarchyMap =
        new TreeMap<UUID, Map<UUID, I_FolderAccess>>();
      ...
      fHierarchyMap
          ...
          .put(
              ...
              buildFolderAccessFromFolderId(
                  (UUID) record.getValue("child_folder"), domainAccess, folderSelectedRecordSub));
    }
   ...
   * @param nodeName Nodename (e.g. "[...]::NODENAME::[...]") from the service layer, which is not
   *     accessible in the access layer
  ...
  public static Set<ObjectVersionId> retrieveFolderVersionIdsInContribution(
    ...
    // get whole "version map" of each matching folder and do fine-grain check for matching
    // contribution
    // precondition: each UUID in `folders` set is unique, so for each the "version map" is only
    // created once below
   ...
   */
  private static Map<Record, Integer> getVersionMapOfFolder(
      I_DomainAccess domainAccess, UUID folderId) {
    Map<Record, Integer> versionMap = new HashMap<>();

    // create counter with highest version, to keep track of version number and allow check in the
    // end
    ...
    Result<FolderHistoryRecord> historyRecords =
        domainAccess
            .getContext()
    ...
    if (versionCounter != 0) throw new InternalServerException("Version Map generation failed");
   ...
   *
   * @param fHierarchyMap {@link java.util.Map} containing as key the UUID of each Folder, and as
   *     value an internal Map. For the internal Map the key is the the UUID of a child {@link
   *     com.nedap.archie.rm.directory.Folder}, and the value is the {@link I_FolderAccess} for
   *     enabling DB access to this child.
   * @param currentFolder {@link java.util.UUID} of the current {@link
   *     com.nedap.archie.rm.directory.Folder} to treat in the current recursive call of the method.
   * @param parentFa the parent {@link I_FolderAccess} that corresponds to the parent {@link
   *     com.nedap.archie.rm.directory.Folder} of the {@link com.nedap.archie.rm.directory.Folder}
   *     identified as current.
   * @param folderSelectedRecordSub {@link org.jooq.Result} containing the Records that represent
   *     the rows to retrieve from the DB corresponding to the children hierarchy.
  ...
  private static FolderAccess buildFolderAccessFromGenericRecord(
    ...
    folderAccess
        .getItems()
        .addAll(
            FolderAccess.retrieveItemsByFolderAndContributionId(
                record.value1(), record.value7(), domainAccess));
   ...
   *
   * @param record_ containing the information of a {@link com.nedap.archie.rm.directory.Folder} in
   *     the DB.
   * @param domainAccess containing the DB connection information.
  ...
  private static FolderAccess buildFolderAccessFromFolderId(
    ...
    for (Record record : folderSelectedRecordSub) {
      // if the FOLDER items were returned in the recursive query use them and avoid a DB
      // transaction
    ...
    // if no data from the Folder has been already recovered for the id of the folder, then query
    // the DB for it.
    FolderRecord folderSelectedRecord =
        domainAccess.getContext().selectFrom(FOLDER).where(FOLDER.ID.eq(id)).fetchOne();
        ...
        domainAccess
            ...
            .where(
                field(name("object_ref_id"), FOLDER_ITEMS.OBJECT_REF_ID.getType())
                    .eq(OBJECT_REF.ID)
                    .and(
                        field(name("item_in_contribution"), FOLDER_ITEMS.IN_CONTRIBUTION.getType())
  ...
  private static ObjectRef parseObjectRefRecordIntoObjectRef(
    ...
    ObjectRefId oref =
        new FolderAccess(domainAccess).new ObjectRefId(objectRefRecord.getId().toString());
   ...
   * @param parent folder of the {@link com.nedap.archie.rm.directory.Folder} procided as the
   *     current parameter.
   * @param dateTime of the transaction that will be stored inthe DB.
   * @param ehrId of the {@link com.nedap.archie.rm.ehr.Ehr} referencing the current {@link
   *     com.nedap.archie.rm.directory.Folder}.
  ...
  private static I_FolderAccess buildFolderAccessTreeRecursively(
    ...
    // if the parent already contains the FolderAccess for the specified folder return the
    // corresponding FolderAccess
    if ((parent != null)
        && (parent.getSubfoldersList().containsKey(UUID.fromString(current.getUid().getValue())))) {
      return parent.getSubfoldersList().get(current.getUid());
    }
    // create the corresponding FolderAccess for the current folder
    folderAccess =
        FolderAccess.buildPlainFolderAccess(
              ...
              child -> {
                ...
                I_FolderAccess childFolderAccess =
                    buildNewFolderAccessHierarchy(
                        domainAccess, child, timeStamp, ehrId, contributionAccess);
   ...
   * @param inContribution contribution that the {@link com.nedap.archie.rm.directory.Folder} refers
   *     to.
   ...
   *     childFolder param.
   */
  private final FolderHierarchyRecord buildFolderHierarchyRecord(
      final UUID parentFolder,
      final UUID childFolder,
      final UUID inContribution,
      final Timestamp sysTransaction,
      final Timestamp sysPeriod) {
    ...
    // fhRecord.setSysPeriod(sysPeriod); sys period can be left to null so the system sets it for
    // the temporal tables.
    return fhRecord;
  }
  ...
  // whole ObjectVersionId is just not necessary for DB query, so this works on access layer
  // (without info like the nodeName), too.
  private static Integer getLastVersionNumber(I_DomainAccess domainAccess, UUID folderUuid) {
    ...
    int versionCount =
        domainAccess.getContext().fetchCount(FOLDER_HISTORY, FOLDER_HISTORY.ID.eq(folderUuid));
  ...
  public static int getVersionNumberAtTime(
      ...
      final ObjectVersionId rootFolderId,
      final Timestamp sysTransaction) {
        ...
        domainAccess
            ...
            .fetchCount(
                ...
                FOLDER
                    .ID
                    .equal(folderUuid)
                    .and(FOLDER.SYS_TRANSACTION.lessOrEqual(sysTransaction)));

    // Count all history entries for the root folder
    int folderHistoryCount =
        domainAccess
            .getContext()
            .fetchCount(
                FOLDER_HISTORY,
    ...
    // If we found entries in both tables return the sum. If there is no current entry the count
    // will be 0
  ...
  public static Timestamp getTimestampForVersion(
    ...
    if (currentVersion > version) {
      ...
      Result<FolderHistoryRecord> folderHistoryRecords =
          domainAccess
              .getContext()
              .selectFrom(FOLDER_HISTORY)
      ...
      if (folderHistoryRecords.size() > 0) {
        timestamp =
            folderHistoryRecords
                .get(folderHistoryRecords.size() - 1)
                .get(FOLDER_HISTORY.SYS_TRANSACTION);
      }
    }
    // The timestamp now contains either the last entry found in folder history or the current time
    // if the desired
