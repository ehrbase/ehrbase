...
public class ContributionAccess extends DataAccess implements I_ContributionAccess {
  ...
  public static I_ContributionAccess retrieveInstance(
      I_DomainAccess domainAccess, UUID contributionId) {
    ...
    CompositionAccess.retrieveCompositionsInContribution(
            domainAccess, contributionAccess.contributionRecord.getId())
        .forEach((access, version) -> compos.put(access.getId(), access));
    contributionAccess.compositions = compos;

    // also retrieve attached audit
    contributionAccess.auditDetails =
        new AuditDetailsAccess(domainAccess.getDataAccess())
            .retrieveInstance(
                domainAccess.getDataAccess(), contributionAccess.getHasAuditDetails());

    return contributionAccess;
  }

  // FIXME: should this really only look into *_history table? semantically this only accesses
  // "versions" other than latest (not actual openEHR versions, since contribution has none) - also
  // naming suggestively wrong?
  public static I_ContributionAccess retrieveVersionedInstance(
      I_DomainAccess domainAccess, UUID contributionVersionedObjId, Timestamp transactionTime) {

    ContributionAccess contributionAccess = new ContributionAccess(domainAccess);

    ContributionHistoryRecord contributionHistoryRecord =
        domainAccess
            .getContext()
            .fetchOne(
  ...
  public void addComposition(I_CompositionAccess compositionAccess) {
    ...
    if (compositionAccess.getComposerId() == null)
      compositionAccess.setComposerId(
          auditDetails
              .getCommitter()); // FIXME: does this work, is there an audit instance available here?
    // not tested, function not used right now...
    if (compositionAccess.getAuditDetailsId() == null)
      throw new IllegalArgumentException("Composition has no embedded audit");
    compositionAccess.setContributionId(
        contributionRecord.getId()); // this is the ContributionVersionId!!!
  ...
  public UUID commit(Timestamp transactionTime) {
    ...
    for (I_CompositionAccess compositionAccess : compositions.values()) {
      // composition can only be added when having an audit attached, so this is assumed to be the
      // case here
  ...
  /**
   * Commit the contribution with optional values, excluding audit, which needs to be created and
   * set beforehand.
   */
  @Override
  public UUID commit(
    ...
    setContributionDataType(
        Objects.requireNonNullElse(contributionType, ContributionDataType.other));
  ...
  /**
   * Commit the contribution with (optional) given values (incl. audit data, which is handled
   * embedded)
   *
   * @throws InternalServerException when contribution couldn't be created because of an internal
   *     problem
   */
  @Override
  public UUID commit(
    ...
    if (committerId == null) {
      ...
      if (scheme == null) scheme = "local";
      committerId =
          new PersistedPartyProxy(this)
              .getOrCreate(
                  defaultUser,
  ...
  public Boolean update(
    ...
    this.auditDetails =
        I_AuditDetailsAccess.getInstance(getDataAccess()); // new audit for new action
    ...
    if (contributionChangeType != null)
      this.auditDetails.setChangeType(
  ...
  public Boolean update(Timestamp transactionTime, boolean force) {
    ...
    if (force || contributionRecord.changed()) {
      ...
      if (!contributionRecord.changed()) {
        ...
        contributionRecord.changed(
            CONTRIBUTION.SYS_PERIOD, false); // managed by an external trigger anyway...
      }
      contributionRecord.setSysTransaction(transactionTime);
    ...
    //        List<UUID> allUuids =
    // context.select(COMPOSITION.ID).from(COMPOSITION).where(COMPOSITION.IN_CONTRIBUTION.eq(contributionRecord.getId())).fetch(COMPOSITION.ID);
    //        updateChangedCompositions(CollectionUtils.intersection(allUuids,
    // compositions.keySet()), transactionTime, force);
    //        commitAddedCompositions(CollectionUtils.subtract(compositions.keySet(), allUuids),
    // transactionTime);
  ...
  public void setIncomplete() {
    contributionRecord.setState(
        ContributionState.valueOf(ContributionState.incomplete.getLiteral()));
  }
  ...
  public void setAuditDetailsValues(AuditDetails auditObject) {
    ...
    UUID changeType =
        I_ConceptAccess.fetchContributionChangeType(this, auditObject.getChangeType().getValue());
        ...
        status -> {
          ...
          if (delStatus.isEmpty()) {
            throw new InternalServerException(
                "Admin deletion of Status failed! Unexpected result.");
          }
