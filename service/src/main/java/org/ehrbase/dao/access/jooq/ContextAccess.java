...
public class ContextAccess extends DataAccess implements I_ContextAccess {
  ...
  public static I_ContextAccess retrieveInstance(I_DomainAccess domainAccess, UUID id) {
    ...
    contextAccess.eventContextRecord =
        domainAccess.getContext().fetchOne(EVENT_CONTEXT, EVENT_CONTEXT.ID.eq(id));
  ...
  public static I_ContextAccess retrieveInstance(I_DomainAccess domainAccess, Result<?> records) {
    ...
    eventContextRecord.setStartTime(
        (Timestamp) records.getValue(0, I_CompositionAccess.F_CONTEXT_START_TIME));
    eventContextRecord.setStartTimeTzid(
        (String) records.getValue(0, I_CompositionAccess.F_CONTEXT_START_TIME_TZID));
    eventContextRecord.setEndTime(
        (Timestamp) records.getValue(0, I_CompositionAccess.F_CONTEXT_END_TIME));
    eventContextRecord.setEndTimeTzid(
        (String) records.getValue(0, I_CompositionAccess.F_CONTEXT_END_TIME_TZID));
    eventContextRecord.setLocation(
        (String) records.getValue(0, I_CompositionAccess.F_CONTEXT_LOCATION));
    //        eventContextRecord.setSetting(records.getValue(0,
    // I_CompositionAccess.F_CONTEXT_SETTING));
    eventContextRecord.setOtherContext(
        (JSONB) records.getValue(0, I_CompositionAccess.F_CONTEXT_OTHER_CONTEXT));
  ...
  public static EventContext retrieveHistoricalEventContext(
    ...
    if (eventContextHistoryRecord.getFacility() != null) {
      PartyIdentifiedRecord partyIdentifiedRecord =
          domainAccess
              ...
              .fetchOne(
                  PARTY_IDENTIFIED,
                  PARTY_IDENTIFIED.ID.eq(eventContextHistoryRecord.getFacility()));
        ...
        .forEach(
            record -> {
              ...
              PartyProxy performer =
                  new PersistedPartyProxy(domainAccess).retrieve(record.getPerformer());

              DvInterval<DvDateTime> startTime =
                  convertDvIntervalDvDateTimeFromRecord(eventContextHistoryRecord);
    ...
    DvCodedText setting =
        (DvCodedText)
            new RecordedDvCodedText()
                .fromDB(eventContextHistoryRecord, EVENT_CONTEXT_HISTORY.SETTING);

    return new EventContext(
        healthCareFacility,
        new RecordedDvDateTime()
            .decodeDvDateTime(
                eventContextHistoryRecord.getStartTime(),
  ...
  private static PartyIdentified getPartyIdentifiedFromRecord(
    ...
    if (partyIdentifiedRecord.getPartyRefValue() != null
        && partyIdentifiedRecord.getPartyRefScheme() != null) {
  ...
  public void setRecordFields(UUID id, EventContext eventContext) {
    ...
    if (eventContext.getHealthCareFacility() != null) {
      UUID healthcareFacilityId =
          new PersistedPartyProxy(this).getOrCreate(eventContext.getHealthCareFacility());
    ...
    new RecordedDvCodedText()
        .toDB(eventContextRecord, EVENT_CONTEXT.SETTING, eventContext.getSetting());
    ...
    if (eventContext.getParticipations() != null) {
      for (Participation participation : eventContext.getParticipations()) {
        ...
        new RecordedDvText()
            .toDB(participationRecord, PARTICIPATION.FUNCTION, participation.getFunction());
        if (participation.getMode() != null)
          new RecordedDvCodedText()
              .toDB(participationRecord, PARTICIPATION.MODE, participation.getMode());
    ...
    if (eventContext.getOtherContext() != null
      ...
      eventContextRecord.setOtherContext(
          JSONB.valueOf(new RawJson().marshal(eventContext.getOtherContext())));
    }
  }
  ...
  public Boolean update(Timestamp transactionTime) {
    ...
    for (ParticipationRecord participationRecord : participations) {
      ...
      if (participationRecord.changed()) {
        ...
        try {
          if (getContext()
              .fetchExists(PARTICIPATION, PARTICIPATION.ID.eq(participationRecord.getId()))) {
  ...
  public EventContext mapRmEventContext() {
    ...
    PartyIdentifiedRecord partyIdentifiedRecord =
        getContext()
            .fetchOne(PARTY_IDENTIFIED, PARTY_IDENTIFIED.ID.eq(eventContextRecord.getFacility()));
    // facility identifiers
    PartyIdentified healthCareFacility = null;
    ...
    DvCodedText concept =
        (DvCodedText) new RecordedDvCodedText().fromDB(eventContextRecord, EVENT_CONTEXT.SETTING);
    ...
    return new EventContext(
        ...
        new RecordedDvDateTime()
            .decodeDvDateTime(
                eventContextRecord.getStartTime(), eventContextRecord.getStartTimeTzid()),
        new RecordedDvDateTime()
  ...
  private static DvInterval<DvDateTime> convertDvIntervalDvDateTimeFromRecord(Record record) {
    ...
    if (record.get(PARTICIPATION.TIME_LOWER)
        != null) { // start time null value is allowed for participation
      dvDateTimeDvInterval =
          new DvInterval<>(
              new RecordedDvDateTime()
                  .decodeDvDateTime(
                      record.get(PARTICIPATION.TIME_LOWER),
                      record.get(PARTICIPATION.TIME_LOWER_TZ)),
              new RecordedDvDateTime()
                  .decodeDvDateTime(
                      record.get(PARTICIPATION.TIME_UPPER),
