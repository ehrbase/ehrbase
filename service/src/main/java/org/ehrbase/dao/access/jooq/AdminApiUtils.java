...
public class AdminApiUtils {
  ...
  private void internalDeleteComposition(UUID id) {
    ...
    Result<AdminDeleteCompositionRecord> delCompo =
        Routines.adminDeleteComposition(ctx.configuration(), id);
   ...
   *
   * @param id Audit
   * @param context Object context to build error message, e.g. "Composition" for the audit of a
   *     Composition
   * @param resultCanBeEmpty Config parameter to disable check of result, in case the object is
   *     deleted already (for broader scopes, like EHR itself)
   */
  ...
  public void deleteContribution(UUID id, UUID audit, Boolean resultCanBeEmpty) {
    // delete contribution
    Result<AdminDeleteContributionRecord> rec =
        Routines.adminDeleteContribution(ctx.configuration(), id);
    // and its audit (depending of how this is called, either get audit ID from parameter or from
    // response)
    if (rec.isNotEmpty()) {
   ...
   *
   * @param id Folder
   * @param deleteContributions Option to en- or disable the deletion of all linked contributions.
   *     Disable when calling in context of cascading high level object like EHR.
   */
  public void deleteFolder(UUID id, Boolean deleteContributions) {
    ...
    folders.forEach(
        folder -> ctx.selectQuery(new AdminDeleteFolderHistory().call(folder)).execute());
    contribs.forEach(
        contrib -> ctx.selectQuery(new AdminDeleteFolderObjRefHistory().call(contrib)).execute());
