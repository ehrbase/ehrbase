...
public class TemplateStoreAccess extends DataAccess implements I_TemplateStoreAccess {
  ...
  public static String adminUpdateTemplate(
      I_DomainAccess domainAccess, OPERATIONALTEMPLATE template) {

    // Check if template is used anymore
    Result<AdminGetTemplateUsageRecord> usingCompositions =
        Routines.adminGetTemplateUsage(
            domainAccess.getContext().configuration(), template.getTemplateId().getValue());
  ...
  public static boolean deleteTemplate(I_DomainAccess domainAccess, String templateId) {
    ...
    if (usingCompositions.isNotEmpty()) {
      ...
      return Routines.adminDeleteTemplate(domainAccess.getContext().configuration(), templateId)
          > 0;
    }
  }
  ...
  private static TemplateMetaData buildMetadata(Record2<String, Timestamp> r) {
    ...
    templateMetaData.setOperationaltemplate(
        TemplateStoreAccess.buildOperationaltemplate(r.component1()));
    // @TODO read from DB
    templateMetaData.setCreatedOn(
        OffsetDateTime.ofInstant(r.component2().toInstant(), ZoneId.systemDefault()));
