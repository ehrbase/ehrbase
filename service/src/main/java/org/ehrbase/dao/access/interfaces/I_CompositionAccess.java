...
public interface I_CompositionAccess extends I_SimpleCRUD {
  ...
  static I_CompositionAccess getNewInstance(
      I_DomainAccess domain, Composition composition, UUID ehrId) {
   ...
   */
  static I_CompositionAccess retrieveCompositionVersion(
  ...
  /**
   * Calculate the version corresponding to a {@link com.nedap.archie.rm.ehr.VersionedComposition}
   * which is the closest in time (before) the {@link Timestamp} provided.
   *
   * @param domainAccess The {@link I_DomainAccess} containing the persistence information and DB
   *     connection parameters for persisting a composition.
   * @param vCompositionUid The {@link UUID} corresponding to a {@link
   ...
   */
  static int getVersionFromTimeStamp(
  ...
  /**
   * Returns the instance of a {@link com.nedap.archie.rm.ehr.VersionedComposition} corresponding to
   * the version which is the closest in time before the timeCommitted provided.
   *
   ...
   * @param timeCommitted {java.sql.Timestamp} that indicates the point in time to search version
   *     for the composition backwards.
   * @return the number of the version that is the closest in time (before) the timeCommitted
   *     parameter provided. If a null timeCommitted is provided the latest composition will be
   *     returned.
  ...
  static I_CompositionAccess retrieveInstanceByTimestamp(
    ...
    return CompositionAccess.retrieveInstanceByTimestamp(
        domainAccess, compositionUid, timeCommitted);
  }
   ...
   */
  static Map<I_CompositionAccess, Integer> retrieveInstancesInContribution(
   ...
   */
  static Map<I_CompositionAccess, Integer> getVersionMapOfComposition(
  ...
  /**
   * Unlike the basic commit it is necessary to set the contribution manually beforehand, here. This
   * allows committing with a specific contribution ID, which could be used to process several
   * objects at once.
   *
  ...
  /**
   * Unlike the basic update it is necessary to set the contribution manually beforehand, here. This
   * allows updating with a specific contribution ID, which could be used to process several objects
   * at once.
   *
   ...
   */
  Boolean updateWithCustomContribution(
      UUID committerId,
      UUID systemId,
      I_ConceptAccess.ContributionChangeType contributionChangeType,
      String description);
   ...
   *
   * <ul>
   *   <li>delete entry first
   *   <li>delete the corresponding version entry
   *   <li>delete the composition record
   * </ul>
   *
   * @return
   */
  ...
  /**
   * Unlike the basic delete it is necessary to set the contribution manually beforehand, here. This
   * allows deleting with a specific contribution ID, which could be used to process several objects
   * at once.
   *
   * @param committerId Committer of this change
   * @param systemId System of origin of this change
  ...
  /**
   * FIXME: bug? comment says 2-letter while methods are saying Integer get the 2-letters country
   * code
   *
   * @return
   */
  ...
  /**
   * FIXME: bug? comment says 2-letter while methods are saying Integer set the 2-letters territory
   * code
   *
   * @param code String
   */
