 ...
 *
 * <p>Created by Christian Chevalley on 4/2/2015.
 */
public class CompositionAccess extends DataAccess implements I_CompositionAccess {
   ...
   */
  public static I_CompositionAccess retrieveCompositionVersion(
      I_DomainAccess domainAccess, UUID id, int version) {
    ...
    try (PreparedStatement preparedStatement = connection.prepareStatement(versionQuery)) {
      ...
      try (ResultSet resultSet = preparedStatement.executeQuery()) {
        ...
        while (resultSet.next()) {
          ...
          compositionRecord1.setInContribution(
              UUID.fromString(resultSet.getString("in_contribution")));
      ...
      throw new ObjectNotFoundException(
          COMPOSITION_LITERAL, "Composition not found or or invalid DB content", e);
    }

    if (compositionHistoryAccess != null) {
      compositionHistoryAccess.setContent(
          I_EntryAccess.retrieveInstanceInCompositionVersion(
      ...
      I_ContributionAccess contributionAccess =
          I_ContributionAccess.retrieveVersionedInstance(
              domainAccess,
              compositionHistoryAccess.getContributionId(),
              compositionHistoryAccess.getSysTransaction());
      compositionHistoryAccess.setContributionAccess(contributionAccess);

      I_AuditDetailsAccess auditDetailsAccess =
          new AuditDetailsAccess(domainAccess.getDataAccess())
              .retrieveInstance(
                  domainAccess.getDataAccess(), compositionHistoryAccess.getAuditDetailsId());
      compositionHistoryAccess.setAuditDetailsAccess(auditDetailsAccess);

      // retrieve versioned context
      EventContext historicalEventContext =
          I_ContextAccess.retrieveHistoricalEventContext(
  ...
  public static boolean hasPreviousVersion(I_DomainAccess domainAccess, UUID compositionId) {
    return domainAccess
        .getContext()
        .fetchExists(COMPOSITION_HISTORY, COMPOSITION_HISTORY.ID.eq(compositionId));
  }
  ...
  public static I_CompositionAccess retrieveInstance(I_DomainAccess domainAccess, UUID id) {
    ...
    CompositionRecord compositionRecord =
        domainAccess.getContext().selectFrom(COMPOSITION).where(COMPOSITION.ID.eq(id)).fetchOne();

    if (compositionRecord == null) return null;
    ...
    I_AuditDetailsAccess auditAccess =
        new AuditDetailsAccess(domainAccess.getDataAccess())
  ...
  public static int getVersionFromTimeStamp(
    ...
    if (timeCommitted.compareTo(latestCompoTime)
        >= 0) { // if the timestamp is after or equal to the sys_transaction of the latest
      // composition available, add one since its version has not been counted for being
      // the one stored in the ehr.composition table
      versionComHist++;
    }
    if (versionComHist == 0) {
      throw new IllegalArgumentException(
          "There are no versions available prior to date "
              + timeCommitted
              + " for the the composition with id: "
              + vCompositionUid);
    }
  ...
  public static Map<I_CompositionAccess, Integer> retrieveCompositionsInContribution(
      I_DomainAccess domainAccess, UUID contribution) {
    ...
    // get whole "version map" of each matching composition and do fine-grain check for matching
    // contribution
    // precondition: each UUID in `compositions` set is unique, so for each the "version map" is
    // only created once below
    ...
    for (UUID compositionId : compositions) {
      Map<I_CompositionAccess, Integer> map =
          getVersionMapOfComposition(domainAccess, compositionId);
  ...
  private static Map<I_CompositionAccess, Integer> getVersionMapOfComposition(
    ...
    Result<CompositionHistoryRecord> historyRecords =
        domainAccess
            .getContext()
    ...
    for (CompositionHistoryRecord historyRecord : historyRecords) {
      ...
      historyAccess.setContent(
          I_EntryAccess.retrieveInstanceInComposition(domainAccess, historyAccess));
  ...
  public Optional<UUID> getContextId() {
    if (compositionRecord == null) return Optional.empty();
    if (compositionRecord.getId() == null) return Optional.empty();
    // conditional handling for persistent composition that do not have a event context
    EventContextRecord eventContext =
        getContext()
            .fetchOne(EVENT_CONTEXT, EVENT_CONTEXT.COMPOSITION_ID.eq(compositionRecord.getId()));
  ...
  public int addContent(I_EntryAccess entry) {
    ...
    if (entry.getComposition() != null) composition = entry.getComposition();
  ...
  /**
   * Commit composition (incl embedded audit). The composition' contribution (incl. its audit) and
   * the composition's own audit are assumed to be prepared correctly before executing this
   * commitment.
   *
  ...
  public UUID commit(Timestamp transactionTime) {
    ...
    auditDetailsAccess.setChangeType(
        I_ConceptAccess.fetchContributionChangeType(
            this, I_ConceptAccess.ContributionChangeType.CREATION));
    UUID auditId = this.auditDetailsAccess.commit();
    compositionRecord.setHasAudit(auditId);

    // check if custom contribution is already set, because changing it would yield updating in DB
    // which is not desired (creates wrong new "version")
    if (getContributionId() != null) {
      // check if set contribution is sane
      Optional.ofNullable(I_ContributionAccess.retrieveInstance(this, getContributionId()))
          .orElseThrow(IllegalArgumentException::new);
    } else {
      // if not set, create DB entry of contribution so it can get referenced in this composition
      contributionAccess.setAuditDetailsChangeType(
          I_ConceptAccess.fetchContributionChangeType(
              this, I_ConceptAccess.ContributionChangeType.CREATION));
    ...
    if (content.isEmpty()) log.warn("Composition has no content:");

    try {
      for (I_EntryAccess entryAccess : content) entryAccess.commit(transactionTime);
  ...
  // commit with composition that has contribution already set with setContributionId(...)
  // beforehand
  @Override
  ...
  public Boolean update(Timestamp transactionTime, boolean force) {
    ...
    if (force || compositionRecord.changed()) {
      ...
      if (contextId.isEmpty()) {
        ...
        contextAccess.commit(transactionTime);
      } else contextAccess = I_ContextAccess.retrieveInstance(this, contextId.get());
  ...
  public Boolean update() {
    ...
    contributionAccess.update(
        ...
        null);
    auditDetailsAccess.update(
        null, null, I_ConceptAccess.ContributionChangeType.MODIFICATION, null);
  ...
  public Boolean update(
    ...
    // update both contribution (incl its audit) and the composition's own audit
    contributionAccess.update(
        timestamp, committerId, systemId, null, state, contributionChangeType, description);
    auditDetailsAccess.update(systemId, committerId, contributionChangeType, description);
    return update(timestamp, true);
  }

  @Override
  public Boolean updateWithCustomContribution(
      UUID committerId,
      UUID systemId,
      I_ConceptAccess.ContributionChangeType contributionChangeType,
      String description) {
    ...
    // update only the audit, so it shows the modification change type. a new custom contribution is
    // set beforehand.
    // TODO: db-wise, this way a new audit "version" will be created which is (openEHR-)semantically
    // wrong. but safe and processable anyway. so need to change that or is it okay?
    auditDetailsAccess.update(systemId, committerId, contributionChangeType, description);
    return update(timestamp);
  }
  ...
  public Integer delete(UUID committerId, UUID systemId, String description) {
    ...
    I_AuditDetailsAccess delAudit =
        I_AuditDetailsAccess.getInstance(
            this,
            systemId,
            committerId,
  ...
  private void createAndCommitNewDeletedVersionAsHistory(UUID delAuditId, UUID contrib) {
    ...
    // (Normal approach of first .update() then .delete() won't work, because postgres' transaction
    // optimizer will
    // just skip the update if it will get deleted anyway.)
    // so copy values, but add deletion meta data
    CompositionHistoryAccess newDeletedVersionAsHistoryAccess =
        new CompositionHistoryAccess(getDataAccess());
    CompositionHistoryRecord newRecord =
        getDataAccess().getContext().newRecord(COMPOSITION_HISTORY);
    ...
    if (newDeletedVersionAsHistoryAccess.commit()
        == null) // commit and throw error if nothing was inserted into DB
  ...
  public static boolean isDeleted(I_DomainAccess domainAccess, UUID versionedObjectId) {
    ...
    if (domainAccess
        ...
        .fetchExists(COMPOSITION_HISTORY, COMPOSITION_HISTORY.ID.eq(versionedObjectId))) {
      // retrieve the record
      Result<CompositionHistoryRecord> historyRecordsRes =
          domainAccess
              .getContext()
              .selectFrom(COMPOSITION_HISTORY)
      ...
      AuditDetailsRecord audit =
          domainAccess
              .getContext()
              .fetchOne(AUDIT_DETAILS, AUDIT_DETAILS.ID.eq(historyRecordsRes.get(0).getHasAudit()));
      if (audit == null)
      ...
      if (audit.getChangeType().equals(ContributionChangeType.deleted)) return true;
    } else {
      throw new ObjectNotFoundException(COMPOSITION_LITERAL, "No composition with given ID found");
    }
