...
public class CompositionAccess extends DataAccess implements I_CompositionAccess {
   ...
   *                                  failed
   */
  public CompositionAccess(DSLContext context, I_KnowledgeCache knowledgeManager,
      IntrospectService introspectCache, ServerConfig serverConfig, Composition composition,
      UUID ehrId) {
  ...
  public boolean update(LocalDateTime timestamp, UUID committerId, UUID systemId,
      String description, I_ConceptAccess.ContributionChangeType changeType) {
    ...
    contributionAccess.setAuditDetailsChangeType(
        I_ConceptAccess.fetchContributionChangeType(this, changeType));
    UUID contributionId = this.contributionAccess.commit();
    setContributionId(contributionId);
  ...
  public int delete(LocalDateTime timestamp, UUID committerId, UUID systemId, String description) {
    ...
    UUID contrib = contributionAccess.commit(TransactionTime.millis(), committerId, systemId, null,
        ContributionDef.ContributionState.COMPLETE, I_ConceptAccess.ContributionChangeType.DELETED,
        description);
  ...
  private void createAndCommitNewDeletedVersionAsHistory(UUID delAuditId, UUID contrib) {
    ...
    CompositionHistoryRecord newRecord = getDataAccess().getContext()
        .newRecord(COMPOSITION_HISTORY);
   ...
   */
  public static I_CompositionAccess retrieveCompositionVersion(I_DomainAccess domainAccess, UUID id,
      int version) {
    ...
    try (PreparedStatement preparedStatement = connection.prepareStatement(versionQuery)) {
      ...
      throw new ObjectNotFoundException(COMPOSITION_LITERAL,
          "Composition not found or or invalid DB content", e);
    }

    if (compositionHistoryAccess != null) {
      compositionHistoryAccess.setContent(
          I_EntryAccess.retrieveInstanceInCompositionVersion(domainAccess, compositionHistoryAccess,
              version));
      ...
      I_AuditDetailsAccess auditDetailsAccess = new AuditDetailsAccess(
          domainAccess.getDataAccess()).retrieveInstance(domainAccess.getDataAccess(),
          compositionHistoryAccess.getAuditDetailsId());
  ...
  public static Integer getLastVersionNumber(I_DomainAccess domainAccess, UUID compositionId) {
    ...
    if (domainAccess.getContext().fetchExists(COMPOSITION, COMPOSITION.ID.eq(compositionId))) {
      return 1 + domainAccess.getContext()
  ...
  public static I_CompositionAccess retrieveInstance(I_DomainAccess domainAccess, UUID id) {
    ...
    CompositionRecord compositionRecord = domainAccess.getContext().selectFrom(COMPOSITION)
        .where(COMPOSITION.ID.eq(id)).fetchOne();
    ...
    I_AuditDetailsAccess auditAccess = new AuditDetailsAccess(
        domainAccess.getDataAccess()).retrieveInstance(domainAccess.getDataAccess(),
        compositionAccess.getAuditDetailsId());
  ...
  public static int getVersionFromTimeStamp(I_DomainAccess domainAccess, UUID vCompositionUid,
    ...
    Record result;
    try {
      result = domainAccess.getContext()
          .select(max(COMPOSITION.SYS_TRANSACTION).as("mostRecentInTable")).from(COMPOSITION)
          .where(COMPOSITION.ID.eq(vCompositionUid)).fetchOne();
    ...
    Record result2;
    try {
      result2 = domainAccess.getContext().select(count().as("countVersionInTable"))
          .from(COMPOSITION_HISTORY).where(
              COMPOSITION_HISTORY.SYS_TRANSACTION.lessOrEqual(timeCommitted)
                  .and(COMPOSITION_HISTORY.ID.eq(vCompositionUid))).fetchOne();
    } catch (RuntimeException e) { // generalize SQL exceptions
      throw new InternalServerException("Problem with SQL statement or input", e);
    }
    int versionComHist = (int) result2.get("countVersionInTable");
    if (timeCommitted.compareTo(latestCompoTime)
        >= 0) {//if the timestamp is after or equal to the sys_transaction of the latest composition available, add one since its version has not been counted for being the one stored in the ehr.composition table
      versionComHist++;
    }
    if (versionComHist == 0) {
   ...
   * @throws ObjectNotFoundException  when no composition could be found with given input
   */
  public static I_CompositionAccess retrieveInstanceByTimestamp(I_DomainAccess domainAccess,
      UUID compositionUid, Timestamp timeCommitted) {
  ...
  public static Map<ObjectVersionId, I_CompositionAccess> retrieveCompositionsInContribution(
    ...
    domainAccess.getContext().select(COMPOSITION.ID).from(COMPOSITION)
        .where(COMPOSITION.IN_CONTRIBUTION.eq(contribution)).fetch()
    ...
    for (UUID compositionId : compositions) {
      Map<Integer, I_CompositionAccess> map = getVersionMapOfComposition(domainAccess,
          compositionId);
  ...
  public String getFeederAudit() {
    return compositionRecord.getFeederAudit() == null ? null
  ...
  public static boolean isDeleted(I_DomainAccess domainAccess, UUID versionedObjectId) {
    ...
    if (domainAccess.getContext()
        .fetchExists(COMPOSITION, COMPOSITION.ID.eq(versionedObjectId))) {
      return false;
    }
    ...
    if (domainAccess.getContext()
      ...
      AuditDetailsRecord audit = domainAccess.getContext()
          .fetchOne(AUDIT_DETAILS, AUDIT_DETAILS.ID.eq(historyRecordsRes.get(0).getHasAudit()));
      if (audit == null) {
        throw new InternalServerException(
            "DB inconsistency: couldn't retrieve referenced audit");
      }
