...
public interface I_ConceptAccess {
  ...
  static String fetchConceptLiteral(
      I_DomainAccess domainAccess, Integer conceptId, String language) {
    return domainAccess
        .getContext()
        .fetchAny(CONCEPT, CONCEPT.CONCEPTID.eq(conceptId).and(CONCEPT.LANGUAGE.equal(language)))
        .getDescription();
  ...
  static UUID fetchContributionChangeType(I_DomainAccess domainAccess, String changeTypeStr) {
    ContributionChangeType contributionChangeType =
        ContributionChangeType.valueOf(changeTypeStr.toUpperCase());
