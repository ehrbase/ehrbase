...
/**
 * Deals with composition links. For example to link an ACTION with an INSTRUCTION with or an
 * OBSERVATION Created by christian on 9/12/2016.
 */
public interface I_CompoXrefAccess {
   ...
   * @return A list of children represented as map, where each child has its system transaction time
   *     as value
   */
  Map<UUID, Timestamp> getLinkList(UUID masterUid);

  /**
   * Retrieve only the latest (by system transaction time) of children matching given parent/master
   * ID
   *
   * @param masterUid parent or master {@link UUID}
