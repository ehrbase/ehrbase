 ...
 * related to the actual Ehr owner (eg. patient or Named Subject generally). Created by Christian
 * Chevalley on 4/21/2015.
 */
public interface I_EhrAccess extends I_SimpleCRUD {
  ...
  /**
   * // TODO: keep! likely to be used by EHR controller retrieve an Ehr for a subject identification
   * with an identifier<br>
   * a subject identification consists of the issuer identification (ex. NHS) and an identification
   * code
   *
   ...
   */
  static UUID retrieveInstanceBySubject(
  ...
  /**
   * // TODO: keep! likely to be used by EHR controller retrieve an Ehr for a subject UUID<br>
   * a subject identification consists of the issuer identification (ex. NHS) and an identification
   * code
   *
   * @param domainAccess SQL access
   ...
   * @throws IllegalArgumentException if retrieving failed for given input
   */
  static I_EhrAccess retrieveInstanceByStatus(
      I_DomainAccess domainAccess, UUID ehrId, UUID status, int version) {
   ...
   * @throws IllegalArgumentException when no EHR found for ID
   */
  static Map<String, Map<String, String>> getCompositionList(
  ...
  /**
   * Updates the whole EHR access in the DB, e.g. to update the status. Embeds contribution and
   * audit handling.
   *
