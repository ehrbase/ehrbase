...
public class FolderAccessTest {
  ...
  public void beforeClass() {
    ...
    try {
      testDomainAccess =
          new DummyDataAccess(context, null, null, KnowledgeCacheHelper.buildServerConfig());
    } catch (Exception e) {
      e.printStackTrace();
    }
  }
  ...
  public void shouldRetriveFolderAccessByUid() throws Exception {
    ...
    FolderAccess fa2 =
        (FolderAccess)
            FolderAccess.retrieveInstanceForExistingFolder(
    ...
    assertEquals(
        "{\"details\": \"xxx1\"}", ((FolderAccess) fa2).getFolderRecord().getDetails().toString());

    assertNotNull(
        fa2.getSubfoldersList().get(UUID.fromString("99550555-ec91-4025-838d-09ddb4e999cb")));
  ...
  public void shouldInsertFolderWithNoSubfolders() throws Exception {
    // the creation and commit returning valid ids implies that the FolderMockDataProvider.java has
    // provided the corresponding result for each SQL generated when inserting
    ...
    FolderAccess fa2 =
        (FolderAccess)
            FolderAccess.getNewFolderAccessInstance(
                fa1,
                folder,
                DateTime.now(),
                UUID.fromString("f6a2af65-fe89-45a4-9456-07c5e17b1634"));
    ...
    String expected =
        ("'{\n"
            ...
            .replaceAll(
                "\\n|\\r\\n",
                System.getProperty(
                    "line.separator")); // avoids problems amond different platforms due to
    // different representations of line change.
    StringWriter expectedStringWriter = new StringWriter();
    PrintWriter printWriter = new PrintWriter(expectedStringWriter);
    printWriter.print(
        expected); // avoids problems amond different platforms due to different representations of
    // line change.
    printWriter.close();
  ...
  public void shouldInsertFolderWithSubfolders() throws Exception {
    ...
    printWriter =
        new PrintWriter(
            expectedStringWriter); // avoids problems amond different platforms due to different
    ...
    printWriter.print(
        expected3); // avoids problems amond different platforms due to different representations of
    // line change.
    assertEquals(
        expectedStringWriter.toString(),
        ((FolderAccess)
  ...
  public void shouldUpdateExistingFolder() throws Exception {
    ...
    I_FolderAccess fa4 =
        fa3.getSubfoldersList().get(UUID.fromString("33550555-ec91-4025-838d-09ddb4e999cb"));
    fa4.setFolderNArchetypeNodeId("new archetype node id");

    // perform update in a third level subforlder
    I_FolderAccess fa5 =
        fa4.getSubfoldersList().get(UUID.fromString("8701233c-c8fd-47ba-91b5-ef9ff23c259b"));
    fa5.setIsFolderActive(false);

    // the mock data provider checks the UPDATE SQL code generated and returns 1 simulating the
    // update of one row in the FOLDER table.
    ...
    // assertEquals(true, updated);// could not manage to mock the result "Affected row(s)
    // : 1" from updates" for the UPDATEE statement. The mock data provider returns 0 irrespectively
    // of weather the record is modified in the DB or not. As a consequence this always is false in
    // the mocked version.
  }
  ...
  public void shouldDeleteExistingFolder() {
    ...
    // assertEquals(5, affectedRows); commented since metadata indicating that affected rows are 5
    // could not be mocked. Now checked relyingon the Moch Data Provider that intercepts the SQL
    // code generated.
  }
  ...
  private Folder generateFolderFromTestFile(FolderTestDataCanonicalJson testEntry)
      throws IOException {
