...
public class QueryProcessorTest {
  ...
  private static List<AqlTestCase> buildAqlTestCase() {
    ...
    testCases.add(
        new AqlTestCase(
            ...
            "select distinct on (\"/ehr_id/value\") \"\".\"/ehr_id/value\" "
                ...
                + "right outer join \"ehr\".\"composition\" as \"composition_join\" on \"composition_join\".\"id\" = \"ehr\".\"entry\".\"composition_id\" "
                + "right outer join \"ehr\".\"ehr\" as \"ehr_join\" on \"ehr_join\".\"id\" = \"composition_join\".\"ehr_id\" "
                + "where (\"ehr\".\"entry\".\"template_id\" = ? and (\"ehr\".\"entry\".\"entry\" #>> '{/composition[openEHR-EHR-COMPOSITION.health_summary.v1],/content[openEHR-EHR-ACTION.immunisation_procedure.v1],0,/description[at0001],/items[at0002],0,/value,value}' IN ('Hepatitis A','Hepatitis B')))) as \"\"",
            true));
    ...
    //                //Do to the way the query is build it is not possible to build sql if the AQL
    // contains only compositions which have no instances in the DB. Thus we must manual handle this
    // case.
    //                "select 1 where 1 = 0",
    //                false));
    ...
    //                "select count(DISTINCT \"_FCT_ARG_0\") as \"count\" from (select
    // ehr.js_dv_date_time(\"ehr_join\".\"date_created\",\"ehr_join\".\"date_created_tzid\")::text
    // as \"_FCT_ARG_0\" from \"ehr\".\"entry\" right outer join \"ehr\".\"composition\" as
    // \"composition_join\" on \"composition_join\".\"id\" = \"ehr\".\"entry\".\"composition_id\"
    // right outer join \"ehr\".\"ehr\" as \"ehr_join\" on \"ehr_join\".\"id\" =
    // \"composition_join\".\"ehr_id\") as \"\"",
    //                true
    //        ));
