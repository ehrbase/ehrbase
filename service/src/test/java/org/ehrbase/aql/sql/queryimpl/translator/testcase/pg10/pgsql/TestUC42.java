...
public class TestUC42 extends UC42 {

    public TestUC42(){
        super();
        this.expectedSqlExpression =
                "(select ARRAY.COLUMN, ARRAY.COLUMN, ARRAY.COLUMN from \"ehr\".\"entry\" right outer join \"ehr\".\"composition\" as \"composition_join\" on \"composition_join\".\"id\" = \"ehr\".\"entry\".\"composition_id\" left outer join lateral (select (select \"composition_join\".\"id\"||'::'||'local'||'::'||1 + COALESCE(\n" +
                        "(select count(*)\n" +
                        ...
                        "    'UTC'\n" +
                        "  )\n" +
                        ") as jsonb),'value') as varchar) > cast('2020-01-01' as varchar)) as \"COLUMN\") as \"ARRAY\" on ? where \"ehr\".\"entry\".\"template_id\" = ?) union (select ARRAY.COLUMN, ARRAY.COLUMN, ARRAY.COLUMN from \"ehr\".\"entry\" right outer join \"ehr\".\"composition\" as \"composition_join\" on \"composition_join\".\"id\" = \"ehr\".\"entry\".\"composition_id\" left outer join lateral (select (select \"composition_join\".\"id\"||'::'||'local'||'::'||1 + COALESCE(\n" +
                        "(select count(*)\n" +
                        "from \"ehr\".\"composition_history\"\n" +
                        "where \"composition_join\".\"id\" = \"ehr\".\"composition_history\".\"id\"\n" +
                        "group by \"ehr\".\"composition_history\".\"id\"), 0) as \"Diagnose\" where cast(jsonb_extract_path_text(cast(\"ehr\".\"js_dv_coded_text_inner\"(\"ehr\".\"entry\".\"name\") as jsonb),'value') as varchar) = cast('Diagnose' as varchar)) as \"COLUMN\") as \"ARRAY\" on ? left outer join lateral (select (select \"composition_join\".\"id\"||'::'||'local'||'::'||1 + COALESCE(\n" +
