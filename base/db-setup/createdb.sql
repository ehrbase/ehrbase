...
CREATE
DATABASE ehrbase ENCODING 'UTF-8' TEMPLATE template0;
...
ALTER
DATABASE ehrbase SET search_path TO "$user",public,ext;
-- ensure INTERVAL is ISO8601 encoded
alter
database ehrbase SET intervalstyle = 'iso_8601';

GRANT ALL
ON ALL FUNCTIONS IN SCHEMA ext TO ehrbase;
...
CREATE
OR REPLACE FUNCTION ext.versioning()
RETURNS TRIGGER AS $$
DECLARE
sys_period text;
  history_table
...
:= TG_ARGV[3];
  ...
  IF
ignore_unchanged_values AND TG_OP = 'UPDATE' AND NEW IS NOT DISTINCT FROM OLD THEN
    RETURN OLD;
END IF;
...
END IF;
  IF
holder.atttypid != to_regtype('tstzrange') THEN
...
INTO holder2
FROM pg_range
WHERE rngtypid = holder.atttypid;
IF
FOUND THEN
...
END IF;
    ...
    RAISE
'system period column "%" of relation "%" is not a range but type %', sys_period, TG_TABLE_NAME, format_type(holder.atttypid, null) USING
    ERRCODE = 'datatype_mismatch';
END IF;
...
TG_OP = 'UPDATE' OR TG_OP = 'DELETE' THEN
    ...
    IF
OLD.xmin::text >= (txid_snapshot_xmin(transaction_info) % (2^32)::bigint)::text
...
END IF;
    ...
    IF
NOT EXISTS(SELECT * FROM pg_attribute WHERE attrelid = history_table::regclass AND attname = sys_period AND NOT attisdropped) THEN
...
TG_ARGV[2] = 'true' THEN
      ...
      IF
range_lower >= time_stamp_to_use THEN
