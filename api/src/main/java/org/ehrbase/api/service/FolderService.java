...
public interface FolderService extends BaseService {
  ...
  /**
   * Creates a new folder entry at the database from content. The provided content from request
   * payload will be serialized before corresponding to the given source format. The folder will be
   * linked to the EHR addressed by the request.
   *
   ...
   * @param ehrId - ID for the corresponding EHR
   * @param content - {@link com.nedap.archie.rm.directory.Folder} to persist
   * @param contribution Optional (can be null) custom contribution to use, instead of a generic new
   *     one
   * @return UUID of the new created Folder from database
   */
  ...
  /**
   * Retrieves a folder from database identified by object_version_uid and extracts the given sub
   * path of existing. If the object_version_uid does not contain a version number the latest entry
   * will be returned. A path with common unix like root notation '/' will be treated as if there is
  ...
  /**
   * Fetches an folder entry from database identified by the root folder uid and the given
   * timestamp. If the current version has ben modified after the given timestamp the folder will be
   * searched inside the folder history table.
   *
   * @param folderId - object_version_uid for target folder
  ...
  /**
   * Updates a target folder entry identified by the given folderId with new content. The content
   * string will be serialized from the given source format.
   *
   ...
   * @return Updated folder entry
   */
  Optional<FolderDto> update(
      ObjectVersionId folderId, Folder update, UUID ehrId, UUID contribution);

  /**
   * Marks a given folder as deleted and moves it into the history table. The folder will no longer
   * be accessible without time or version information available.
   *
  ...
  /**
   * Serializes folder content from request body into a structured string that can be used by
   * database save mechanism to parse in into jsonb format that will be saved at the database.
   *
  ...
  /**
   * Searches the last version number of a given folder entry from database. This will be executed
   * against the folder table which contains the latest entry of a folder. The version will be
  ...
  /**
   * Searches for the folder version that was the current version at the given timestamp. If the
   * entry from folder table has a later timestamp the folder history will be queried to find the
   * version at the timestamp
   *
