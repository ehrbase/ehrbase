...
public interface TerminologyServer<T, ID, U> {
  ...
  List<T> expandWithParameters(
      ...
      @SuppressWarnings("unchecked")
          U... operationParams); // warning is ignored because the specific implementation will type
  // the method avoiding possible heap pollution

  /**
   * Searches all the attributes associated with the concept that corresponds to the provided ID.
   *
   * @param conceptId
   * @return A complex Object of type T that contains all the attributes directly associated to the
   *     concept identified by the provided ID.
   */
  ...
  /**
   * Evaluates if the concept provided as one operationParams belongs to the value set provided as
   * another operationParam.
   *
   * @param dynamic list of parameters to perform the operation against an external terminology
   *     server.
   * @return true if the concept belongs to the specified value set.
   */
   ...
   * @return {@link org.ehrbase.aql.compiler.tsclient.TerminologyServer.SubsumptionResult}
   *     indicating the result of the subsumption evaluation.
   */
