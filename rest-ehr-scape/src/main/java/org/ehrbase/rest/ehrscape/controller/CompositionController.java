...
public class CompositionController extends BaseController {
  ...
  public ResponseEntity<CompositionWriteRestResponseData> createComposition(
      @ApiParam(value = "Format of the Composition", allowableValues = "XML, ECISFLAT, FLAT")
      ...
      @ApiParam(value = "Id of the EHR to which the Composition is added")
          @RequestParam(value = "ehrId")
          UUID ehrId,
  ...
  public ResponseEntity<CompositionResponseData> getComposition(
      @ApiParam(value = "Id of the Composition in the form uuid::version") @PathVariable("id")
          String compositionId,
      @ApiParam(value = "Format of the Composition", allowableValues = "XML, ECISFLAT, FLAT, RAW")
          @RequestParam(value = "format", defaultValue = "XML")
          CompositionFormat format) {
  ...
  public ResponseEntity<ActionRestResponseData> update(
      @ApiParam(value = "UUID of the Composition ") @PathVariable("id") UUID compositionId,
      @ApiParam(value = "Format of the Composition", allowableValues = "XML, ECISFLAT, FLAT")
          @RequestParam(value = "format", defaultValue = "XML")
          CompositionFormat format,
      @RequestParam(value = "templateId", required = false) String templateId,
  ...
  public ResponseEntity<ActionRestResponseData> delete(
      @ApiParam(value = "UUID of the Composition ") @PathVariable("id") UUID compositionId) {
  ...
  private UUID getCompositionUid(String fullcompositionUid) {
    if (!fullcompositionUid.contains("::")) return UUID.fromString(fullcompositionUid);
  ...
  private int getCompositionVersion(String fullcompositionUid) {
    if (!fullcompositionUid.contains("::")) return 1; // current version
