...
#### EHR Endpoint
- `EHR` responses now contain in `ehr_status` a reference to the `EHR_STATUS` object, and not the object itself.
- When creating an `EHR`, `EHR_STATUS` now also supports the `links` attribute.

#### EHR Status Endpoint
- `EHR_STATUS` now also supports the `links` attribute in requests and responses.
- `ORIGINAL_VERSION` responses now contain in `contribution` a reference to the `CONTRIBUTION` object, and not the object itself.

#### EHR Composition Endpoint
- `ORIGINAL_VERSION` responses now contain in `contribution` a reference to the `CONTRIBUTION` object, and not the object itself.

#### EHR Directory Endpoint
- Folder responses no longer contain empty `item` and `folders` arrays.
- Root folders now include all LOCATABLE attributes, e.g. `archetype_node_id`, `archetype_details`, and `links`.

#### Stored Query Endpoint
