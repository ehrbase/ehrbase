...
MF-004 Execute Ad-Hoc Query - Get EHRs (filtered: top 5)
    ...
    # NOT WORKING - REQUIRES SEPARATE FLOW
    A/107_get_ehrs_top_5.json    A/107.tmp.json

MF-005 Execute Ad-Hoc Query - Get EHRs (ordered by: time-created)
    [Template]          execute ad-hoc query and check result (loaded DB)
    [Tags]              not-ready

    # NOT WORKING - REQUIRES SEPARATE FLOW & QUERY FAILS
    A/108_get_ehrs_orderby_time-created    A/108.tmp.json

MF-006 Execute Ad-Hoc Query - Get EHRs (filtered: timewindow)
    [Template]          execute ad-hoc query and check result (loaded DB)
    [Tags]              not-ready

    # NOT WORKING - REQUIRES SEPARATE FLOW & QUERY FAILS
    A/109_get_ehrs_within_timewindow       A/109.tmp.json
...
MF-008 Execute Ad-Hoc Query - Get Compositions From EHR By ID
    [Template]          execute ad-hoc query and check result (loaded DB)
    [Tags]              not-ready
...
Make Temp Copy of Query Blueprint
    Return From Keyword If    ${blueprint[2:5]}<400  NOPE
...
Update Temp Result Data-Set
    [Arguments]         ${dataset}
                        Pass Execution

Create Temp List
    [Documentation]     exposes var {temp_list} to test-suite level scope
    [Arguments]         @{list_items}   
                        Log Many  ${list_items} 

    @{temp_list}=       Set Variable    ${list_items}
                        Set Suite Variable    ${temp_list}    ${temp_list}


Establish Preconditions

    # TODO: change order
    # 1. Load Blueprints of Queries and Expected-Results
    # 2. Generate Test-Data and Expected-Results
    #    a) Upload Required OPTs
    #    b) Create EHR records
    #    c) Commit Compositions
    #    d) Generate Expected-Results for Loaded Test-Data
...
Preconditions (PART 2) - Load Result-Blueprints
    ...
    # DATA
    @{blueprints}=      Create List    D/200_select_data_values_from_all_ehrs_contains_composition.json
                ...
                FOR     ${blueprint}    IN    @{blueprints}
                        ...
                        # comment: for queries >= D/400 we also need temp copy of the query itself
                        #          because they have a placeholder __MODIFY_EHR_ID_1__
                        # Run Keyword If  ${blueprint[2:5]}>=400    Make Temp Copy of Query Blueprint
                        Make Temp Copy of Query Blueprint
                END
...
Commit Compo
    ...
    # TODOO: use this vars and remove the old ones above @WLAD
    ...
    # TODO-NOTE: B/100 oder Update Test-Data-Set B/100
...
B/800
    ...
    ${query}=           Load JSON From File    ${VALID QUERY DATA SETS}/B/800_query.tmp.json
                        Update Value To Json   ${query}    $.q    SELECT c FROM COMPOSITION c [uid/value='${composition_uid}']
    ...
    ${expected}=        Load JSON From File    ${QUERY RESULTS LOADED DB}/B/800.tmp.json
                        Update Value To Json   ${expected}    $.q    SELECT c FROM COMPOSITION c [uid/value='${composition_uid}']
                        Update Value To Json   ${expected}    $.meta._executed_aql    SELECT c FROM COMPOSITION c [uid/value='${composition_uid}']
                        Add Object To Json     ${expected}    $.rows    ${response.body}
...
B/801
    ...
    ${query}=           Load JSON From File    ${VALID QUERY DATA SETS}/B/801_query.tmp.json
                        Update Value To Json   ${query}    $.query_parameters['uid']    ${composition_uid}
...
B/802
    ...
    ${query}=           Load JSON From File    ${VALID QUERY DATA SETS}/B/802_query.tmp.json
                        Update Value To Json   ${query}    $.q    SELECT c FROM COMPOSITION c WHERE c/uid/value='${composition_uid}'
    ...
    ${expected}=        Load JSON From File    ${QUERY RESULTS LOADED DB}/B/802.tmp.json
                        Update Value To Json   ${expected}    $.q    SELECT c FROM COMPOSITION c WHERE c/uid/value='${composition_uid}'
                        Update Value To Json   ${expected}    $.meta._executed_aql    SELECT c FROM COMPOSITION c WHERE c/uid/value='${composition_uid}'
                        Add Object To Json     ${expected}    $.rows    ${response.body}
...
D/200
                        ...
                        Add Object To Json    ${expected}    $.rows    ${temp_list}
...
D/201
                        ...
                        Load Temp Result Data-Set    D/201
                        ...
                        Add Object To Json    ${expected}    $.rows    ${temp_list}
                        Output    ${expected}    ${QUERY RESULTS LOADED DB}/D/201.tmp.json
                        Clean Up Suite Variables
...
D/300
                    # same flow as D/200
                        ...
                        Load Temp Result Data-Set    D/300
                        Create Temp List    ${ehr_id_value}[0]    ${time_created}[0]    ${system_id}[0]
                        Add Object To Json    ${expected}    $.rows    ${temp_list}
                        Output    ${expected}    ${QUERY RESULTS LOADED DB}/D/300.tmp.json
                        Clean Up Suite Variables
...
D/301
                    # same flow as D/201
                        ...
                        Load Temp Result Data-Set    D/301
                        # TODO-NOTE: same problem with {time_created} as in data-set A/105 --> check comment there!!!
                        Create Temp List    ${ehr_id_obj}[0]  ${time_created}[0]  ${system_id_obj}[0]
                        Add Object To Json    ${expected}    $.rows    ${temp_list}
                        Output    ${expected}    ${QUERY RESULTS LOADED DB}/D/301.tmp.json
                        Clean Up Suite Variables
...
D/302
                    # same flow as D/200
                        ...
                        Load Temp Result Data-Set    D/302
                        Create Temp List    ${ehr_id_value}[0]    ${time_created}[0]    ${system_id}[0]
                        Add Object To Json    ${expected}    $.rows    ${temp_list}
                        Output    ${expected}    ${QUERY RESULTS LOADED DB}/D/302.tmp.json
                        Clean Up Suite Variables
...
D/303
                    # same flow as D/201
                        ...
                        Load Temp Result Data-Set    D/303
                        # TODO-NOTE: same problem with {time_created} as in data-set A/105 --> check comment there!!!
                        Create Temp List    ${ehr_id_obj}[0]  ${time_created}[0]  ${system_id_obj}[0]
                        Add Object To Json    ${expected}    $.rows    ${temp_list}
                        Output    ${expected}    ${QUERY RESULTS LOADED DB}/D/303.tmp.json
                        Clean Up Suite Variables
...
D/304
                    # same flow as D/200
                        ...
                        Load Temp Result Data-Set    D/304
                        Create Temp List    ${ehr_id_value}[0]    ${time_created}[0]    ${system_id}[0]
                        Add Object To Json    ${expected}    $.rows    ${temp_list}
                        Output    ${expected}    ${QUERY RESULTS LOADED DB}/D/304.tmp.json
                        Clean Up Suite Variables
...
D/306
                    # same flow as D/200
                        ...
                        Load Temp Result Data-Set    D/306
                        Create Temp List    ${ehr_id_value}[0]    ${time_created}[0]    ${system_id}[0]
                        Add Object To Json    ${expected}    $.rows    ${temp_list}
                        Output    ${expected}    ${QUERY RESULTS LOADED DB}/D/306.tmp.json
                        Clean Up Suite Variables
...
D/307
                    # same flow as D/201
                        ...
                        Load Temp Result Data-Set    D/307
                        # TODO-NOTE: same problem with {time_created} as in data-set A/105 --> check comment there!!!
                        Create Temp List    ${ehr_id_obj}[0]  ${time_created}[0]  ${system_id_obj}[0]
                        Add Object To Json    ${expected}    $.rows    ${temp_list}
                        Output    ${expected}    ${QUERY RESULTS LOADED DB}/D/307.tmp.json
                        Clean Up Suite Variables
...
D/308
                    # same flow as D/200
                        ...
                        Load Temp Result Data-Set    D/308
                        Create Temp List    ${ehr_id_value}[0]    ${time_created}[0]    ${system_id}[0]
                        Add Object To Json    ${expected}    $.rows    ${temp_list}
                        Output    ${expected}    ${QUERY RESULTS LOADED DB}/D/308.tmp.json
                        Clean Up Suite Variables
...
D/309
                    # same flow as D/201
                        ...
                        Create Temp List    ${ehr_id_obj}[0]  ${time_created}[0]  ${system_id_obj}[0]
                        Add Object To Json    ${expected}    $.rows    ${temp_list}
                        Output    ${expected}    ${QUERY RESULTS LOADED DB}/D/309.tmp.json
                        Clean Up Suite Variables
...
D/310
                    # same flow as D/200
                        ...
                        Load Temp Result Data-Set    D/310
                        Create Temp List    ${ehr_id_value}[0]    ${time_created}[0]    ${system_id}[0]
                        Add Object To Json    ${expected}    $.rows    ${temp_list}
                        Output    ${expected}    ${QUERY RESULTS LOADED DB}/D/310.tmp.json
                        Clean Up Suite Variables
...
D/311
                    # same flow as D/201
                        ...
                        Load Temp Result Data-Set    D/311
                        # TODO-NOTE: same problem with {time_created} as in data-set A/105 --> check comment there!!!
                        Create Temp List    ${ehr_id_obj}[0]  ${time_created}[0]  ${system_id_obj}[0]
                        Add Object To Json    ${expected}    $.rows    ${temp_list}
                        Output    ${expected}    ${QUERY RESULTS LOADED DB}/D/311.tmp.json
                        Clean Up Suite Variables
...
D/312
                    # same flow as D/200
                        ...
                        Load Temp Result Data-Set    D/312
                        Create Temp List    ${ehr_id_value}[0]    ${time_created}[0]    ${system_id}[0]
                        ...                 TODOO: ONLY TOP 5 SHOULD BE HERE!
                        Add Object To Json    ${expected}    $.rows    ${temp_list}
                        Output    ${expected}    ${QUERY RESULTS LOADED DB}/D/312.tmp.json
                        Clean Up Suite Variables

D/400
                        Return From Keyword If    ${ehr_index}!=1    NOTHING TO DO HERE!

                        # comment: updates the query
                        Run Keyword if    ${ehr_index}==1    Update Temp Query Data-Set    D/400

                        # comment: updates expected result set
                        # Load Temp Result Data-Set  D/400
                        Create Temp List    ${compo_uid_value}    ${compo_name_value}    ${compo_archetype_node_id}
                        ...                 ${compo_language}    ${compo_territory}   ${compo_category_value}

                        # Update Temp Result Data-Set    D/400
                        ...
                        Add Object To Json     ${expected}    $.rows    ${temp_list}
                        Output    ${expected}     ${QUERY RESULTS LOADED DB}/D/400.tmp.json
                        Clean Up Suite Variables
...
#     ...          AND    Add Object To Json    ${expected}    $.rows    ${temp_list}
