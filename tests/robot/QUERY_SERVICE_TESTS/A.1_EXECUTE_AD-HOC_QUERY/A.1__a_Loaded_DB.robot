...
MF-019 Execute Ad-HOc Query - Get Data
    [Documentation]     Get Data related queries.
    [Template]          execute ad-hoc query and check result (loaded DB)
    [Tags]              not-ready
...
Make Temp Copy of Query Blueprint
    # Return From Keyword If    ${blueprint[2:5]}<400    NOTHING TO DO HERE!
    Return From Keyword If    not (${blueprint[2:5]}>=400)    NOTHING TO DO HERE!
...
Commit Compo
    ...
    # TODOO: remmove line
    # ${compo_content_archetype_node_id}=    Set Variable    ${response.body.content[0].archetype_node_id}
    ...
    B/100
                        ...
                        Run Keyword If    "${compo_archetype_id_value}"=="openEHR-EHR-COMPOSITION.minimal.v1"    B/300

                        Run Keyword If    "${compo_content_type}"=="OBSERVATION"    B/500

                        Run Keyword If    "${compo_content_type}"=="EVALUATION"    B/501

                        Run Keyword If    "${compo_content_archetype_node_id}"=="openEHR-EHR-OBSERVATION.minimal.v1"    B/600

                        Run Keyword If    "${compo_content_archetype_node_id}"=="openEHR-EHR-EVALUATION.minimal.v1"    B/601

                        Run Keyword If    "${compo_content_archetype_node_id}"=="openEHR-EHR-OBSERVATION.minimal.v1"
                        ...                B/700 701 702    B/700
                        ...
                        Run Keyword If    ${ehr_index}==1 and "${compo_content_type}"=="OBSERVATION"   C/300

                        Run Keyword If    ${ehr_index}==1 and "${compo_content_type}"=="EVALUATION"   C/301

                        Run Keyword If    "${compo_content_archetype_node_id}"=="openEHR-EHR-OBSERVATION.minimal.v1"   C/400

                        Run Keyword If    "${compo_content_archetype_node_id}"=="openEHR-EHR-OBSERVATION.minimal.v1"   C/500
