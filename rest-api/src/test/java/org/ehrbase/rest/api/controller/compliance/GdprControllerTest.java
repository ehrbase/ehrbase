...
class GdprControllerTest {
    ...
    private final GdprController controller =
            new GdprController(mockDsl, mockEhrService, mockAuditService, mockRequestContext);

    @Test
    void exportInvalidEhrId() {
        assertThatThrownBy(() -> controller.exportPatientData("not-a-uuid"))
                .isInstanceOf(InvalidApiParameterException.class);
    }

    @Test
    void pseudonymizeInvalidEhrId() {
        assertThatThrownBy(() -> controller.pseudonymize("not-a-uuid"))
                .isInstanceOf(InvalidApiParameterException.class);
    }
}
