...
public final class StoredQueryRequestUtils {
    ...
    public static Optional<Long> getOptionalLong(Map<String, Object> map, String key) {
        return Optional.ofNullable(map).map(m -> m.get(key)).map(o -> switch (o) {
            ...
            case String s -> {
                try {
                    ...
                    throw new InvalidApiParameterException("invalid '%s' value '%s'".formatted(key, s));
                }
            }
            default -> throw new InvalidApiParameterException("invalid '%s' value '%s'".formatted(key, o));
        });
    }
