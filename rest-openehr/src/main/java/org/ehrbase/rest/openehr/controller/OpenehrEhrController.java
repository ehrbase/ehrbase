...
public class OpenehrEhrController extends BaseController {
  ...
  @ApiOperation(
      value = "Create a new EHR with an auto-generated identifier.",
      response = EhrResponseData.class)
  @ApiNotes(
      "ehrPostPutEhrWithStatus.md") // TODO this utilizes a workaround, see source class for info
  @ApiResponses(
      value = {
        @ApiResponse(
            code = 201,
            message =
                "Successfully created - new EHR has been successfully created. The EHR resource is returned in the body when the Prefer header has the value of return=representation. The default for Prefer header (or when Prefer header if missing) is return=minimal. The Location header is always returned.",
            responseHeaders = {
              @ResponseHeader(
                  name = CONTENT_TYPE,
        ...
        @ApiResponse(
            code = 400,
            message = "Bad request - Request body (if provided) could not be parsed."),
        @ApiResponse(
            code = 406,
            message = "Not Acceptable - Service can not fulfil requested Accept format."),
        @ApiResponse(
            code = 409,
            message =
                "Conflict - Unable to create a new EHR due to a conflict with an already existing EHR with the same subject id, namespace pair.")
      })
  ...
  public ResponseEntity createEhr(
      ...
      @ApiParam(value = "Client should specify expected response format")
          @RequestHeader(value = HttpHeaders.ACCEPT, required = false)
          String accept,
      @ApiParam(value = "May be used by clients for resource representation negotiation")
          @RequestHeader(value = PREFER, required = false, defaultValue = RETURN_MINIMAL)
          String prefer,
      @ApiParam(value = "An ehr_status may be supplied as the request body")
  ...
  @ApiOperation(
      value = "Create a new EHR with the specified EHR identifier.",
      response = EhrResponseData.class)
  @ApiNotes(
      "ehrPostPutEhrWithStatus.md") // TODO this utilizes a workaround, see source class for info
  @ApiResponses(
      value = {
        ...
        @ApiResponse(
            code = 400,
            message =
                "Bad request - Request body (if provided) or when supplied ehr_id doesn't follow the specification."),
        @ApiResponse(
            code = 406,
            message = "Not Acceptable - Service can not fulfil requested Accept format."),
        @ApiResponse(
            code = 409,
            message =
                "Conflict - Unable to create a new EHR due to a conflict with an already existing EHR. Can happen when the supplied ehr_id is already already used by an existing EHR.")
      })
  @ResponseStatus(value = HttpStatus.CREATED)
  // overwrites default 200, fixes the wrong listing of 200 in swagger-ui (EHR-56)
  public ResponseEntity<EhrResponseData> createEhrWithId(
      @ApiParam(value = "Optional custom request header for versioning")
          @RequestHeader(value = "openEHR-VERSION", required = false)
          String openehrVersion,
      @ApiParam(value = "Optional custom request header for auditing")
          @RequestHeader(value = "openEHR-AUDIT_DETAILS", required = false)
          String openehrAuditDetails,
      @ApiParam(value = "Client should specify expected response format")
          @RequestHeader(value = HttpHeaders.ACCEPT, required = false)
      ...
      @ApiParam(value = "May be used by clients for resource representation negotiation")
          ...
          String prefer,
      @ApiParam(value = "User supplied EHR ID", required = true) @PathVariable(value = "ehr_id")
          String ehrIdString,
      @ApiParam(value = "An ehr_status may be supplied as the request body")
          @RequestBody(required = false)
          EhrStatus ehrStatus) {

    UUID ehrId; // can't use getEhrUuid(..) because here another exception needs to be thrown (->
    // 400, not 404 in response)
    try {
  ...
  private ResponseEntity<EhrResponseData> internalPostEhrProcessing(
      String accept, String prefer, UUID resultEhrId) {
    URI url =
        URI.create(
            this.encodePath(
                getBaseEnvLinkURL() + "/rest/openehr/v1/ehr/" + resultEhrId.toString()));

    List<String> headerList =
        Arrays.asList(
            CONTENT_TYPE, LOCATION, ETAG, LAST_MODIFIED); // whatever is required by REST spec
        ...
        .map(
            i ->
                Optional.ofNullable(i.getResponseData())
                    .map(j -> ResponseEntity.created(url).headers(i.getHeaders()).body(j))
  ...
  @ApiOperation(
      value = "Retrieve the EHR with the specified ehr_id.",
      response = EhrResponseData.class)
  @ApiResponses(
      value = {
        @ApiResponse(
            code = 200,
            message = "Ok - EHR resource is successfully retrieved.",
            responseHeaders = {
              @ResponseHeader(
                  name = CONTENT_TYPE,
                  description = RESP_CONTENT_TYPE_DESC,
  ...
  @ApiOperation(
      value = "Retrieve the EHR with the specified subject_id and subject_namespace.",
      response = EhrResponseData.class)
  @ApiResponses(
      value = {
        @ApiResponse(
        ...
        @ApiResponse(
            code = 404,
            message = "Not Found - EHR with supplied subject parameters does not exist."),
        @ApiResponse(
            code = 406,
            message = "Not Acceptable - Service can not fulfil requested Accept format."),
        @ApiResponse(code = 415, message = "Unsupported Media Type - Type not supported.")
  ...
  public ResponseEntity<EhrResponseData> retrieveEhrBySubject(
      @ApiParam(value = "Client should specify expected response format")
          @RequestHeader(value = HttpHeaders.ACCEPT, required = false)
          String accept,
      @ApiParam(value = "subject id", required = true) @RequestParam(value = "subject_id")
          String subjectId,
      @ApiParam(value = "subject namespace", required = true)
          @RequestParam(value = "subject_namespace")
          String subjectNamespace) {

    Optional<UUID> ehrIdOpt = ehrService.findBySubject(subjectId, subjectNamespace);

    UUID ehrId =
        ehrIdOpt.orElseThrow(
            () ->
                new ObjectNotFoundException(
                    "ehr", "No EHR with supplied subject parameters found"));
   ...
   * @param <T> Either EhrResponseData itself or more specific sub-class
   *     EhrResponseDataRepresentation
   * @return
   */
  private <T extends EhrResponseData> Optional<InternalResponse<T>> buildEhrResponseData(
    ...
    if (StringUtils.isBlank(accept)
        || accept.equals("*/*")) { // "*/*" is standard for "any mime-type"
