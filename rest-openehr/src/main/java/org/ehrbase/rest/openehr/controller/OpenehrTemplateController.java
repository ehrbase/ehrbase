...
@RestController
@RequestMapping(
    path = "/rest/openehr/v1/definition/template",
...
public class OpenehrTemplateController extends BaseController {
  ...
  */
  @PostMapping("/adl1.4")
  @ApiOperation(
      value = "Upload a new ADL 1.4 operational template (OPT).",
      response = TemplateResponseData.class)
  @ApiResponses(
      value = {
        @ApiResponse(
            code = 201,
            message =
                "Created successfully. Depending on the header Prefer either an empty body or a full representation body is returned.",
            responseHeaders = {
              @ResponseHeader(
                  name = CONTENT_TYPE,
              ...
              @ResponseHeader(
                  ...
                  response = URI.class), // version string in location header optional
        ...
        @ApiResponse(
            ...
            message = "Bad request - unable to upload a template, because of invalid content."),
        // TODO: "This response is optional." how to proceed?
        @ApiResponse(
            code = 409,
            response = ErrorResponseData.class,
            message = "Version Conflict - template with given id and version already exists."),
        @ApiResponse(
            code = 406,
            response = ErrorResponseData.class,
            message = RESP_NOT_ACCEPTABLE_DESC),
        @ApiResponse(
            code = 415,
            response = ErrorResponseData.class,
            message = RESP_UNSUPPORTED_MEDIA_DESC)
      })
  ...
  public ResponseEntity createTemplateClassic(
      @ApiParam(value = REQ_OPENEHR_VERSION)
          ...
          String openehrVersion, // TODO, see EHR-267
    ...
    if (!MediaType.parseMediaType(contentType).isCompatibleWith(MediaType.APPLICATION_XML)) {
      return ResponseEntity.status(HttpStatus.UNSUPPORTED_MEDIA_TYPE)
          .body("Only XML is supported at the moment");
    }
    ...
    URI uri =
        URI.create(
            this.encodePath(
                getBaseEnvLinkURL() + "/rest/openehr/v1/definition/template/adl1.4/" + templateId));
    ...
    if (Optional.ofNullable(prefer)
      ...
      respData =
          buildTemplateResponseData(
              templateId, accept, uri, headerList, () -> new TemplateResponseData());
        ...
        .map(
            i ->
                Optional.ofNullable(i.getResponseData())
                    .map(j -> ResponseEntity.created(uri).headers(i.getHeaders()).body(j.get()))
  ...
  // Note: based on latest-branch of 1.1.0 release of openEHR REST API, because this endpoint was
  // changed significantly
  @GetMapping("/adl1.4")
  @ApiOperation(
      value = "List all available ADL 1.4 operational templates on the system.",
      response = TemplateResponseData.class)
  @ApiResponses(
      value = {
        @ApiResponse(
  ...
  public ResponseEntity getTemplatesClassic(
      ...
      @ApiParam(value = REQ_OPENEHR_AUDIT)
          @RequestHeader(value = "openEHR-AUDIT_DETAILS", required = false)
          String openehrAuditDetails, // TODO, see EHR-267
      @ApiParam(value = REQ_ACCEPT) @RequestHeader(value = ACCEPT, required = false)
          String accept) {

    URI uri =
        URI.create(
            this.encodePath(getBaseEnvLinkURL() + "/rest/openehr/v1/definition/template/adl1.4"));

    List<String> headerList =
        Collections
            .emptyList(); // whatever is required by REST spec - CONTENT_TYPE only needed for 201,
                          // so handled separately
    ...
    return respData
        .map(i -> ResponseEntity.ok().headers(i.getHeaders()).body(i.getResponseData().get()))
        .orElse(ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).build());
  }
  ...
  @ApiOperation(
      value = "Gets a specified ADL 1.4 operational template.",
      response = TemplateResponseData.class)
  @ApiResponses(
      value = {
        @ApiResponse(
            ...
            responseHeaders = {
              ...
              @ResponseHeader(
                  name = LOCATION,
                  description = RESP_LOCATION_DESC,
                  response = URI.class), // only for single template
              @ResponseHeader(
                  name = ETAG,
                  description = RESP_ETAG_DESC,
                  response = String.class), // only for single template
              @ResponseHeader(
                  name = LAST_MODIFIED,
                  description = RESP_LAST_MODIFIED_DESC,
                  response = long.class) // only for single template
            }),
        @ApiResponse(
            code = 404,
            response = ErrorResponseData.class,
            message = "Object Not Found - template with the specified id does not exist."),
        @ApiResponse(
            code = 406,
            response = ErrorResponseData.class,
            message = RESP_NOT_ACCEPTABLE_DESC)
  ...
  public ResponseEntity getTemplateClassic(
      ...
      @ApiParam(value = REQ_OPENEHR_AUDIT)
          ...
          String openehrAuditDetails, // TODO, see EHR-267
      @ApiParam(value = REQ_ACCEPT) @RequestHeader(value = ACCEPT, required = false) String accept,
      @ApiParam(value = "Template ID", required = true) @PathVariable(value = "template_id")
          String templateId) {
  ...
  */
  ...
  @ApiOperation(
      value = "Upload a new ADL 2 operational template.",
      response = TemplateResponseData.class)
  @ApiResponses(
      value = {
        @ApiResponse(
  ...
  public ResponseEntity<TemplateResponseData> createTemplateNew(
      ...
      @ApiParam(value = REQ_ACCEPT) @RequestHeader(value = ACCEPT, required = false) String accept,
      @ApiParam(value = REQ_PREFER) @RequestHeader(value = PREFER, required = false) String prefer,
      @ApiParam(value = "a SEMVER version format, i.e. 1.0.1")
  ...
  // TODO possible changes pending, keep an eye on:
  // https://github.com/openEHR/specifications-ITS-REST/issues/85
  ...
  @ApiOperation(
      value =
          "List all available ADL 2 operational templates on the system or a specific one. (Combined endpoint)",
      response = TemplateResponseData.class)
  @ApiResponses(
      value = {
        @ApiResponse(
  ...
  public ResponseEntity<TemplateResponseData> getTemplateNew(
      ...
      @ApiParam(value = REQ_OPENEHR_AUDIT)
          ...
          String openehrAuditDetails, // TODO, see EHR-267
      @ApiParam(value = REQ_ACCEPT) @RequestHeader(value = ACCEPT, required = false) String accept,
      @ApiParam(value = "Template ID") @PathVariable(value = "template_id", required = false)
          String templateId,
      @ApiParam(value = "SEMVER version pattern")
  ...
  /**
   * Builds response data for template endpoints. As specified there are two kinds of returns, with
   * one or all templates. Hence factory can work with TemplateResponseData or
  ...
  private <T> Optional<InternalResponse<T>> buildTemplateResponseData(
    ...
    // create either TemplateResponseData or null (means no body, only headers incl. link to
    ...
    // parse and set accepted format. with XML as fallback for empty header and error for non
    // supported header
    ...
    if (oneOrAllTemplates != null) {
      if (oneOrAllTemplates
          .getClass()
          .equals(TemplateResponseData.class)) { // get only one template
      ...
      } else if (oneOrAllTemplates
          .getClass()
          .equals(TemplatesResponseData.class)) { // get all templates
        ...
        throw new InternalServerException(
            "Building template response data failed"); // i.e. wrong usage of
                                                       // buildTemplateResponseData()
    }
