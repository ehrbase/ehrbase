...
/**
 * Admin API controller for EHR related endpoints. Provides methods to update and delete EHRs
 * physically in the DB.
 */
@Api(tags = {"Admin", "EHR"})
...
public class OpenehrAdminEhrController extends BaseController {
  ...
  @ApiResponses(
      value = {
        @ApiResponse(
            code = 200,
            message =
                "EHR has been updated and number of updated items will be returned in the body.",
            responseHeaders = {
              @ResponseHeader(
                  name = CONTENT_TYPE,
        ...
        @ApiResponse(code = 401, message = "Client credentials are invalid or have been expired."),
        @ApiResponse(
            code = 403,
            message =
                "Client is not permitted to access this resource since the admin role is missing."),
        @ApiResponse(code = 404, message = "EHR with id could not be found.")
      })
  public ResponseEntity<AdminUpdateResponseData> updateEhr(
      @ApiParam(value = "Client requested response content type")
          ...
          String accept,
      @ApiParam(value = "Target EHR id to update", required = true) @PathVariable(value = "ehr_id")
          String ehrId) {
    ...
    if (!ehrService.hasEhr(ehrUuid)) {
      throw new ObjectNotFoundException(
          "Admin EHR", String.format("EHR with id %s does not exist.", ehrId));
    }
  ...
  @ApiResponses(
      value = {
        @ApiResponse(code = 204, message = "EHR has been deleted successfully"),
        @ApiResponse(code = 401, message = "Client credentials are invalid or have been expired."),
        @ApiResponse(
            code = 403,
            message =
  ...
  public ResponseEntity<AdminDeleteResponseData> deleteEhr(
      @ApiParam(value = "Target EHR id to delete", required = true) @PathVariable(value = "ehr_id")
          String ehrId) {
