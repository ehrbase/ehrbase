...
@RestController
@RequestMapping(
    path = "/rest/openehr/v1/definition/query",
...
public class OpenehrDefinitionQueryController extends BaseController {
  ...
  public ResponseEntity<QueryDefinitionListResponseData> getStoredQueryList(
      ...
      @ApiParam(
              value = "query name to be listed, example: org.openehr::compositions",
              required = false)
  ...
  @RequestMapping(
      ...
      method = RequestMethod.GET) //
  @ApiOperation(
      value = "Get stored query and info/metadata",
      response = QueryDefinitionResponseData.class)
  @ApiResponses(
      value = {
        @ApiResponse(
            code = 200,
  ...
  public ResponseEntity<QueryDefinitionResponseData> getStoredQueryVersion(
      ...
      @ApiParam(
          ...
          String qualifiedQueryName,
      @ApiParam(value = "query version (SEMVER), example: 1.2.3") @PathVariable(value = "version")
          Optional<String> version) {

    log.debug(
        "getStoredQueryVersion invoked with the following input: "
            + qualifiedQueryName
            + ", version:"
            + version);

    QueryDefinitionResponseData queryDefinitionResponseData =
        new QueryDefinitionResponseData(
            queryService.retrieveStoredQuery(
                qualifiedQueryName, version.isPresent() ? version.get() : null));
  ...
  public ResponseEntity<QueryDefinitionResponseData> putStoreQuery(
      @ApiParam(value = REQ_ACCEPT) @RequestHeader(value = ACCEPT, required = false) String accept,
      @ApiParam(value = "query name to store, example: org.openehr::compositions", required = true)
          @PathVariable(value = "qualified_query_name")
          String qualifiedQueryName,
      @ApiParam(value = "query version (SEMVER), example: 1.2.3") @PathVariable(value = "version")
          Optional<String> version,
      @ApiParam(value = "query type (default AQL)")
    ...
    log.debug(
        "putStoreQuery invoked with the following input: "
            + qualifiedQueryName
            + ", version:"
            + version
            + ", query:"
    ...
    QueryDefinitionResponseData queryDefinitionResponseData =
        new QueryDefinitionResponseData(
            queryService.createStoredQuery(
                qualifiedQueryName, version.isPresent() ? version.get() : null, aql));
  ...
  public ResponseEntity<QueryDefinitionResponseData> deleteStoredQuery(
      ...
      @ApiParam(value = "query name to store, example: org.openehr::compositions", required = true)
          @PathVariable(value = "qualified_query_name")
          String qualifiedQueryName,
      @ApiParam(value = "query version (SEMVER), example: 1.2.3", required = true)
          @PathVariable(value = "version")
          String version) {

    log.debug(
        "deleteStoredQuery for the following input: "
            + qualifiedQueryName
            + ", version:"
            + version);
    ...
    QueryDefinitionResponseData queryDefinitionResponseData =
        new QueryDefinitionResponseData(
            queryService.deleteStoredQuery(qualifiedQueryName, version));
