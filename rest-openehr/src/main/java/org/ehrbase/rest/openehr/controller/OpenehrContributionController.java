...
public class OpenehrContributionController extends BaseController {
  ...
  @OperationNotesResourcesReaderOpenehr.ApiNotes(
      "contributionPost.md") // this utilizes a workaround, see source class for info
  ...
  @ApiResponses(
      value = {
        ...
        @ApiResponse(
            code = 204,
            message =
                "No Content - New Contribution was created but not full representation requested. Details in response headers.",
            responseHeaders = {
              @ResponseHeader(
                  name = LOCATION,
                  description = RESP_LOCATION_DESC,
  ...
  public ResponseEntity createContribution(
      ...
      @ApiParam(value = "EHR identifier taken from EHR.ehr_id.value", required = true)
          ...
          String ehrIdString,
      @ApiParam(value = "The contribution to create", required = true) @RequestBody
          String contribution) {
    UUID ehrId = getEhrUuid(ehrIdString);

    UUID contributionId =
        contributionService.commitContribution(
            ehrId, contribution, extractCompositionFormat(contentType));
    ...
    List<String> headerList =
        Arrays.asList(
            LOCATION,
            ETAG); // whatever is required by REST spec - CONTENT_TYPE only needed for 201, so
                   // handled separately
    ...
    if (Optional.ofNullable(prefer)
      ...
      respData =
          buildContributionResponseData(contributionId, ehrId, accept, uri, headerList, () -> null);
    }
  ...
  @ApiResponses(
      value = {
        @ApiResponse(
            code = 200,
            response = CompositionResponseData.class,
            message = "Contribution found and returned.",
            responseHeaders = {
              @ResponseHeader(
                  name = CONTENT_TYPE,
                  description = RESP_CONTENT_TYPE_DESC,
        ...
        @ApiResponse(
            ...
            response = ErrorResponseData.class,
            message =
                "Not Found - No EHR with the supplied ehr_id or no Contribution with the supplied contribution_uid was found.")
      })
  public ResponseEntity getContribution(
      ...
      @ApiParam(value = "EHR identifier taken from EHR.ehr_id.value", required = true)
          @PathVariable(value = "ehr_id")
          String ehrIdString,
      @ApiParam(value = "", required = true) @PathVariable(value = "contribution_uid")
        ...
        URI.create(
            this.encodePath(
                getBaseEnvLinkURL()
                    ...
                    + ehrId.toString()
                    + "/contribution/"
                    + contributionUid.toString()));

    List<String> headerList =
        Arrays.asList(
            LOCATION,
            ETAG,
            LAST_MODIFIED); // whatever is required by REST spec - CONTENT_TYPE handled separately
  ...
  private <T extends ContributionResponseData>
      Optional<InternalResponse<T>> buildContributionResponseData(
          UUID contributionId,
          UUID ehrId,
          String accept,
          URI uri,
    ...
    if (minimalOrRepresentation != null) {
      ...
      Optional<ContributionDto> contribution =
          contributionService.getContribution(ehrId, contributionId);
