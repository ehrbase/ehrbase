 ...
 *
 * <p>TODO WIP state only implements endpoints from outer server side, everything else is a stub.
 * Also with a lot of duplication at the moment, which should be reduced when implementing
 * functionality.
 */
@Api(tags = "Composition")
@RestController
@RequestMapping(
    path = "/rest/openehr/v1/ehr",
...
public class OpenehrCompositionController extends BaseController {
  ...
  @ApiResponses(
      value = {
        @ApiResponse(
            code = 201,
            response = CompositionResponseData.class,
            message =
                "Successfully created - New COMPOSITION was created. Content body is only returned when Prefer header has return=representation, otherwise only headers are returned.",
            responseHeaders = {
              @ResponseHeader(
                  name = CONTENT_TYPE,
              ...
              @ResponseHeader(
                  name = LOCATION,
                  description = RESP_LOCATION_DESC,
                  response = URI.class),
              @ResponseHeader(name = ETAG, description = RESP_ETAG_DESC, response = String.class),
              @ResponseHeader(
                  name = LAST_MODIFIED,
                  description = RESP_LAST_MODIFIED_DESC,
                  response = long.class)
            }),
        @ApiResponse(
            code = 204,
            message =
                "No Content - New COMPOSITION was created but not full representation requested. Details in response headers.",
            responseHeaders = {
              @ResponseHeader(
                  name = LOCATION,
                  description = RESP_LOCATION_DESC,
  ...
  public ResponseEntity createComposition(
      @ApiParam(value = REQ_OPENEHR_VERSION)
          @RequestHeader(value = "openEHR-VERSION", required = false)
          String openehrVersion,
      @ApiParam(value = REQ_OPENEHR_AUDIT)
          @RequestHeader(value = "openEHR-AUDIT_DETAILS", required = false)
          String openehrAuditDetails,
      @ApiParam(value = REQ_CONTENT_TYPE_BODY, required = true) @RequestHeader(value = CONTENT_TYPE)
          String contentType,
      ...
      @ApiParam(value = "EHR identifier taken from EHR.ehr_id.value", required = true)
          @PathVariable(value = "ehr_id")
          String ehrIdString,
      @ApiParam(value = "The composition to create", required = true) @RequestBody
          String composition) {
        ...
        URI.create(
            this.encodePath(
                getBaseEnvLinkURL()
                    ...
                    + ehrId.toString()
                    + "/composition/"
                    + compositionUuid.toString()));

    List<String> headerList =
        Arrays.asList(
            LOCATION,
            ETAG,
            LAST_MODIFIED); // whatever is required by REST spec - CONTENT_TYPE only needed for 201,
    ...
    if (Optional.ofNullable(prefer)
      ...
      respData =
          buildCompositionResponseData(compositionUuid, 0, accept, uri, headerList, () -> null);
    }

    // returns 201 with body + headers, 204 only with headers or 500 error depending on what
    // processing above yields
    return respData
        .map(
            i ->
                Optional.ofNullable(i.getResponseData())
                    .map(StructuredString::getValue)
                    .map(j -> ResponseEntity.created(uri).headers(i.getHeaders()).body(j))
  ...
  @ApiResponses(
      value = {
        ...
        @ApiResponse(
            code = 204,
            message =
                "No Content - COMPOSITION was updated but no full representation requested. Details in response headers.",
            responseHeaders = {
              @ResponseHeader(
                  name = LOCATION,
                  description = RESP_LOCATION_DESC,
        ...
        @ApiResponse(
            code = 201,
            message =
                "(not valid, ignore. documentation produces this entry automatically."), // workaround to avoid confusion with auto-generated 201 (EHR-56)
        @ApiResponse(
            code = 400,
            response = ErrorResponseData.class,
            message =
        ...
        @ApiResponse(
            ...
            response = ErrorResponseData.class,
            message =
                "Not Found - No EHR with the supplied ehr_id or no COMPOSITION with the supplied object_id."),
        @ApiResponse(
            code = 409,
            response = ErrorResponseData.class,
            message =
                "Version Conflict - Returned when supplied version_uid is not the latest version. Returns latest version in the Location and ETag headers.")
      })
  public ResponseEntity updateComposition(
      ...
      @ApiParam(value = REQ_OPENEHR_AUDIT)
          @RequestHeader(value = "openEHR-AUDIT_DETAILS", required = false)
          String openehrAuditDetails,
      @ApiParam(value = REQ_CONTENT_TYPE_BODY)
          @RequestHeader(value = CONTENT_TYPE, required = false)
          String contentType,
      @ApiParam(value = REQ_ACCEPT) @RequestHeader(value = ACCEPT, required = false) String accept,
      @ApiParam(value = REQ_PREFER) @RequestHeader(value = PREFER, required = false) String prefer,
      @ApiParam(value = "{preceding_version_uid}", required = true) @RequestHeader(value = IF_MATCH)
          String ifMatch,
      @ApiParam(value = "EHR identifier taken from EHR.ehr_id.value", required = true)
          @PathVariable(value = "ehr_id")
          String ehrIdString,
      @ApiParam(value = "identifier of the VERSIONED COMPOSITION to be updated.", required = true)
    ...
    // If body already contains a composition uid it must match the {versioned_object_uid} in
    // request url
    ...
    inputUuid.ifPresent(
        id -> {
          // TODO currently the this part of the spec is implemented as "the request body's
          // composition version_uid must be compatible to the given versioned_object_uid"
    ...
    try {
      ...
      String compositionVersionUid =
          compositionService.update(versionedObjectUid, compositionFormat, composition);
      ...
      List<String> headerList =
          Arrays.asList(
              LOCATION,
              ETAG,
              LAST_MODIFIED); // whatever is required by REST spec - CONTENT_TYPE only needed for
      // 200, so handled separately

      if (RETURN_REPRESENTATION.equals(prefer)) {
        // both options extract needed info from versionUid
        respData =
            buildCompositionResponseData(
                extractVersionedObjectUidFromVersionUid(compositionVersionUid),
    ...
    // returns 200 with body + headers, 204 only with headers or 500 error depending on what
    // processing above yields
    return respData
        .map(
            i ->
  ...
  @ApiResponses(
      value = {
        @ApiResponse(
            code = 204,
            message = "COMPOSITION was deleted.",
            responseHeaders = {
              @ResponseHeader(
                  name = LOCATION,
                  description = RESP_LOCATION_DESC,
        ...
        // TODO @ApiResponse(code = 201, message = "(not valid, ignore. documentation produces this
        // entry automatically."), // workaround to avoid confusion with auto-generated 201 (EHR-56)
        @ApiResponse(
            code = 400,
            response = ErrorResponseData.class,
            message =
                "Bad request - The composition with preceding_version_uid is already deleted."),
        @ApiResponse(
            code = 404,
            response = ErrorResponseData.class,
            message =
                "Not Found - No EHR with the supplied ehr_id or no COMPOSITION with the supplied preceding_version_uid."),
        @ApiResponse(
            code = 409,
            response = ErrorResponseData.class,
            message =
                "Version Conflict - Returned when supplied preceding_version_uid doesn’t match the latest version. Returns latest version in the Location and ETag headers.",
            responseHeaders = {
              @ResponseHeader(
                  name = LOCATION,
                  description = RESP_LOCATION_DESC,
  ...
  @ResponseStatus(
      value =
          HttpStatus
              .NO_CONTENT) // overwrites default 200, fixes the wrong listing of 200 in swagger-ui
  // (EHR-56)
  public ResponseEntity deleteComposition(
    ...
    if (!compositionService
      ...
      // 409 is returned when supplied preceding_version_uid doesn’t match the latest version.
    ...
    try { // the actual deleting
      LocalDateTime time =
          compositionService.delete(extractVersionedObjectUidFromVersionUid(precedingVersionUid));
  ...
  @ApiResponses(
      value = {
        ...
        @ApiResponse(
            code = 204,
            message =
                "No Content - Returned when the composition is deleted (logically). (Note: ignore body)",
            response = ResponseEntity.class,
            responseHeaders = { // response = ResponseEntity.class removes body
              @ResponseHeader(
                  name = CONTENT_TYPE,
                  description = RESP_CONTENT_TYPE_DESC,
                  response =
                      MediaType
                          .class), // TODO is the spec correct saying that a content-type header is
              // necessary here?
              @ResponseHeader(name = ETAG, description = RESP_ETAG_DESC, response = String.class)
            }),
        @ApiResponse(
            ...
            response = ErrorResponseData.class,
            message =
                "Not Found - No EHR with the supplied ehr_id or no COMPOSITION with the supplied version_uid.")
      })
  public ResponseEntity<CompositionResponseData> getCompositionByVersionId(
      ...
      @ApiParam(value = "EHR identifier taken from EHR.ehr_id.value", required = true)
          @PathVariable(value = "ehr_id")
          String ehrIdString,
      @ApiParam(value = "VERSION identifier", required = true) @PathVariable(value = "version_uid")
          String versionUid,
      @ApiParam(value = "A timestamp in the ISO8601 format", hidden = true)
  ...
  @ApiResponses(
      value = {
        ...
        @ApiResponse(
            code = 204,
            message =
                "No Content - The COMPOSITION at specified version_at_time time has been deleted. (Note: ignore body)",
            response = ResponseEntity.class,
            responseHeaders = { // response = ResponseEntity.class removes body
              @ResponseHeader(
                  name = CONTENT_TYPE,
        ...
        @ApiResponse(
            ...
            response = ErrorResponseData.class,
            message =
                "Not Found - No EHR with the supplied ehr_id or no VERSIONED_COMPOSITION with the supplied versioned_object_uid or no COMPOSITION at specified version_at_time time.")
      })
  public ResponseEntity getCompositionByTime(
      ...
      @ApiParam(value = "EHR identifier taken from EHR.ehr_id.value", required = true)
          ...
          String ehrIdString,
      @ApiParam(
              value = "VERSIONED_COMPOSITION identifier taken from VERSIONED_COMPOSITION.uid.value",
              required = true)
          @PathVariable(value = "versioned_object_uid")
          String versionedObjectUid,
      @ApiParam(value = "A timestamp in the ISO8601 format")
          @RequestParam(value = "version_at_time", required = false)
          LocalDateTime versionAtTime) {
    UUID ehrId = getEhrUuid(ehrIdString);

    // Note: Since this method can be called by another mapping as "almost overloaded" function some
    // parameters might be semantically named wrong in that case. E.g. versionedObjectUid can
    // contain a versionUid.
    // Note: versionUid should be of format "uuid::domain::version", versionObjectUid of format
    // "uuid"
    UUID compositionUid =
    ...
    if (extractVersionFromVersionUid(versionedObjectUid) != 0) {
      ...
      if (versionAtTime != null) {
        ...
        Optional<Integer> versionFromTimestamp =
            Optional.ofNullable(
                compositionService.getVersionByTimestamp(compositionUid, versionAtTime));
        version =
            versionFromTimestamp.orElseThrow(
                () ->
                    new ObjectNotFoundException(
    ...
    return respData
        .map(
            i ->
                Optional.ofNullable(i.getResponseData().getValue())
                    .map(j -> ResponseEntity.ok().headers(i.getHeaders()).body(j))
                    // when the body is empty
                    .orElse(ResponseEntity.noContent().headers(i.getHeaders()).build()))
        // when no response could be created at all
  ...
  @ApiResponses(
      value = {
        ...
        @ApiResponse(
            ...
            response = ErrorResponseData.class,
            message =
                "Not Found - No EHR with the supplied ehr_id or no VERSIONED_COMPOSITION with the supplied versioned_object_uid.")
      })
  // TODO @ResponseStatus(value= HttpStatus.NO_CONTENT)  // overwrites default 200, fixes the wrong
  // listing of 200 in swagger-ui (EHR-56)
  public ResponseEntity<VersionedCompositionResponseData> getVersionedCompositionById(
      @ApiParam(value = "EHR identifier taken from EHR.ehr_id.value", required = true)
          @PathVariable(value = "ehr_id")
          String ehrIdString,
      @ApiParam(
    ...
    return Optional.ofNullable(data)
        .map(i -> new ResponseEntity<>(i, headers, HttpStatus.OK))
  ...
  // TODO @ResponseStatus(value= HttpStatus.NO_CONTENT)  // overwrites default 200, fixes the wrong
  // listing of 200 in swagger-ui (EHR-56)
  public ResponseEntity<VersionedCompositionResponseData>
    ...
    VersionedCompositionResponseData data =
        new VersionedCompositionResponseData(); // empty for now - set revision history!
  ...
  @ApiOperation(
      value = "Get versioned composition version by id and reference to revision history") // TODO
  // correct?
  // also
  // method
  // name
  @ApiResponses(
      value = {
        ...
        @ApiResponse(
            ...
            response = ErrorResponseData.class,
            message =
                "Not Found - No EHR with the supplied ehr_id or no VERSIONED_COMPOSITION with the supplied versioned_object_uid or no VERSION with the supplied versione_uid.")
      })
  // TODO @ResponseStatus(value= HttpStatus.NO_CONTENT)  // overwrites default 200, fixes the wrong
  // listing of 200 in swagger-ui (EHR-56)
  public ResponseEntity<CompositionResponseData> getCompositionByRevisionHistory(
      @ApiParam(value = "EHR identifier taken from EHR.ehr_id.value", required = true)
          @PathVariable(value = "ehr_id")
          String ehrIdString,
      @ApiParam(
          ...
          @PathVariable(value = "versioned_object_uid")
          String versionedObjectUidString,
      @ApiParam(value = "VERSIONED identifier taken from VERSIONED.uid.value", required = true)
          @PathVariable(value = "version_uid")
          String versionUid) {
    UUID ehrId = getEhrUuid(ehrIdString);
    UUID versionedObjectUid = getCompositionVersionedObjectUidString(versionedObjectUidString);
    ...
    CompositionResponseData data =
        new CompositionResponseData(null, null); // empty for now - set version!
  ...
  // TODO @ResponseStatus(value= HttpStatus.NO_CONTENT)  // overwrites default 200, fixes the wrong
  // listing of 200 in swagger-ui (EHR-56)
  public ResponseEntity<CompositionResponseData> getVersionedCompositionAtTime(
      @ApiParam(value = "EHR identifier taken from EHR.ehr_id.value", required = true)
          @PathVariable(value = "ehr_id")
          String ehrIdString,
      @ApiParam(
   ...
   * @return
   */
  private <T extends CompositionResponseData>
      Optional<InternalResponse<T>> buildCompositionResponseData(
          UUID compositionId,
          Integer version,
          String accept,
          URI uri,
          List<String> headerList,
          Supplier<T> factory) {
    // create either CompositionResponseData or null (means no body, only headers incl. link to
    // resource), via lambda request
    ...
    for (String header : headerList) {
      switch (header) { // no default because everything else can be ignored
        ...
        case ETAG:
          ...
          break;
        case LAST_MODIFIED:
          // TODO should be VERSION.commit_audit.time_committed.value which is not implemented yet -
          // mock for now
    ...
    // if (minimalOrRepresentation.getClass().equals(CompositionResponseData.class)) {     // TODO
    // make Optional.ofNull....
    if (minimalOrRepresentation != null) {
      // when this "if" is true the following casting can be executed and data manipulated by
      // reference (handled by temporary variable)
      ...
      if (accept.equals("*/*") || accept.isEmpty()) accept = MediaType.APPLICATION_JSON.toString();
      ...
      Optional<CompositionDto> compositionDto =
          compositionService.retrieve(compositionId, versionNumber);
      // TODO how to handle error situation here only with Optional? is there a better way without
      ...
      if (compositionDto.isPresent()) {
        ...
        // objByReference.setComposition(compositionService.serialize(compositionDto.get(),
        // format));
      } else {
