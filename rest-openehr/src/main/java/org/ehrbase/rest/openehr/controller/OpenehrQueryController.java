...
public class OpenehrQueryController extends BaseController {
  ...
  @ApiResponses(
      value = {
        ...
        @ApiResponse(
            code = 400,
            message =
                "Invalid input, e.g. a request with missing required field q or invalid query syntax."),
        @ApiResponse(code = 204, message = "The query didn't give any result.")
      })
  public ResponseEntity<QueryResponseData> getAdhocQuery(
      @ApiParam(value = REQ_ACCEPT) @RequestHeader(value = ACCEPT, required = false) String accept,
      @ApiParam(value = "AQL query to be executed", required = true) @RequestParam(value = "q")
          String query,
      @ApiParam(value = "row number in result-set to start result-set from (0-based), default 0")
          @RequestParam(value = "offset", required = false)
          Integer offset,
      @ApiParam(value = "number of rows to fetch, default depends on the implementation")
          @RequestParam(value = "fetch", required = false)
          Integer fetch,
      @ApiParam(value = "query parameters (can appear multiple times)") @RequestParam
          Map<String, Object> queryParameters) {

    // deal with offset and fetch
    if (fetch != null) query = withFetch(query, fetch);

    if (offset != null) query = withOffset(query, offset);
    ...
    if (query != null) {
      ...
      if (queryParameters != null && !queryParameters.isEmpty())
        queryResponseData =
            new QueryResponseData(queryService.query(query, queryParameters, QueryMode.AQL, false));
      else
        queryResponseData = new QueryResponseData(queryService.query(query, QueryMode.AQL, false));

      if (queryResponseData.getRows().size() > 0) return ResponseEntity.ok(queryResponseData);
      else return ResponseEntity.noContent().build();
    } else return missingRequestResponseEntity();
  }
  ...
  public ResponseEntity<QueryResponseData> postAdhocQuery(
      @ApiParam(value = REQ_ACCEPT) @RequestHeader(value = ACCEPT, required = false) String accept,
    ...
    if (aql != null) {
      ...
      if (parameters != null && !parameters.isEmpty())
        queryResponseData =
            new QueryResponseData(queryService.query(aql, parameters, QueryMode.AQL, false));
      else queryResponseData = new QueryResponseData(queryService.query(aql, QueryMode.AQL, false));
    } else return missingRequestResponseEntity();

    if (queryResponseData == null) return ResponseEntity.noContent().build();
    // NB. Empty result -> HTTP 200 with empty columns and rows (EtherCIS previously returned 204,
    // but I think it's wrong)
    //        else if (queryResponseData.getRows().size() == 0)
    //            return ResponseEntity.noContent().build();
    else return ResponseEntity.ok(queryResponseData);
  }
  ...
  @GetMapping(
      value = {
        "/{qualified_query_name}/{version}{?offset,fetch,query_parameter}",
        "/{qualified_query_name}{?offset,fetch,query_parameter}"
      })
  ...
  public ResponseEntity<QueryResponseData> getStoredQuery(
      @ApiParam(value = REQ_ACCEPT) @RequestHeader(value = ACCEPT, required = false) String accept,
      @ApiParam(
              value = "query name to be executed, example: org.openehr::compositions",
              required = true)
          @PathVariable(value = "qualified_query_name")
          String qualifiedQueryName,
      @ApiParam(value = "query version (SEMVER), default is LATEST")
    ...
    QueryDefinitionResultDto queryDefinitionResultDto =
        queryService.retrieveStoredQuery(
            qualifiedQueryName, version.isPresent() ? version.get() : "LATEST");
    ...
    if (queryResponseData == null) {
      ...
      queryResponseData.setName(
          queryDefinitionResultDto.getQualifiedName()
              + "/"
  ...
  @ApiResponses(
      value = {
        ...
        @ApiResponse(
            code = 412,
            message = "Precondition failed, ID given as If-None-Match header already exists.")
      })
  public ResponseEntity<QueryResponseData> postStoredQuery(
      @ApiParam(value = REQ_ACCEPT) @RequestHeader(value = ACCEPT, required = false) String accept,
      @ApiParam(value = REQ_CONTENT_TYPE_BODY, required = true) @RequestHeader(value = CONTENT_TYPE)
          String contentType,
      // TODO: what is this header about? couldn't be clarified and will be discussed with openEHR
      // REST API people
      @ApiParam(value = "use this ehrid") @RequestHeader(value = IF_NONE_MATCH, required = false)
          String ifNoneMatch,
      @ApiParam(
              value = "query name to be executed, example: org.openehr::compositions",
              required = true)
      ...
      @ApiParam(value = "query version (SEMVER), default is LATEST")
          @PathVariable(value = "version")
          Optional<String> version,
      @ApiParam(value = "parameters used to execute the query") @RequestBody(required = false)
          String parameterBody) {

    log.debug(
        "postStoredQuery with the following input: "
            + qualifiedQueryName
            + " - "
            + version
            + " - "
  ...
  public ResponseEntity badRequestResponseEntity(
      String qualifiedQueryName, Optional<String> version) {
    String errorBody =
        new ErrorBodyPayload(
                "Invalid query",
                "could not retrieve query identified by:"
                    + qualifiedQueryName
                    + "/"
                    + (version.isPresent() ? version.get() : "LATEST"))
            .toString();
  ...
  QueryResponseData invoke(String query, Map<String, Object> queryParameter) {
    ...
    if (queryParameter != null && !queryParameter.isEmpty()) {
      ...
      queryResponseData =
          new QueryResponseData(queryService.query(query, parameters, QueryMode.AQL, false));
    } else
      queryResponseData = new QueryResponseData(queryService.query(query, QueryMode.AQL, false));
