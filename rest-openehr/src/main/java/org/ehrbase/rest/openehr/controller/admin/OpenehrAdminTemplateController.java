...
@RestController
@RequestMapping(
    path = "/rest/openehr/v1/admin/template",
...
public class OpenehrAdminTemplateController extends BaseController {
  ...
  @Autowired AdminApiConfiguration adminApiConfiguration;
  ...
  @ApiResponses(
      value = {
        ...
        @ApiResponse(code = 404, message = "Template could not be found."),
        @ApiResponse(
            code = 422,
            message =
                "Template could not be replaced since it is used in at least one Composition.")
      })
  public ResponseEntity<String> updateTemplate(
      @ApiParam(value = REQ_ACCEPT)
          ...
          String accept,
      @ApiParam(value = REQ_CONTENT_TYPE) @RequestHeader(value = CONTENT_TYPE) String contentType,
      @ApiParam(
              value =
                  "Target template id to update. The value comes from the 'template_id' property.")
          @PathVariable(value = "template_id")
          String templateId,
      @ApiParam(value = "New template content to replace old one with") @RequestBody()
          String content) {
  ...
  @ApiResponses(
      value = {
        @ApiResponse(code = 202, message = "Template has been deleted successfully."),
        @ApiResponse(code = 401, message = "Client credentials are invalid or have expired."),
        @ApiResponse(
            code = 403,
            message = "Client has no access permission since admin role is missing."),
