...
## [unreleased]
 ### Added
- **GraphQL API**: Embedded Spring for GraphQL 2.0.2 with mutations (createEhr, createComposition, updateComposition, deleteComposition), subscriptions (onCompositionChange, onAuditEvent via PG LISTEN/NOTIFY), and dynamic template-driven queries with Relay-style cursor pagination
- **Schema Generator**: New `schema-generator` module that analyzes openEHR templates and generates PostgreSQL DDL (tables, history tables, views, search indexes) and GraphQL types automatically
- **Dynamic Query Layer**: Template upload triggers automatic DDL execution, view creation, view catalog registration, and GraphQL schema hot-reload — no restart required
- **Normalized Database Schema**: New `ehr_system`, `ehr_data`, `ehr_views` schemas replacing the legacy JSONB storage model with fully normalized PostgreSQL 18 tables using UUIDv7, temporal `WITHOUT OVERLAPS` constraints, and Row-Level Security
- **Versioning Engine**: Explicit app-code versioning with `composition_history` tables, using PostgreSQL's transaction-scoped `now()` for temporal continuity
- **PostgreSQL Full-Text Search**: Auto-generated `tsvector` columns and GIN indexes on clinical data tables for fast text search via GraphQL `TextSearch` filter
- **GraphiQL IDE**: Built-in GraphiQL UI at `/api/v2/graphiql` with introspection support (depth-limit exempt)
- **View Catalog**: `ehr_system.view_catalog` tracks all auto-generated views, enabling dynamic GraphQL schema generation at startup
- **Redis Support**: Optional distributed caching via Redis for multi-instance deployments
- **Docker Dev Setup**: `docker-compose-dev.yml` with PostgreSQL 18, Redis, and EHRbase for local development
 ### Changed
...
- REST API endpoints moved to `/api/v2/` prefix (EHR, composition, template, contribution, directory, admin)
- GraphQL endpoint path configured under `spring.graphql` (was incorrectly under `springdoc.graphql`)
- Connection types (edges, pageInfo, cursors) now auto-generated by Spring's `ConnectionTypeDefinitionConfigurer` — removed manual generation
- `GenericViewDataFetcher` returns `Window<Map>` for native Spring pagination adapter support
 ### Removed
- **AQL Engine**: Entire `aql-engine` module removed (125+ files) — replaced by GraphQL queries against auto-generated SQL views
- **Stored Queries**: Dropped stored query support — SQL views replace them
- **rm-db-format**: Legacy RM-to-database format conversion module removed — replaced by `schema-generator`
- **rest-openehr / rest-ehr-scape**: Legacy REST modules removed — replaced by `rest-api` with v2 endpoints
- **JSONB Storage**: Replaced with normalized PostgreSQL tables per template
- **Legacy Flyway Migrations**: Old V1–V23 migrations replaced with clean V1–V6 migration set for the new schema
 ### Fixed
...
- Fixed `tstzrange` type mismatch in versioning SQL (Java `OffsetDateTime` was passed as `varchar`)
- Fixed FK constraint violation during composition update/delete (clinical data now archived before composition metadata)
- Fixed `SELECT *` in archive queries copying PostgreSQL GENERATED ALWAYS columns to history tables
- Fixed temporal overlap in versioning caused by Java `now` vs PostgreSQL `now()` timing difference — all timestamps now use transaction-scoped `now()`
- Fixed `MaxQueryDepthInstrumentation` blocking GraphiQL introspection queries
